 ...
 *   2025-Jan-09 Add secondary ranges for UCA 17.0.
...
/*
 * CTT table ID. This value needs to be updated for each new version
 * of the table. It needs to match the id documented in Appendix B
 * of UTS #10.
 */
...
extern void dumpTimeStamp ( FILE *fd, char *commentchar );
...
"<D10EFA>",    /* double vertical bar below */
...
char localbuf[48];
    ...
    fputs ( localbuf, fd );
    sprintf ( localbuf, "%%   Generated from %s\n\n", GetUnidataFileName() );
    fputs ( localbuf, fd );
}
...
int d;
     ...
     * trees. The greatestDepth for the 17.0.0 datafile is 8912.
     */
...
int greatestdepth;
    ...
    rc = unisift_BuildSymWtTreeRange ( 0x12000, 0x143FF, &greatestdepth );
    if ( rc < 0 )
    {
        return ( rc );
    }
    ...
    rc = unisift_BuildSymWtTreeRange ( 0x1D800, 0x1E6FF, &greatestdepth );
    if ( rc < 0 )
    {
        return ( rc );
    }
...
void unisift_PrintSymWtTree ( FILE *fd )
{
    ...
    fputs ( "<RFB05> % Symbol for first element of computed weights for Seal ideographs\n", fd );
