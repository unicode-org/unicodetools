...
public class Typology {
    ...
    static class MyReader extends FileUtilities.SemiFileReader {
        ...
        protected boolean handleLine(int startRaw, int endRaw, String[] items) {
            ...
            final String[] labels = fullPath.split("/");
            String path = "";
    ...
    static {
        ...
        // fix the paths
        final Map<String, UnicodeSet> temp= new TreeMap<String, UnicodeSet>();
        for (int i = 0; i < ECharacterCategory.CHAR_CATEGORY_COUNT; ++i) {
            final UnicodeSet same = new UnicodeSet()
            .applyIntPropertyValue(UProperty.GENERAL_CATEGORY, i);
            final String gcName = UCharacter.getPropertyValueName(UProperty.GENERAL_CATEGORY, i, NameChoice.SHORT);
            //System.out.println("\n" + gcName);
            final String prefix = gcName.substring(0,1);

            for (final String path : path_to_uset.keySet()) {
                final UnicodeSet uset = path_to_uset.get(path);
                if (!same.containsSome(uset)) {
                    continue;
                }
                final String path2 = prefix + path;
                temp.put(path2, new UnicodeSet(uset).retainAll(same));
                final String[] labels = path2.split("/");
                String parent = "";
    ...
    private static Map<String, UnicodeSet> freezeMapping(Map<String, UnicodeSet> map) {
        for (final String key : map.keySet()) {
            final UnicodeSet uset = map.get(key);
            uset.freeze();
        }
