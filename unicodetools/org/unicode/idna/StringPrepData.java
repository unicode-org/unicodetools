        ...
        try {
            ...
            while (true) {
                ...
                if (line.startsWith("-----")) {
                    ...
                    final StringPrepData.Idna2003Table newTable = Idna2003Table.valueOf(tableDelimiter.group(2).replace(".","_"));
                    if (inTable) {
                        if (table != null) {
                ...
                final String endCodeString = lineMatcher.groupCount() < 2 ? null : lineMatcher.group(2);
                ...
                final int endCode = endCodeString == null ? startCode : Utility.fromHex(endCodeString,4," ").codePointAt(0);
            ...
            for (final UnicodeSetIterator it = new UnicodeSetIterator(IdnaTypes.U32); it.next();) {
                ...
                if (mapValue == null) {
                    ...
                    final String newValue = Idna.NFKC_3_2.transform(mapValue); // Normalizer.normalize(mapValue, Normalizer.NFKC, Normalizer.UNICODE_3_2);
                    if (DEBUG) {
                        System.out.println("Change for NFKC mapping of " + Utility.hex(i) + ", \t" + Utility.hex(mapValue) + " \t => \t" + Utility.hex(newValue));
                    }
                    addedMappings.add(i);
            ...
            if (DEBUG) {
                System.out.println("Adding NFKC mapping for " + addedMappings.toPattern(false) + ",\t" + addedMappings);
            }
            ...
            if (DEBUG) {
                System.out.println("Removing Identical mapping for " + identicals.toPattern(false) + ",\t" + identicals);
            }
            ...
            for (final String source : mappings) {
                ...
                if (wasProhibited != shouldBeProhibited) {
                    if (shouldBeProhibited) {
                        if (DEBUG) {
                            System.out.println("Changing to prohibited for " + source + "\t" + Utility.hex(cpSource) + ",\t" + shouldBeProhibited);
                        }
                        prohibited.add(cpSource);
                    } else {
                        if (DEBUG) {
                            System.out.println("Removing from prohibited for " + source + "\t" + Utility.hex(cpSource) + ",\t" + shouldBeProhibited);
                        }
