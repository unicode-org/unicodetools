...
public class Linkifier2 {
    ...
    private static class UrlScanner extends Scanner {
        ...
        public Status scan(String input, ParsePosition pos) {
            ...
            if (cp == PATH_START) {
                overallResult = pathScanner.scan(input, pos) == Status.INVALID ? Status.INVALID : overallResult;
            ...
            if (cp == QUERY_START) {
                overallResult = queryScanner.scan(input, pos) == Status.INVALID ? Status.INVALID : overallResult;
                int newPos = pos.getIndex();
                if (newPos != current) {
                    current = newPos;
                    cp = current < input.length() ? input.codePointAt(current) : 0;
            ...
            if (cp == FRAGMENT_START) {
                overallResult = fragmentScanner.scan(input, pos) == Status.INVALID ? Status.INVALID : overallResult;
                int newPos = pos.getIndex();
                if (newPos != current) {
                    current = newPos;
                    cp = current < input.length() ? input.codePointAt(current) : 0;
