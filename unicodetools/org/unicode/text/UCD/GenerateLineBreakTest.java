...
public class GenerateLineBreakTest implements UCD_Types {
    ...
    static byte getHangulType(int cp) {
        Default.ucd();
        if (UCD.isLeadingJamo(cp)) {
            return hL;
        }
        Default.ucd();
        if (UCD.isVowelJamo(cp)) {
            return hV;
        }
        Default.ucd();
        if (UCD.isTrailingJamo(cp)) {
            return hT;
        }
        Default.ucd();
        if (UCD.isHangulSyllable(cp)) {
            Default.ucd();
            if (UCD.isDoubleHangul(cp)) {
                return hLV;
            }
            return hLVT;
        }
    ...
    public void run() throws IOException {
        ...
        final String[] testCase = new String[50];
        // do main test
        ...
        for (int k = 0; k < 2; ++k) {
            ...
            for (int ii = 0; ii < getLimit(); ++ii) {
                final int i = TypeOrder[ii];
                if (i == LB_SG) {
                    continue;
                }
                final String before = samples[i];

                for (int jj = 0; jj < getLimit(); ++jj) {
                    Utility.dot(counter);
                    final int j = TypeOrder[jj];
                    if (j == LB_SG) {
                        continue;
                    }
                    final String after = samples[j];
                    // do line straight
                    final int len = genTestItems(before, after, testCase);
                    for (int q = 0; q < len; ++q) {
    ...
    public String getTypeID(int cp) {
        final byte result = getType(cp);
        if (result == LB_SUP) {
            return "SUP";
        }
        if (result >= LB_LIMIT) {
            return hNames[result - LB_LIMIT];
        }
        Default.ucd();
        return UCD.getLineBreakID_fromIndex(result);
    }
    ...
    public void generateTable(PrintWriter out, boolean recommended) {
        ...
        for (int i = 0; i < getTableLimit(); ++i) {
            ...
            final String h = getTypeID(samples[TypeOrder[i]]);
        ...
        for (int i = 0; i < getTableLimit(); ++i) {
            ...
            final String before = samples[type];
            ...
            for (int j = 0; j < getTableLimit(); ++j) {
                ...
                final String after = samples[type];
                final String t = getTableEntry(before, after, recommended, rule);
                String background = "";
                final String t2 = getTableEntry(before, after, !recommended, rule2);
                if (!t.equals(t2)) {
                    if (t.equals(NOBREAK)) {
    ...
    public String getTableEntry(String before, String after, boolean recommended, String[] ruleOut) {
        ...
        final boolean spaceBreak2 = isBreak(before + " " + after, before.length(), recommended);
        final String spaceRule2 = rule;
    ...
    public boolean isBreak(String source, int offset, boolean recommended) {
        ...
        if (before == LB_BK || before == LB_LF || before == LB_CR) {
            return true;
        }
        ...
        if ((before == LB_LV || before == LB_V) && (after == LB_V || after == LB_T)) {
            return false;
        }

        if ((before == LB_LVT || before == LB_T) && (after == LB_T)) {
            return false;
        }
        ...
        if (before == LB_CM) {
            setBase = true;
            final int backOffset = findLastNon(source, offset, LB_CM, recommended);
            if (backOffset < 0) {
                before = LB_ID;
            } else {
        ...
        if (before == LB_AL && after == LB_NU) {
            return false;
        }
        if (before == LB_NU && after == LB_AL) {
            return false;
        }
        ...
        if (before == LB_HY && after == LB_NU) {
            return false;
        }
        if (before == LB_IS && after == LB_NU) {
            return false;
        }
        if (before == LB_NU && after == LB_NU) {
            return false;
        }
        if (before == LB_NU && after == LB_PO) {
            return false;
        }

        if (before == LB_PR && after == LB_AL) {
            return false;
        }
        if (before == LB_PR && after == LB_HY) {
            return false;
        }
        if (before == LB_PR && after == LB_ID) {
            return false;
        }
        if (before == LB_PR && after == LB_NU) {
            return false;
        }
        if (before == LB_PR && after == LB_OP) {
            return false;
        }

        if (before == LB_SY && after == LB_NU) {
            return false;
        }
    ...
    static class GenerateWordBreakTest extends GenerateLineBreakTest {
        ...
        @Override
        public String getTypeID(int cp) {
            final byte type = getType(cp);
            if (type >= oLIMIT) {
                return hNames[type - oLIMIT];
            }
            return Names[type];
        }
        ...
        public byte getType(int cp) {
            ...
            if (linkProp.hasValue(cp)) {
                return Link;
            }
            if (extendProp.hasValue(cp)) {
                return Extend;
            }
            if (baseProp.hasValue(cp)) {
                return Base;
            }
