...
public class Segmenter {
        ...
        for (int i = 0; i < cannedRules.length; ++i) {
            if (cannedRules[i][0].equals(type)) {
                for (int j = 1; j < cannedRules[i].length; ++j) {
                    ...
                    try {
                        if (cannedRule.contains("Extended_Pictographic")) {
                            int debug = 0;
                        }
    ...
    public static class Builder {
        ...
        private UnicodeMap<String> samples = new UnicodeMap<String>();
        private final UnicodeSet extended_pictographic;

        public Builder(UnicodeProperty.Factory factory) {
            propFactory = factory;
            extended_pictographic = IndexUnicodeProperties.make().loadEnum(UcdProperty.Extended_Pictographic, Binary.class)
                    .getSet(Binary.Yes);
