...
public class GenerateEmoji {
	...
	static {
		...
		JSOURCES.addAll(dcmProp.keySet()).addAll(kddiProp.keySet()).addAll(sbProp.keySet())
		.removeAll(WHITESPACE.getSet(UcdPropertyValues.Binary.Yes.toString())).freeze();
		// if (true)
	...
	static public String getImage(Emoji.Source type, String charsForFile, String chars, boolean useDataUrl,
		...
		if (filename != null && new File(Emoji.IMAGES_OUTPUT_DIR, filename).exists()) {
			...
			+ (useDataUrl
					? " class='" + className + extraClasses + (age == Age_Values.Unassigned ? " new" : "") + "'"
	...
	public static void main(String[] args) throws IOException {
		...
		String[] replacementList = new String[] { "%%PLAIN_VERSION%%",
				Emoji.VERSION_STRING + " " + Emoji.BETA_TITLE_AFFIX, // "v4.0 —
				// Beta",
				"%%VERSION%%", Emoji.VERSION_STRING + " " + Emoji.BETA_HEADER_AFFIX, "%%LINK_OTHER%%",
				getPointToOther("index.html", "Unicode® Emoji Charts"), // "<i>For
				// the
				// current
				// released
				// version,
...
//		showSequences();
	...
	private static void showTextStyle(Visibility visibility) throws IOException {
		...
		final PrintWriter out = visibility == Visibility.external
				...
				final PrintWriter out2 = visibility == Visibility.internal
						...
						if (out != null)
							writeHeader(outFileName, out, "Text vs Emoji", null,
	...
	private static void showTextRow(PrintWriter outExternal, PrintWriter outInternal, Age_Values version,
			boolean minusDings, UnicodeSet current, UnicodeSet defaultText) {
		if (current.size() == 0) {
			return;
		}
		...
		if (outExternal != null) {
			if (emojiSet.size() > 50 || emojiSetVs.size() > 50 || textSetVs.size() > 50) {
				outExternal.println("<tr><th rowSpan='2'></th>"
						+ "<th width='60%' colSpan='2' style='text-align: center'>Emoji Presentation</th>"
						+ "<th width='30%' style='text-align: center'>Text Presentation</th>" + "</tr>" + "<tr>"
						...
						+ "<th width='30%' style='text-align: center'><i>Has and needs EPSq</i></th>" + "</tr>");
			}
	...
	private static void showOrdering(Style style, Visibility visibility, boolean showUca) throws IOException {
		...
		writeHeader(outFileName, out, "Emoji Ordering", null,
				"<p>This chart shows the default ordering of emoji characters from " + CLDR_DATA_LINK + ". "
						...
						+ (showUca ? " (shown at the right)" : "") + ", by grouping similar items together. "
						...
						+ "<a target='text' href='emoji-ordering-rules.txt'>emoji-ordering-rules.txt</a>" + "</p>\n"
						+ (showUca
								? "<p>The cell divisions for the Default Unicode Collation Order are Unicode code-chart blocks. </p>\n"
										: ""),
						"border='1'", true, false);
	...
	private static void showDefaultStyle() throws IOException {
		...
		writeHeader(outFileName, out, "Emoji Default Style Values", null,
	...
	private static void showVariationSequences() throws IOException {
		...
		PrintWriter out = FileUtilities.openUTF8Writer(Emoji.CHARTS_DIR, outFileName);
		writeHeader(outFileName, out, "Emoji Presentation Sequences", "",
				"<p>This chart specifies the exact list of " + "<i>emoji presentation sequences</i>. "
						...
						+ "the desired appearances for each of these selectors. " + "For more information, see "
						+ "<i><a target='doc' href='" + TR51_HTML + "#Definitions'>Definitions</a></i> "
						+ "in <i><a target='doc' href='" + TR51_HTML + "'>UTR #51: Unicode Emoji</a></i>. " + "</p><p>"
		...
		out.println("<tr><th class='cchars'>Code</th>" + "<th class='narrow cchars'>Sample Text (+FE0E)</th>"
				+ "<th class='narrow cchars'>Sample Emoji (+FE0F)</th>" + "<th>Version</th>" + "<th>Name</th></tr>");
	...
	private static void showSequences() throws IOException {
		...
		try (PrintWriter out = FileUtilities.openUTF8Writer(Emoji.CHARTS_DIR, outFileName)) {
			writeHeader(outFileName, out, "Emoji Sequences", null,
		...
		try (PrintWriter out = FileUtilities.openUTF8Writer(Emoji.CHARTS_DIR, outFileName)) {
			writeHeader(outFileName, out, EMOJI_ZWJ_SEQUENCES_TITLE, null,
					"<p>This page provides a " + CATALOG + " emoji zwj sequences. "
	...
	private static void displayZwjTD(PrintWriter out, String title, UnicodeSet items) {
		...
		out.println(
				"<tr><th>Count</th><th style='width:10em'>Code Points</th><th>Browser</th><th>Sample Image</th><th>Sample Fallback Images</th><th>Description</th></tr>");
	...
	static class VersionData implements Comparable<VersionData> {
		...
		public int compareTo(VersionData o) {
			Comparator foo;
			return ComparisonChain.start().compare(setCharSource, o.setCharSource, ccc)
					.compare(versionInfo, o.versionInfo).result();
		}
	...
	private static void showVersions() throws IOException {
		...
		PrintWriter out = FileUtilities.openUTF8Writer(Emoji.CHARTS_DIR, outFileName);
		writeHeader(outFileName, out, "Emoji Versions &amp; Sources", null,
		...
		for (VersionData value : sorted) {
			...
			displayUnicodesetTD(out, Collections.singleton(value.getCharSources()), null,
					ImmutableSet.of(showVersionOnly(value.versionInfo),
							String.valueOf(VersionToAge.getYear(value.versionInfo)), String.valueOf(chars.size())),
	...
	private static void showVersionsOnly() throws IOException {
		...
		PrintWriter out = FileUtilities.openUTF8Writer(Emoji.CHARTS_DIR, outFileName);
		writeHeader(outFileName, out, "Emoji Versions", null,
				"<p>This chart shows when each emoji first appeared in a Unicode version. "
		...
		for (Age_Values value : sorted) {
			UnicodeSet chars = m.getSet(value);
			displayUnicodesetTD(out, Collections.singleton(showVersionOnly(value)), VersionToAge.getYear(value) + "",
	...
	private static void showAnnotations(String dir, String outFileName, UnicodeSet filterOut,
		...
		try (PrintWriter out = FileUtilities.openUTF8Writer(dir, outFileName)) {
			writeHeader(outFileName, out, "Emoji Annotations", null,
	...
	private static final String HOVER_INSTRUCTIONS = "Emoji without available skintone show a small swatch afterwards;"
			+ " without available images are shown as ✘. "
	...
	enum Form {
		noImages("This chart provides a list of the Unicode emoji characters, with single image and annotations."
				...
				+ "The ordering of the emoji and the annotations are based on " + CLDR_DATA_LINK
				+ ". Emoji sequences have more than one code point in the <b>Code</b> column."), 
		fullForm(
				"This chart provides a list of the Unicode emoji characters, with images from different vendors, version and source information, default style, and annotations. "
						+ "The ordering of the emoji and the annotations are based on " + CLDR_DATA_LINK
						+ ". Emoji sequences have more than one code point in the <b>Code</b> column."), 
		onlyNew(
				"This chart provides a list of the Unicode emoji characters that have been added to the most recent version of Unicode Emoji. "
	...
	public static void writeHeader(String outFileName, PrintWriter out, String title, String styles, String firstLine,
			String tableAttrs, boolean showGeneralComments, boolean skipVersion) {
		...
		out.println(
				"<!DOCTYPE html PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\" \"http://www.w3.org/TR/html4/loose.dtd\">\n"
						...
						+ (styles == null ? "" : "<style>\n" + styles + "\n</style>\n") + "</head>\n" + "<body>\n"
						+ UNICODE_HEADER + getButton() + "<h1>" + fullTitle
						+ (skipVersion ? "" : Emoji.BETA_HEADER_AFFIX) + "</h1>\n"
						+ (skipVersion ? "" : getPointToOther(outFileName, title))
						// + "<p><b>" + chartIndex + "</b></p>\n"
						+ firstLine
						+ "<p class='indent'>For information about the images used in these charts, see <a href='../images.html'>Emoji Images and Rights</a>. "
						+ "For details about the format and fields, see " + chartIndex + ". "
						+ "Support of emoji is not required for conformance to the Unicode Standard — for more information about emoji, see "
						+ UTR_LINK + ". " + (showGeneralComments ? HOVER_INSTRUCTIONS : "")
	...
	public static String toHtmlString(String chars2, Form form, int item) {
		...
		return "<tr>" + "<td class='rchars'>" + item + "</td>\n" + "<td class='code'>"
		+ getDoubleLink(anchor, Emoji.toUHex(chars2WithVS)) + "</td>\n"
		+ (form == Form.noImages ? altClass(bestCell)
				: altClass(browserCell) + altClass(symbolaCell) + otherCells)
		+ "<td class='name'>" + name2 + "</td>\n"
		+ (form == Form.noImages ? ""
				: "<td class='age'>" + VersionToAge.getYear(Emoji.getNewest(chars2))
				+ getSources(chars2, new StringBuilder(), true) + "</td>\n" + "<td class='name'>"
				+ getAnnotationsString(chars2) + "</td>\n"
	...
	public static String toHtmlHeaderString(Form form) {
		...
		return "<tr>" + "<th class='rchars'><a target='text' href='index.html#col-num'>№</a></th>\n"
		...
		+ (form == Form.noImages ? ""
				: "<th><a target='text' href='index.html#col-data'>Date</a></th>\n"
				+ "<th><a target='text' href='index.html#col-annotations'>Keywords</a></th>\n"
				// + "<th><a target='text'
				// href='index.html#col-default'>Default</a></th>\n"
				)
	...
	static void showCandidateStyle(CandidateStyle candidateStyle, String outFileName, UnicodeSet emoji)
		...
		MajorGroup lastMajorGroup = null;
		String header = "<tr>" + "<th><a target='text' href='index.html#col-num'>№</a></th>"
		...
		for (String source : sorted) {
			...
			if (majorGroup != lastMajorGroup) {
				output.add("<tr><th colspan='8' class='bighead'>" + getDoubleLink(majorGroup.toString() + "")
				+ "</th></tr>");
				lastMajorGroup = majorGroup;
			}
			...
			String color = SPACE_JOINER.join(getImage(Source.google, source, true, ""),
					...
					getImage(Source.adobe, source, true, ""), getImage(Source.sample, source, true, ""));
			if (color.isEmpty()) {
...
//				+ "The representative chart glyphs (black and white) are not yet available for most of these emoji.</p>";
		...
		if (!future) {
			topHeader = "<p>This chart shows the emoji that have been added to this release of Unicode Emoji. "
		...
		try (PrintWriter out = FileUtilities.openUTF8Writer(Emoji.RELEASE_CHARTS_DIR, outFileName);) {
			writeHeader(outFileName, out, title, null, topHeader, "border='1'", true, true);
