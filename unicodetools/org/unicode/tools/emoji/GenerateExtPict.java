...
public class GenerateExtPict {
    ...
    public static void main(String[] args) {
        ...
        UnicodeSet unassigned = iup.loadEnumSet(UcdProperty.General_Category, General_Category_Values.Unassigned);
        ...
        UnicodeSet extpict = new UnicodeSet()
        ...
        try (PrintWriter out = FileUtilities.openUTF8Writer(Emoji.TR51_INTERNAL_DIR, filename)) {
            ...
            for (String s : extpict) {
                Cat cat = emojiPres.contains(s) ? Cat.E 
                        : emoji.contains(s) ? Cat.T
                                : Cat.X;
                ...
                out.println("<tr>"
                        + "<td class='code'>" + Utility.hex(s) + "</td>"
                        + "<td class='chars'>" + s + "</td>"
                        + "<td class='andr'>" + plain + "</td>"
                        + "<td class='andr'" + (cat != Cat.X ? "" : "style='background-color:LightGray'")
                        + ">" + color + "</td>"
                        + "<td class='name' " 
                        + (cat == Cat.E ? " style='background-color:LightGreen'>EPres" 
                                : cat == Cat.T ? ">TPres" 
                                        : " style='background-color:LightGray'>n/a")
                        + "</td>"
                        + "<td class='name'>" + iup.getName(s, "+") + "</td>"
                        + "</tr>");
            }
