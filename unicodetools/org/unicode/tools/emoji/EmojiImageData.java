...
public class EmojiImageData {
    ...
    public static void write(Set<Source> platforms2) throws IOException {
        ...
        try (PrintWriter out = FileUtilities.openUTF8Writer(Emoji.TR51_INTERNAL_DIR, "missing-emoji-list.html")) {
    ...
    private static final class Breakdown {
        ...
        static void add(List<Breakdown> result, String title, UnicodeSet v3, UnicodeSet v4) {
            final UnicodeSet old = new UnicodeSet(v3).retainAll(v4).freeze();
            result.add(new Breakdown(title + "\tv3.0", old));
            result.add(new Breakdown(title + "\tv4.0", new UnicodeSet(v4).removeAll(old).freeze()));
        }
    ...
    private static List<Breakdown> getBreakdown() {
        ...
        Breakdown.add(result, "keycaps",
                last.getKeycapSequences(),
                current.getKeycapSequences()
                );
        Breakdown.add(result, "flags",
                last.getFlagSequences(),
                current.getFlagSequences()
                );
        Breakdown.add(result, "modifiers",
                last.getModifierSequences(),
                current.getModifierSequences()
                );
        Breakdown.add(result, "zwj",
    ...
    private static UnicodeSet getCounts(PrintWriter out, Source source, Breakdown breakdown, int MAX) {
        ...
        if (!lastMissingSingletons.isEmpty()) {
            out.println(source + "\t"
                    ...
                    + "\t" + max(lastMissingSingletons.toPattern(false), MAX)
                    );
        }
