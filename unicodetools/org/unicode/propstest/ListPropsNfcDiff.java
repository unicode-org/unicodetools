...
public class ListPropsNfcDiff {
    ...
    public static void main(String[] args) {        
        System.out.println("Property\tType\tCount\tSource\tValue(source)\tValue(nfc(source)) or Value(nfkc(source))");
        ...
        for (UcdProperty prop : toTest) {
            ...
            TreeMultimap<Pair<String, String>, String> nfcDiffMap = findDiffs(nfcDiff, prop); // TreeMultimap.create();
        ...
        System.out.println("NFC ≠ CP:\tTOTAL\t" + nfcDiff.size() + "\t" + nfcDiff.keySet().toPattern(false));
        System.out.println("NFKC ≠ NFC:\tTOTAL\t" + nfkcDiff.size() + "\t" + nfkcDiff.keySet().toPattern(false));
    ...
    private static void showDiffs(UcdProperty prop, String title, TreeMultimap<Pair<String, String>, String> nfcDiffMap, Set<String> totals, EnumSet<UcdProperty> hasDiffs) {
        for (Entry<Pair<String, String>, Collection<String>> entry : nfcDiffMap.asMap().entrySet()) {
    ...
    private static TreeMultimap<Pair<String, String>, String> findDiffs(UnicodeMap<String> nfcDiff, UcdProperty prop) {
    ...
    private static void filterProps(EnumSet<UcdProperty> toTest, EnumSet<UcdProperty> toSkip) {
        for (UcdProperty prop : iup.getAvailableUcdProperties()) {
            switch (prop) {
            default: if (!prop.toString().startsWith("k")) {
                break;
            }
            case Age:
            case Block:
    ...
    private static void getChanges(UnicodeMap<String> nfcDiff, UnicodeMap<String> nfkcDiff) {
        ...
        for (int cp = 0; cp <= 0x10FFFF; ++cp) {
            ...
            if (singleCodePoint2 != Integer.MAX_VALUE 
                    && singleCodePoint2 != singleCodePoint) {
