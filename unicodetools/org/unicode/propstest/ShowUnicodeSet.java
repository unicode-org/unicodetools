...
public class ShowUnicodeSet {
    ...
    private static UnicodeSet BLANKS = new UnicodeSet("[\\p{defaultignorablecodepoint}\\p{whitespace}\\p{Z}\\p{Cc}\\p{Cf}-\\p{Cn}-\\p{block=Tags}-[\\x{20}\\x{600}-\\x{603}]]").freeze(); 
	...
	locales.parallelStream().forEach(localeStr -> {
		...
		for (Integer cp : blankCount) {
		    System.out.println(new StringBuffer().append(localeStr)
			    .append('\t').append(blankCount.get(cp))
			    .append("\t\\x{").append(Utility.hex(cp)).append('}')
			    .append("\t ").appendCodePoint(cp).append(' ')
			    .append('\t').append(iup.getName(cp))
			    );
		}
	    }
	});
	...
	String rules = CLDRTransforms.getIcuRulesFromXmlFile(CLDRPaths.COMMON_DIRECTORY + "transforms/", "Any-Superscript.xml", new ParsedTransformID());
	Transliterator toSuperscript = Transliterator.createFromRules("any-superscript", rules, Transliterator.FORWARD);
	...
	toSuperscript.getSourceSet().forEach(x -> {
	...
	try {
	    ...
	    //	    }
	    us = new UnicodeSet("[\\p{defaultignorablecodepoint}\\p{Z}\\p{whitespace}\\p{Cc}\\p{Cf}-[\\u0020]-\\p{Cn}-\\p{block=Tags}-\\p{Deprecated}]");
	}
	...
	UnicodeMap<Set<Script_Values>> script = iup.loadEnumSet(UcdProperty.Script_Extensions, Script_Values.class);
	...
	//	        "any-show", "[\\p{dicp}\\p{Z}\\p{whitespace}]-\\p{ascii}] > &hex($0)", Transliterator.FORWARD);
	...
	for (String s : us) {
	    ...
	    System.out.println("\\u{" + Utility.hex(s,1) + "} ↔︎ "
