...
public class ScriptCategories2 {
    ...
    public static final UnicodeSet IPA =
            (UnicodeSet)
                    new UnicodeSet(
                                    "[a-zæçðøħŋœǀ-ǃɐ-ɨɪ-ɶ ɸ-ɻɽɾʀ-ʄʈ-ʒʔʕʘʙʛ-ʝʟʡʢ ʤʧʰ-ʲʴʷʼˈˌːˑ˞ˠˤ̀́̃̄̆̈ ̘̊̋̏-̜̚-̴̠̤̥̩̪̬̯̰̹-̽͜ ͡βθχ↑-↓↗↘]")
                            .freeze();

    public static final UnicodeSet IPA_EXTENSIONS =
            (UnicodeSet) new UnicodeSet("[ɩɷɼɿʅ-ʇʓʖʗʚʞʠʣʥʦʨ-ʯ]").freeze();

    public static final UnicodeSet DEPRECATED_NEW =
            (UnicodeSet)
                    new UnicodeSet(
                                    "[[:deprecated:][\\u0149\\u0F77\\u0F79\\u17A4\\u2329\\u232A]-[\\u0340\\u0341\\u17D3]]")
                            .freeze();
    // removing 0340, 0341, 17D3, and adding 0149, 0F77, 0F79, 17A4, 2329, 232A

    // TODO - change to Blocks
    public static final UnicodeSet ARCHAIC_31 =
            (UnicodeSet)
                    new UnicodeSet(
    ...
    public static final UnicodeSet ARCHAIC_39 =
            (UnicodeSet)
                    new UnicodeSet(
    ...
    public static final UnicodeSet ARCHAIC_HEURISTIC =
            (UnicodeSet)
                    new UnicodeSet(
                                    "[ "
                                            + "[:blk=Ancient_Symbols:]"
    ...
    public static final UnicodeSet ARCHAIC_ADDITIONS =
            (UnicodeSet)
                    new UnicodeSet(
                                    "[ "
                                            + "[:blk=Aegean_Numbers:] "
    ...
    public static final UnicodeSet ARCHAIC =
            (UnicodeSet)
                    new UnicodeSet(ARCHAIC_31)
                            .addAll(ARCHAIC_39)
    ...
    static {
        UnicodeSet knownOk =
                new UnicodeSet(
                        "[\u0392\u0398\u03A0\u03A6\u03B2\u03B8\u03C0\u03C6[\u10C7\u10CD]\u1C88]");
        final UnicodeSet caseProblems =
                new UnicodeSet(ARCHAIC)
                        .closeOver(UnicodeSet.CASE)
                        .removeAll(ARCHAIC)
                        .removeAll(knownOk);
    ...
    public static final UnicodeSet SCRIPT_CHANGED =
            (UnicodeSet)
                    new UnicodeSet(
    ...
    public static final UnicodeSet CATEGORY_CHANGED =
            (UnicodeSet)
                    new UnicodeSet(
    ...
    public static UnicodeSet applyPropertyAlias(
            String propertyName, String propertyValue, UnicodeSet result) {
        UnicodeSet original;
        if (DEBUG) {
    ...
    public static final Set<String> MIDDLE_EASTERN =
            ScriptMetadata.Groupings.MIDDLE_EASTERN.scripts;
    public static final Set<String> SOUTH_ASIAN = ScriptMetadata.Groupings.SOUTH_ASIAN.scripts;
    public static final Set<String> SOUTHEAST_ASIAN =
            ScriptMetadata.Groupings.SOUTHEAST_ASIAN.scripts;
    ...
    public static final Set<String> HISTORIC_SCRIPTS =
            With.in(ScriptMetadata.getScripts())
                    .toUnmodifiableCollection(
                            new UsageFilter(IdUsage.EXCLUSION, IdUsage.LIMITED_USE), new TreeSet());
    ...
    //        "old italic");
    //
    //    public static final Set<String> OLD_MIDDLE_EASTERN = loadUnmodifiable(new
    // TreeSet<String>(),
    ...
    //        "Sinhala", "Tibetan", "Phags-Pa", "Limbu", "Sylo", "Kharoshthi", "lepcha",
    // "saurashtra", "ol chiki"
    //        );
    //    public static final Set<String> OLD_SOUTHEAST_ASIAN = loadUnmodifiable(new
    // TreeSet<String>(),
    ...
    //        );
    //
    //    public static final Set<String> OLD_HISTORIC_SCRIPTS = loadUnmodifiable(new
    // TreeSet<String>(),
    //        "Buginese", "Buhid", "Carian", "Coptic", "Cypriot", "Deseret", "Glagolitic",
    //        "Gothic", "Hanunoo", "Old_Italic", "Kharoshthi", "Linear_B", "Lycian", "Lydian",
    //        "Ogham", "Osmanya", "Phags_Pa", "Phoenician", "Rejang", "Runic", "Shavian",
    // "Sundanese",
    //        "Syloti_Nagri", "Syriac", "Tagbanwa", "Tagalog", "Ugaritic", "Old_Persian",
    // "Cuneiform"
    //        );
    ...
    static Map<RemapType, Map<String, UnicodeSet>> getRemapData(String filename)
            throws IOException {
        BufferedReader in = new BufferedReader(new FileReader(filename));
        Map<RemapType, Map<String, UnicodeSet>> data =
                new TreeMap<RemapType, Map<String, UnicodeSet>>();
            ...
            try {
                if (line.startsWith("@")) { // reassignment
                    ...
                    if (!line.equalsIgnoreCase("no-change")) {
                        ...
                        for (String part : parts) {
                            ...
                            try {
                                ...
                                fixed =
                                        getFixedPropertyValue(
                                                UProperty.SCRIPT, part, UProperty.NameChoice.LONG);
                            ...
                            if (remapType == RemapType.NONE) {
                                ...
                                throw new IllegalArgumentException(
                                        "Mixing Script and Category on one line: " + line);
                            }
                    ...
                    for (String fixed : propertiesToAddTo) {
                        addToMapToUnicodeSet(
                                data.get(remapType),
                                fixed,
                                new UnicodeSet().add(Integer.parseInt(line, 16)));
                    }
                    // data.composeWith(new UnicodeSet().add(Integer.parseInt(line,16)),
    ...
    private static <T> void addToMapToUnicodeSet(
            Map<T, UnicodeSet> mapToUnicodeSet, T key, UnicodeSet additions) {
    ...
    private static UnicodeSet getChanged(
            Map<RemapType, Map<String, UnicodeSet>> data, RemapType remapType) {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (String item :
                Arrays.asList(
                        ...
                        "SOUTHEAST_ASIAN")) {
            Collection<String> c =
                    item.equals("EUROPEAN")
                            ? EUROPEAN
                            : item.equals("MIDDLE_EASTERN")
                                    ? MIDDLE_EASTERN
                                    : item.equals("AFRICAN")
                                            ? AFRICAN
                                            : item.equals("SOUTH_ASIAN")
                                                    ? SOUTH_ASIAN
                                                    : item.equals("EAST_ASIAN")
                                                            ? EAST_ASIAN
                                                            : item.equals("AMERICAN")
                                                                    ? AMERICAN
                                                                    : item.equals("SOUTHEAST_ASIAN")
                                                                            ? SOUTHEAST_ASIAN
                                                                            : null;
    ...
    private static void checkHistoricClosure() {
        ...
        for (int pValue = UCharacter.getIntPropertyMinValue(UProperty.SCRIPT);
                pValue <= UCharacter.getIntPropertyMaxValue(UProperty.SCRIPT);
                pValue++) {
            String pValueName =
                    UCharacter.getPropertyValueName(
                            UProperty.SCRIPT, pValue, UProperty.NameChoice.LONG);
    ...
    private static void testNormalizationConsistency(
            String title, UnicodeSet testSet, Normalizer.Mode mode) {
        ...
        if (inSet.size() != 0 || inDecompSet.size() != 0) {
            ...
            System.out.println(
                    "// \t In decomps, but not in ordinary: " + inDecompSet.toPattern(false));
        }
    }
        ...
        for (RemapType r : data.keySet()) {
            ...
            System.out.println(
                    "public static final UnicodeSet "
                            + r
                            + "_CHANGED = (UnicodeSet) new UnicodeSet(\""
                            + changed.toString().replace("\\", "\\\\")
                            + "\").freeze();");
            System.out.println(
                    "public static final Map<String,UnicodeSet> "
                            + r
                            + "_NEW;\n"
                            + "static {\n"
                            + "String[][] data = {");
