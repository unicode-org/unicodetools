...
public class CheckComparison {
    ...
    private static void showTimes(final ULocale myLocale, final int dataSize) {
        ...
        System.out.println(
                "Construction Time: "
                        + directTimes2[0]
                        + showPercent(directTimes2[0], directTimes[0]));
        System.out.println(
                "Query Time: " + directTimes2[1] + showPercent(directTimes2[1], directTimes[1]));
        ...
        System.out.println(
                "Construction Time: "
                        + sortKeyTimes[0]
                        + showPercent(sortKeyTimes[0], directTimes[0]));
        System.out.println(
                "Query Time: " + sortKeyTimes[1] + showPercent(sortKeyTimes[1], directTimes[1]));
    }
    ...
    private static void timeSortkeyComparison(
            Collator collator,
            final int constructionIterations,
            final int queryIterations,
            long[] times) {
        final Map<RawCollationKey, String> sortKeyComparison =
    ...
    private static void timeDirectComparison(
            Collator collator,
            final int constructionIterations,
            final int queryIterations,
            long[] times) {
    ...
    private static void timeDirectComparison2(
    ...
    private static Collator prepareData(final ULocale myLocale, int maxCount) {
        ...
        main:
        for (final String languageCode : ULocale.getISOLanguages()) {
            ...
            for (int i = UScript.COMMON; i < UScript.CODE_LIMIT; ++i) {
                ...
                for (final String countryCode : ULocale.getISOCountries()) {
                    final String countryName =
                            ULocale.getDisplayCountry("und-" + countryCode, myLocale);
                    final String someString = languageName + ", " + scriptName + ", " + countryName;
                    final String someString2 =
                            scriptName + ", " + countryName + ", " + languageName;
