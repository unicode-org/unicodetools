...
public class Misc {
    public static void main(String[] args) throws ParseException {
        ...
        am =
                MapBuilder.of(new TreeMap<String, Integer>(col))
                        .put("A", 1)
                        .put("B", 2)
                        .unmodifiable();
        cm =
    ...
    private static final class RegionData {
        ...
        public RegionData(
                Region continentalContainer,
                Set<Region> contained,
                Set<Region> territoriesContained) {
            super();
        ...
        public static void load() {
            final HashMap<EnumSet<Region>, EnumSet<Region>> internCache =
                    new HashMap<EnumSet<Region>, EnumSet<Region>>();
                ...
                switch (r) {
                    case U001:
                        r.data =
                                new RegionData(
                                        null,
                                        intern(internCache, EnumSet.of(Region.US, Region.IT)),
                                        null);
                        break;
                    case US:
                    case IT:
                        r.data = new RegionData(Region.U001, null, null);
                        break;
                }
            }
        }
            ...
            for (final String expression :
                ...
                for (final boolean caseless : new boolean[] {false, true}) {
                    final boolean matches =
                            Pattern.compile(expression, caseless ? Pattern.CASE_INSENSITIVE : 0)
                                    .matcher(string)
                                    .matches();
                    System.out.format(
                            "expression: %s,\tcaseless: %s, \tstring: \"%s\",\tresult: %s\n",
    ...
    static final UnicodeSet ALLOW_IN_NAME =
            new UnicodeSet(
                            "["
    ...
    static final Normalizer2 NFKC =
            Normalizer2.getNFKCInstance(); // Normalizer2.getInstance(null, "NFKC", Mode.COMPOSE);

    static final SpoofChecker spoofChecker =
            new SpoofChecker.Builder()
                    .setChecks(SpoofChecker.SINGLE_SCRIPT | SpoofChecker.INVISIBLE)
                    .build();
    ...
    private static boolean checkName(String possibleName, ULocale locale, List<String> reasons) {
        ...
        if (!ALLOW_IN_NAME.containsAll(possibleName)) {
            reasons.add(
                    "Contains disallowed characters: "
                            + new UnicodeSet().addAll(possibleName).removeAll(ALLOW_IN_NAME));
            result = false;
        }
        // now check for NFKC
        ...
        // should add another check, that we don't have multiple whitespace characters in a row, and
        // don't start or end with whitespace
    ...
    public static boolean isInterchangeValid(CharSequence s) {
        ...
        while (index < length) {
            final int cp =
                    !((c1 = s.charAt(index++)) >= Character.MIN_HIGH_SURROGATE
    ...
    public static int codePointAt(CharSequence s, int index) {
        char c1, c2;
        return !Character.isHighSurrogate(c1 = s.charAt(index++))
                ? c1
                : index < s.length() && Character.isLowSurrogate(c2 = s.charAt(index))
                        ? Character.toCodePoint(c1, c2)
                        : c1;
    }
        ...
        public M unmodifiable() {
            final M result =
                    map instanceof SortedMap
                            ? (M) Collections.unmodifiableSortedMap((SortedMap<K, V>) map)
        ...
        public C unmodifiable() {
            ...
            final C result =
                    collection instanceof SortedSet
                            ? (C) Collections.unmodifiableSortedSet((SortedSet<E>) collection)
                            : collection instanceof Set
                                    ? (C) Collections.unmodifiableSet((Set<E>) collection)
                                    : collection instanceof List
                                            ? (C) Collections.unmodifiableList((List<E>) collection)
