...
public class UnicodeDataOutput {
    ...
    /**
     * Writes a UnicodeSet as: the count of ranges (int), each pair of ranges (int), a count of
        ...
        for (final UnicodeSetIterator it = new UnicodeSetIterator(toWrite); it.nextRange(); ) {
            if (it.codepoint != UnicodeSetIterator.IS_STRING) {
                ...
                if (firstString) {
                    output.writeInt(
                            toWrite.size()
                                    - count); // write terminator. negatives will not occur above.
                    firstString = false;
                }
    ...
    public abstract static class ItemWriter<T> {
         ...
         * @throws IOException
         */
        public Map<T, Integer> writeArray(DataOutput output, Collection<T> values)
                throws IOException {
     ...
     * The transitions and corresponding value indexes are written separately to allow compression
     * of the former.
     *
     * @param <T>
     * @param output
     ...
     * @throws IOException
     */
    public <T> void writeUnicodeMap(UnicodeMap<T> toWrite, ItemWriter<T> writer)
            throws IOException {
        writeUnicodeMap(toWrite, writer, output);
    }

    public static <T> void writeUnicodeMap(
            UnicodeMap<T> toWrite, ItemWriter<T> writer, DataOutput dataOutput) throws IOException {
        // values
