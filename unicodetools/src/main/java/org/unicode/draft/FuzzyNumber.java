...
public class FuzzyNumber {
    ...
    public FuzzyNumber multiply(FuzzyNumber other) {
        // optimize later
        final double a = lower * other.lower;
        final double b = lower * other.upper;
        final double c = upper * other.lower;
        final double d = upper * other.upper;
        final double min = Math.min(a, Math.min(b, Math.min(c, d)));
        final double max = Math.max(a, Math.max(b, Math.max(c, d)));
