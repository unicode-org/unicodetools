...
public final class Punycode {
        ...
        for (int i = 0; i < length; ++i) {
            ...
            if (c > 0xFF) {
                throw new IllegalArgumentException(
                        "Illegal character, must be 0..FF: " + Integer.toHexString(c));
            }
            if (basicToDigit[c] >= 0) {
                throw new IllegalArgumentException(
                        "Illegal character, cannot repeat in string: " + Integer.toHexString(c));
            }
            basicToDigit[c] = i;
            digitToBasic2[i] = c;
        }
     ...
     * @draft
     */
    public StringBuilder encode(CharSequence src, StringBuilder dest)
        ...
        for (handledCPCount = basicLength; handledCPCount < srcCPCount; /* no op */ ) {
             ...
             */
            if (nextLargerCodePoint - cpBeingEncoded
                    > (0x7fffffff - srcLength - delta) / (handledCPCount + 1)) {
                throw new IllegalStateException("Internal program error");
            }

            // delta is composed of the delta to the next character * gap (the characters so far)
            // below, we'll also add to it slowly as we find each new identical character in the
            // input
     ...
     * @draft
     */
    public StringBuffer decode(CharSequence src, StringBuffer dest)
            throws StringPrepParseException {
        final int srcLength = src.length();
    ...
    String valueOfCodePoint(int codepoint) {
        return codepoint < 0x10000
                ...
                : new StringBuilder(2).appendCodePoint(codepoint).toString();
    }

    StringBuilder insertCodePoint(StringBuilder target, int offset, int codepoint) {
        return codepoint < 0x10000
                ? target.insert(offset, (char) codepoint)
                : target.insert(offset, valueOfCodePoint(codepoint));
    }
