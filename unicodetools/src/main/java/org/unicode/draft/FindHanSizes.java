...
public class FindHanSizes {
    ...
    static class EncodingInfo {
        ...
        public UnicodeMap<EnumSet<NamedHanSet>> showContents() {
            ...
            for (final EnumSet<NamedHanSet> value : set) {
                ...
                System.out.println(
                        NamedHanSet.toString(value)
                                + "\t"
                                + keys.size()
                                + "\t"
                                + toAbbreviated(SHOW_LIMIT, keys));
            }
            System.out.println(
                    "Total:\t"
                            + unicodeMap.size()
                            + "\t"
                            + toAbbreviated(SHOW_LIMIT, unicodeMap.keySet()));
            return unicodeMap;
        }
    }
    ...
    static String toAbbreviated(int limit, UnicodeSet input) {
        ...
        if (ranges * 3 < limit) {
            ...
            for (final UnicodeSetIterator it = new UnicodeSetIterator(input); it.nextRange(); ) {
                count +=
                        it.codepoint == it.codepointEnd
                                ? 1
                                : it.codepoint + 1 == it.codepointEnd ? 2 : 3;
                if (count > limit) {
                    break;
                }
    ...
    public static void main(String[] args) {
        ...
        collators.addAll(
                Arrays.asList(
                        Collator.getKeywordValuesForLocale("collation", ULocale.CHINESE, false)));
    ...
    private static void addNewCollator(EncodingInfo info, NamedHanSet e) {
        try {
            final BufferedReader in =
                    FileUtilities.openFile(FindHanSizes.class, e + "_repertoire.txt");
    ...
    private static UnicodeSet getTailoredHan(String type) {
        ...
        final UnicodeSet tailored =
                new UnicodeSet(collator.getTailoredSet()).retainAll(HAN).removeAll(NONCANONICAL);
        return tailored;
    }
