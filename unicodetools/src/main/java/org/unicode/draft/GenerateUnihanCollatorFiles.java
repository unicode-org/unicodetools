...
public class GenerateUnihanCollatorFiles {
    ...
    static final Pattern START_AUTOGEN =
            Pattern.compile(".*#\\s*START\\s*AUTOGENERATED\\s*([^(]*).*");
    ...
    private static void composeFile(String inputDirectory, String fileName, boolean fixChoice)
            throws IOException {
        final Matcher start_autogen = START_AUTOGEN.matcher("");
        final Matcher end_autogen = END_AUTOGEN.matcher("");
        int count = 0;
        try (final PrintWriter newFile =
                        FileUtilities.openUTF8Writer(OUTPUT_DIRECTORY_REPLACE, fileName);
                final BufferedReader oldFile =
                        FileUtilities.openUTF8Reader(inputDirectory, fileName)) {
            while (true) {
                ...
                if (matchingLine == null) {
                    if (count == 0) {
                        throw new IllegalArgumentException(
                                "No START comments for autogeneration: " + matchingLine);
                    }
                    break;
                }
                ++count;
                final String choice = start_autogen.group(1).trim();
                final String replacementFile =
                        fixChoice
                                ? choice.toLowerCase(Locale.ENGLISH)
                                                .replaceAll("\\s+", "_")
                                                .replace("_long", "")
                                                .replace("stroke", "strokeT")
                                        + ".txt"
                                : choice;

                // copy the file to be inserted
                try (final BufferedReader insertFile =
                        FileUtilities.openUTF8Reader(
                                GenerateUnihanCollatorFiles.OUTPUT_DIRECTORY, replacementFile)) {
                ...
                if (matchingLine == null || !choice.equals(end_autogen.group(1).trim())) {
                    throw new IllegalArgumentException(
                            "Mismatched comments for autogeneration: "
                                    + choice
                                    + ", "
                                    + matchingLine);
                }
