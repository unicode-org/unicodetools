...
public class GenerateCharacterFrequencyCharts {
    ...
    private void run() throws IOException {
        ...
        final TablePrinter indexTable =
                new TablePrinter()
                        .addColumn("Char Count")
                        .setCellAttributes("class='count'")
                        .setSortAscending(false)
                        .setSortPriority(0)
                        .setCellPattern("{0,number,#,##0}")
                        .addColumn("%/Total")
                        .setCellAttributes("class='pc'")
                        .setCellPattern("{0,number,#,##0.00%}")
                        .addColumn("Locale")
                        .setCellAttributes("class='locale'")
                        .setCellPattern("<a href=''{0}.html''>{0}</a>")
                        .addColumn("Name")
                        .addColumn("Lit. Pop")
                        .setCellAttributes("class='count'")
                        .setCellPattern("{0,number,#,##0}")
        ...
        final TablePrinter indexTable2 =
                new TablePrinter()
                        .addColumn("Char Count")
                        .setCellAttributes("class='count'")
                        .setSortAscending(false)
                        ...
                        .addColumn("%/Total")
                        .setCellAttributes("class='pc'")
                        .setCellPattern("{0,number,#,##0.0000%}")
                        .addColumn("Locale")
                        .setCellAttributes("class='locale'")
                        .setCellPattern("<a href=''{0}.html''>{0}</a>")
                        .addColumn("Name")
        ...
        final TablePrinter summaryTable =
                new TablePrinter()
                        .addColumn("Locale")
                        .setCellAttributes("class='locale'")
                        .addColumn("Name")
        ...
        for (final String language : CharacterFrequency.getLanguagesWithCounter()) {
            ...
            final String cldrLanguage = ExemplarInfo.getCldrLanguage(language);
            final Counter<Integer> counter =
                    CharacterFrequency.getCodePointCounter(language, false);
            ...
            final double pop =
                    language.equals("mul") ? worldPop : getPopulation(supplemental, cldrLanguage);
            ...
            final DataOutputStream dataOutputStream =
                    new DataOutputStream(
                            new FileOutputStream(
                                    Settings.Output.GEN_DIR + "/frequency/" + language + ".txt"));
            final CompressedDataOutput out = new CompressedDataOutput().set(dataOutputStream);
            final PrintWriter html =
                    FileUtilities.openUTF8Writer(
                            Settings.Output.GEN_DIR + "/frequency-html", htmlFilename);
            html.println(
                    "<html><head><meta charset='UTF-8'>\n"
                            ...
                            + htmlTitle
                            + "</h1>");
            final TablePrinter table =
                    new TablePrinter()
                            .addColumn("Rank")
                            .setCellAttributes("class='rank'")
                            .addColumn("Count")
                            .setCellAttributes("class='count'")
                            .addColumn("%")
                            .setCellAttributes("class='pc'")
                            ...
                            .setCellPattern("·{0}·")
                            .addColumn("Level")
                            .setCellAttributes("class='chars'")
                            .addColumn("Chars")
                            .setCellAttributes("class='name'")
                            .addColumn("Hex")
                            .setCellAttributes("class='hex'")
                            .addColumn("Name")
                            .setCellAttributes("class='name'");
            ...
            for (final int sequence : counter.getKeysetSortedByCount(false)) {
                ...
                String name =
                        SKIP_NAME.containsAll(sequence2) ? "" : UCharacter.getName(sequence2, "+");
                if (name == null) {
                    name = "none";
                }
    ...
    public double getPopulation(SupplementalDataInfo supplemental, final String cldrLanguage) {
        PopulationData popInfo = supplemental.getLanguagePopulationData(cldrLanguage);
        if (popInfo == null) {
            String defaultScript = supplemental.getDefaultScript(cldrLanguage);
            if (defaultScript != null) {
                popInfo =
                        supplemental.getLanguagePopulationData(cldrLanguage + "_" + defaultScript);
            }
            if (popInfo == null) {
    ...
    private void printIndex(TablePrinter indexTable, String file) throws IOException {
        final PrintWriter index =
                FileUtilities.openUTF8Writer(Settings.Output.GEN_DIR + "/frequency-html", file);
        index.println(
                "<html><head><meta charset='UTF-8'>\n"
                        + "<link rel='stylesheet' type='text/css' href='index.css' media='screen'/>\n"
                        + "</head><body>\n");
    ...
    private static class CharacterSamples {
        ...
        CharacterSamples(String language) {
            exemplarsLeft =
                    new UnicodeSet(ExemplarInfo.make(language, null).getExemplars())
                            .removeAll(ExemplarInfo.IGNORE);
            isKey = language.equals("mul");
            pp.setOrdering(Collator.getInstance(new ULocale(language)));
            final RuleBasedCollator spaceComp =
                    (RuleBasedCollator) Collator.getInstance(new ULocale(language));
