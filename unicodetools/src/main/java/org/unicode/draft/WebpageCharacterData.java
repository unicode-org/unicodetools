...
/**
 * Tool to process raw character data, extracting a subset for faster processing
by other tools.
...
pre-HTML code point count, post-HTML code point count, document count, UTF-8
document count
...
public class WebpageCharacterData {
  ...
  private static final UnicodeSet DEBUG_SET =
      new UnicodeSet(0x0020, 0x0020).freeze();
  private static final String SOURCE_DATA =
      "Apr.11.2015.tsv"; // "unicode-count75.txt"; //
                         // "unicode-count-2012-July-21.txt";

  enum Columns {
    // 000009    ht    954857442    0    0    0    953577889    0    0    0
    // 11182029595621    0    0    0    804363    56255    139    22
    // http://www.palmbeachschools.org/    71269    55048    139    22
    // http://www.palmbeachschools.org/jobs/    50871    54366    139    22
    // http://rtghaiti.com/
    codePoint,
    language,
    preHtmlCount1,
    ...
    utf8DocumentCount3;
    static String[] parts;
    public static void set(String line) { parts = line.split("\t"); }
    public String get() { return parts[ordinal()]; }
    @Override
  ...
  static public void doData() throws IOException {
    ...
    while (true) {
      ...
      final boolean debugCodepoint =
          DEBUG_SET != null && DEBUG_SET.contains(codePoint);
      if (debugCodepoint) {
    ...
    writeData(lang2charsPageRank,
              Settings.Output.GEN_DIR + "frequency/languages-rank");
  }

  public static void writeData(Map<String, Counter<Integer>> map,
    ...
    for (final Entry<String, Counter<Integer>> entry : map.entrySet()) {
      ...
      final PrintWriter out =
          FileUtilities.openUTF8Writer(directory, lang + ".txt");
      long totalCount = 0;
      long totalChars = 0;
      for (final Integer cp : counter.getKeysetSortedByCount(false)) {
        ...
        totalCount += count;
        totalChars += 1;
        out.println(com.ibm.icu.impl.Utility.hex(cp) + " ; " +
    ...
    for (final String lang : totalLang.getKeysetSortedByCount(false)) {
      System.out.println(lang + "\t" + totalLang.get(lang) + "\t" +
                         totalLangChars.get(lang));
    }
  }

  public static void addToCounter(Map<String, Counter<Integer>> map,
                                  String lang, int codePoint, long post) {
    if (post == 0) {
      return;
    }
