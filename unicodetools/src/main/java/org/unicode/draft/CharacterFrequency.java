 ...
 * Note that the data is post-html conversion, and whitespace is thus not
 * represented. For a program that uses this, see ScriptPopulation
 */
public class CharacterFrequency {
  public static final String DATA_DIR =
      Settings.Output.GEN_DIR + "frequency/languages/";
  ...
  //    private static Map<String,Double> languageToPopulation = new
  //    HashMap<String,Double>(); private static Map<String,String>
  //    languagesFound = new TreeMap<String,String>();
  ////    private static Map<String,String> languageNameToTag = new
  ///HashMap<String,String>();
  //    private static Map<String, Counter<String>> languageToCharsCounter = new
  //    TreeMap<String, Counter<String>>();
  ...
  //
  //        Transform<String,ULocale> transform = new
  //        Transform<String,ULocale>() {
  ...
  ///String languageTag = locale.toLanguageTag(); / languageNameToTag.put(name,
  ///languageTag); /        } /        languageNameToTag.put("CHINESET",
  ///"zh-Hant"); /        languageNameToTag.put("DHIVEHI", "dv"); /
  ///languageNameToTag.put("FRISIAN", "fy"); /
  ///languageNameToTag.put("HAITIAN_CREOLE", "ht"); /
  ///languageNameToTag.put("KYRGYZ", "ky"); / languageNameToTag.put("LAOTHIAN",
  ///"lo"); /        languageNameToTag.put("SCOTS_GAELIC", "gd"); /
  ///languageNameToTag.put("SINHALESE", "si"); /
  ///languageNameToTag.put("TAGALOG", "fil"); / languageNameToTag.put("BIHARI",
  ///"bho"); /        languageNameToTag.put("UNKNOWN", "und");
  //
  //        Map<String, Counter<String>> rawLanguageToSequencesCounter = new
  //        TreeMap<String, Counter<String>>();
  //        //Map<String, Counter<String>> rawLanguageToCharsCounter = new
  //        TreeMap<String, Counter<String>>();
  //
  //        //      System.out.println("loading stats.characters.txt");
  //        //      SemiFileReader handler = new
  //        SequenceHandler(rawLanguageToCharsCounter).process(Utility.DATA_DIRECTORY
  //        + "/frequency/", "stats.short_sequences.txt");
  ...
  //                rawLanguageToSequencesCounter.put(language,
  //                combinedCounter);
  //            }
  //            //        if (!rawLanguageToCharsCounter.containsKey(language))
  //            {
  //            //          rawLanguageToCharsCounter.put(language,
  //            combinedCounter);
  //            //        }
  //
  //            // get the counters
  //            Counter<String> sequenceCounter =
  //            rawLanguageToSequencesCounter.get(language);
  //            //Counter<String> charCounter =
  //            rawLanguageToCharsCounter.get(language);
  //
  //
  ...
  //            //              System.out.println(language + "\tsequence:\t" +
  //            sequenceCount + "\tchar:\t" + charCount);
  //            //            }
  ...
  //
  //            for (String sequence : sequenceCounter.keySet()) {
  //                addNormalizedCount(sequence, sequenceCounter.get(sequence),
  //                locale, combinedCounter);
  //            }
  //            //        for (String sequence : charCounter.keySet()) {
  //            //          addNormalizedCount(sequence,
  //            charCounter.get(sequence), locale, combinedCounter);
  //            //        }
  //
  //
  //            // at this point, the chars contain all the NFC'd characters,
  //            and the sequences contain all the sequences
  ...
  //    private static void addNormalizedCount(String sequence, long countValue,
  //    ULocale locale, Counter<String> combinedCounter) {
  //        String nfcSequence = ExemplarInfo.specialNormalize(sequence,
  //        locale); int cp; for (int i = 0; i < nfcSequence.length();
  //        i+=Character.charCount(cp)) {
  ...
  //        //    String result =
  //        CharacterFrequency.languageNameToTag.get(string); if (result ==
  //        null) {
  ...
  //            SemiFileReader handler = new
  //            SequenceHandler(result).process(DATA_DIR, "mul.txt");
  //
  //        }
  //    }

  public static Counter<Integer> getCodePointCounter(String language,
                                                     boolean ranked) {
    Counter<Integer> result = languageToCodePointCounter.get(language);
    if (result == null) {
      result = new Counter<Integer>();
      final SemiFileReader handler = new SequenceHandler(result).process(
          ranked ? DATA_DIR_RANK : DATA_DIR, language + ".txt");
  ...
  static class SequenceHandler extends FileUtilities.SemiFileReader {
    ...
    public SequenceHandler(Counter<Integer> counter) { this.counter = counter; }
    @Override
    public boolean handleLine(int start, int end, String[] items) {
      ...
      if (DEBUG && ((++lineCounter % 1000) == 0 || lineCounter < 100)) {
        System.out.println(lineCounter + "\t" + Arrays.asList(items) + "\t" +
  ...
  public static Set<String> getLanguagesWithCounter() { return LANGUAGES; }
}
