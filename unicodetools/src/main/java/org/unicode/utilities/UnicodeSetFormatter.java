...
public class UnicodeSetFormatter {
    ...
    private static final UnicodeSet SORT_AT_END =
            (UnicodeSet) new UnicodeSet("[[:Cn:][:Cs:][:Co:][:Ideographic:]]").freeze();
    ...
    public UnicodeSetFormatter() {}

    public StringTransform getQuoter() {
        return quoter;
    }

    public UnicodeSetFormatter setQuoter(StringTransform quoter) {
        this.quoter = quoter;
     ...
     * @return
     */
    public UnicodeSetFormatter setCompressRanges(boolean compressRanges) {
     ...
     * @return
     */
    public UnicodeSetFormatter setOrdering(Comparator ordering) {
        this.ordering =
                ordering == null
                        ? CODEPOINT_ORDER
     ...
     * @return this, for chaining
     */
    public UnicodeSetFormatter setSpaceComparator(Comparator spaceComp) {
     ...
     * @param toQuote
     */
    public UnicodeSetFormatter setToQuote(UnicodeSet toQuote) {
        if (toQuote != null) {
     ...
     * @param toQuote
     */
    public UnicodeSetFormatter setRawToQuote(UnicodeSet toQuote) {
        if (toQuote == null) {
            toQuote = UnicodeSet.EMPTY;
        }
        if (!toQuote.isFrozen()) {
            toQuote = toQuote.cloneAsThawed().freeze();
        }
        this.toQuote = toQuote;
        return this;
    }
    ...
    private UnicodeSetFormatter appendUnicodeSetItem(String s) {
    ...
    private void appendQuoted(String s) {
        target.append("{");
        if (quoter != null && toQuote.containsSome(s)) {
