...
public class FindProps {
    ...
    public static class UnicodeRelation<T> {
        ...
        public String toString(Transform<String,String> getName) {
            ...
            for (Entry<String, Set<T>> unicodeData : data.entrySet()) {
                ...
                result.append("U+" + Utility.hex(s, 4, " U+") 
    ...
    static Transform<String,String> GET_NAME = new Transform<String,String>() {
        public String transform(String s) {
            ...
            if (cc.length() > 0) {
                result.append("\t[\t").append(ULocale.getDisplayCountry("und_" + cc, ULocale.ENGLISH) + "\t]");
            }
            return result.toString();
        }
    };

    enum Source {dcm, kddi, sb, apple, sv, poss}
    ...
    public static void main(String[] args) {
        ...
        final UnicodeRelation<Source> emoji = new UnicodeRelation<Source>(new SetMaker<Source>() {
        ...
        UnicodeSet apples = new UnicodeSet("[🌍🌎🌐🌒🌖-🌘🌚🌜-🌞🌲🌳🍋🍐🍼🏇🏉🏤 🐀-🐋🐏🐐🐓🐕🐖🐪👥👬👭💭💶💷📬📭📯📵🔀-🔂 🔄-🔇🔉🔕🔬🔭🕜-🕧😀😇😈😎😐😑😕😗😙😛😟 😦😧😬😮😯😴😶🚁🚂🚆🚈🚊🚍🚎🚐🚔🚖🚘🚛-🚡 🚣🚦🚮-🚱🚳-🚵🚷🚸🚿🛁-🛅]");
        ...
        for (String s : ULocale.getISOCountries()) {
            ...
            other.add(new StringBuilder().appendCodePoint(first).appendCodePoint(second).toString());
        }
