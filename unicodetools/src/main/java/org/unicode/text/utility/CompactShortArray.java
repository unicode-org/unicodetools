 ...
 * ******************************************************************************
 *
 * <p>$Source: /home/cvsroot/unicodetools/org/unicode/text/utility/CompactShortArray.java,v $
 *
 * <p>******************************************************************************
 */
 ...
 * indicies of Unicode characters to the value array.
 *
...
public final class CompactShortArray implements Serializable {
    ...
    /** Default constructor for CompactShortArray, the default value of the compact array is 0. */
     ...
     */
    public CompactShortArray(short indexArray[], short newValues[])
        ...
        for (i = 0; i < INDEXCOUNT; ++i) {
            final short index = indexArray[i];
    ...
    public void compact() {
        if (isCompact == false) {
            ...
            for (iBlock = 1; iBlock < INDEXCOUNT; ++iBlock) {
                ...
                if (DEBUGSMALL) {
                    if (block > DEBUGSMALLLIMIT) {
                        break;
                    }
                }
                firstPosition = FindOverlappingPosition(block, tempIndex, tempIndexCount);

                newCount = firstPosition + BLOCKCOUNT;
                if (newCount > tempIndexCount) {
                    for (iIndex = (short) tempIndexCount; iIndex < newCount; ++iIndex) {
                        tempIndex[iIndex] = (char) (iIndex - firstPosition + block);
    ...
    void writeArrays() {
        int i;
        final int cnt = ((values.length > 0) ? values.length : (values.length + UNICODECOUNT));
        System.out.println("{");
        for (i = 0; i < INDEXCOUNT - 1; i++) {
            System.out.print(
                    "(short)"
                            + ((getIndexArrayValue(i) >= 0)
        ...
        System.out.println(
                "(short)"
                        + ((getIndexArrayValue(INDEXCOUNT - 1) >= 0)
                                ? (int) getIndexArrayValue(i)
                                : (int) (getIndexArrayValue(i) + UNICODECOUNT))
                        + " }");
        System.out.println("{");
    ...
    void printIndex(short start, short count) {
        ...
        for (i = start; i < count; ++i) {
            System.out.println(
                    i + " -> : " + ((indices[i] >= 0) ? indices[i] : indices[i] + UNICODECOUNT));
        }
        System.out.println();
    }
    ...
    private int FindOverlappingPosition(int start, char[] tempIndex, int tempIndexCount) {
        int i;
        short j;
        short currentCount;
        ...
        for (i = 0; i < tempIndexCount; i += BLOCKCOUNT) {
            ...
            for (j = 0; j < currentCount; ++j) {
                if (values[start + j] != values[tempIndex[i + j]]) {
                    break;
                }
            }
            if (j == currentCount) {
