...
public class ComparisonNormalizer {
    ...
    public <T> ComparisonNormalizer(Transform<String, T> transform, Comparator<String> bestFirst) {
        ...
        Relation<T, String> mapping =
                Relation.of(new HashMap<T, Set<String>>(), TreeSet.class, bestFirst);
        for (String cp : chars) {
    ...
    public static class RawCollationKeyTransform implements Transform<String, RawCollationKey> {
        ...
        public RawCollationKey transform(String source) {
            ...
            if (result.size != result.bytes.length) {
                ...
                result.append(
                        bytes, 0, size); // make the capacity = size, to work around hashCode bug.
            }
            return result;
        }
    }

    public static final Comparator<String> CODEPOINT =
    ...
    public static ComparisonNormalizer getSimple() {
        Comparator<String> combiningLast =
                new Comparator<String>() {
                    @Override
                    public int compare(String o1, String o2) {
                        boolean c1 = !o1.isEmpty() && isCombining(o1.codePointAt(0));
                        boolean c2 = !o2.isEmpty() && isCombining(o2.codePointAt(0));
                        return c1 == c2 ? 0 : c1 ? 1 : -1; // is NOT combining is better
                    }

                    private boolean isCombining(int cp) {
                        int t1 = UCharacter.getType(cp);
                        return t1 == UCharacter.NON_SPACING_MARK
                                || t1 == UCharacter.ENCLOSING_MARK
                                || t1 == UCharacter.COMBINING_SPACING_MARK;
                    }
                };

        Comparator<String> isNFKC_CF =
                new Comparator<String>() {
                    @Override
                    public int compare(String o1, String o2) {
                        boolean c1 = nfkc_cf.isNormalized(o1);
                        boolean c2 = nfkc_cf.isNormalized(o2);
                        return c1 == c2 ? 0 : c1 ? -1 : 1; // isNormalized is better
                    }
                };

        Comparator<String> secondarySet =
                new Comparator<String>() {
                    @Override
                    public int compare(String o1, String o2) {
                        boolean c1 = !o1.isEmpty() && secondaryData.containsAll(o1);
                        boolean c2 = !o1.isEmpty() && secondaryData.containsAll(o2);
                        return c1 == c2 ? 0 : c1 ? 1 : -1; // NOT secondary is better
                    }
                };

        Comparator<String> firstIsBetterTarget =
                new MultiComparator<String>(
                        ...
                        CODEPOINT);
        Transform<String, RawCollationKey> transform =
                new RawCollationKeyTransform(UCA_SECONDARY_DECOMPOSING);

        ComparisonNormalizer norm = new ComparisonNormalizer(transform, firstIsBetterTarget);
        return norm;
    }

    //  private static void checkProblem(final RuleBasedCollator uca_raw, Comparator<String>
    // ucaFull) {
    ...
    //    Relation<RawCollationKey,String> mapping = Relation.of(new
    // HashMap<RawCollationKey,Set<String>>(), TreeSet.class, ucaFull);
    //    mapping.put(a, "०");
    //    mapping.put(b, "𐒠");
    ...
    public static void main(String[] args) {
        ...
        Comparator<String> ucaFull =
                new MultiComparator<String>(
                        (Comparator<String>) (Comparator<?>) UCA_SECONDARY_DECOMPOSING, CODEPOINT);
        ...
        for (String target : sorted) {
            ...
            CollationElementIterator it =
                    UCA_SECONDARY_DECOMPOSING.getCollationElementIterator(target);
            key.setLength(0);
            for (int ce = it.next(); ce != CollationElementIterator.NULLORDER; ce = it.next()) {
                if (key.length() != 0) {
                    key.append(' ');
            ...
            System.out.println(
                    "\n#"
                            ...
                            + (target.isEmpty()
                                    ? ""
                                    : UCharacter.getExtendedName(target.codePointAt(0))));
