...
/**
 * Sample Poor-Man's Enum. To use as a template, copy and
 *
 * <ul>
 *   <li>replace all instances of "SampleEnum" by your enum's name
 *   <li>change the enum values to your values
 *   <li>set any aliases (or remove that section)
 * </ul>
 */
public final class SampleEnum extends PoorMansEnum {
    private static PoorMansEnum.EnumStore store = new PoorMansEnum.EnumStore();

    public static final SampleEnum ALPHA = add("The"),
            BETA = add("Quick"),
            GAMMA = add("Brown"),
            FIRST = ALPHA;
    ...
    /* Boilerplate */
    public SampleEnum next() {
        return (SampleEnum) next;
    }
    ...
    public static SampleEnum get(String s) {
        return (SampleEnum) store.get(s);
    }

    public static SampleEnum get(int v) {
        return (SampleEnum) store.get(v);
    }
    ...
    private static SampleEnum add(String name) {
        return (SampleEnum) store.add(new SampleEnum(), name);
    }
    ...
    public static void test() {
        ...
        for (int i = 0; i < SampleEnum.getMax(); ++i) {
            final String n = SampleEnum.get(i).toString();
