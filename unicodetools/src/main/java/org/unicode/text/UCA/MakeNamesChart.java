...
public class MakeNamesChart {
    ...
    static UnicodeSet
            usePicture; // = new UnicodeSet("[[:whitespace:][:defaultignorablecodepoint:]]");
    ...
    public static void main(String[] args) throws Exception {
        ...
        rtl =
                new UnicodeSet(up.getSet("bidiclass=r"))
                        .addAll(up.getSet("bidiclass=al")); // "[[:bidiclass=r:][:bidiclass=al:]]");
        usePicture = new UnicodeSet().addAll(up.getSet("defaultignorablecodepoint=Yes")); // new
        // UnicodeSet("[[:whitespace:][:defaultignorablecodepoint:]]");
        ...
        for (int count = 0; count < limit; ++count) {
            ...
            // PrintWriter out = FileUtilities.openUTF8Writer("C:/DATA/GEN/charts/namelist/",
            // chartPrefix + fileName);
            PrintWriter out =
                    Utility.openPrintWriter(
                            NAMESLIST_DIR, chartPrefix + fileName, Utility.UTF8_WINDOWS);
            ...
            if (collectedCodePoints.size() == 0) {
                ...
                int counter = 0;
                for (final UnicodeSetIterator it = new UnicodeSetIterator(collectedCodePoints);
                        it.next(); ) {
            ...
            out.println("</body>\n</html>");
            out.close();
            // out = FileUtilities.openUTF8Writer("C:/DATA/GEN/charts/namelist/", namePrefix +
            // fileName);
            out =
                    Utility.openPrintWriter(
                            NAMESLIST_DIR, namePrefix + fileName, Utility.UTF8_WINDOWS);
            out.println(
                    "<!DOCTYPE HTML PUBLIC '-//W3C//DTD HTML 4.01 Transitional//EN' 'http://www.w3.org/TR/html4/loose.dtd'>\n"
                ...
                try {
                    if (line.startsWith("@") && !line.startsWith("@+\t*")) {
                        ...
                        if (line.equals("@+")) {
                            ...
                            if ((i + 1 < lines.size())
                                    && ((String) lines.get(i + 1)).startsWith("@+")) {
                                // also skip a following @+ notice line that continues the variation
                                // subheader
                                ++i;
                            }
                        } else if (line.startsWith("+")) {
                            ...
                            out.println(
                                    "<tr><td class='comment' colspan='4'>" + line + "</td></tr>");
                        } else if (line.startsWith("@")) {
                            System.err.println("*** Can't handle line: " + i + "\t" + line);
                        } else {
                            line = line.trim();
                            out.println("<tr><td colspan='4'><h2>" + line + "</h2></td></tr>");
                        }
                    } else {
                        ...
                        if (line.startsWith("\t")) {
                            ...
                            if (false && line.indexOf(body) != 1) {
                                System.out.println(
                                        "Format error: too much inital whitespace: <" + line + ">");
                            }
                            ...
                            switch (firstChar) {
                                ...
                                case '%':
                                    ...
                                    break;
                                case ':':
                                    body = checkCanonical(lastCodePoint, body);
                                    break;
                                case '#':
                                    body = checkCompatibility(lastCodePoint, body);
                                    break;
                                case 'x':
                                    body = getOther(body);
                                    break;
                            ...
                            out.println(
                                    "<tr><td"
                                            ...
                                            + "</a></code></td>"
                                            + "<td class='c'"
                                            + (rtl.contains(lastCodePoint) ? " dir='rtl'" : "")
                                            + ">\u00A0"
                                            + showChar(lastCodePoint, true)
                                            + "\u00A0</td>"
                                            + "<td colSpan='2'"
                                            + (lastCodePointIsNew ? " class='new'" : "")
                                            + ">"
                                            + nameStyle(showTextConvertingHex(lineParts[1], false))
                                            + "</td></tr>");
                    ...
                    throw (IllegalArgumentException)
                            new IllegalArgumentException("Error on line: " + line).initCause(e);
                }
            }
            finishItem(out);
            out.println(
        ...
        final PrintWriter out =
                Utility.openPrintWriter(NAMESLIST_DIR, "mainList.html", Utility.UTF8_WINDOWS);
        FileUtilities.appendFile(WriteCharts.class, "nameslist_chart_header.html", out);
        //        out.println("<html><head><meta http-equiv='Content-Type' content='text/html;
        // charset=utf-8'>" +
        //                "<title>Main List</title><link rel='stylesheet' type='text/css'
        // href='nameslist.css'>" +
        ...
        // System.out.println(bf.showSetDifferences("Has name in decomps", hasName, "Has no name in
        // decomps", hasNoName));
        ...
        //        System.out.println("Characters with names in decomps: " +
        // hasName.toPattern(true));
        //        System.out.println("Characters without names in decomps: " +
        // hasNoName.toPattern(true));
        //        System.out.println("Characters sometimes with, sometimes without names in decomps:
        // " + both.toPattern(true));
    ...
    private static void checkFile() throws IOException {
        final BufferedReader in =
                Utility.openUnicodeFile(
                        "NamesList", Default.ucdVersion(), true, Utility.LATIN1_WINDOWS);
    ...
    private static void finishItem(PrintWriter out) {
        ...
        final String upper =
                showForm(
                        ...
                        null,
                        null,
                        Default.ucd().getCase(str, UCD_Types.FULL, UCD_Types.UPPER),
                        "\u2191");
        showForm(
                out,
                str,
                upper,
                null,
                Default.ucd().getCase(str, UCD_Types.FULL, UCD_Types.TITLE),
                "\u2195");
        final String lower =
                showForm(
                        out,
                        ...
                        null,
                        null,
                        Default.ucd().getCase(str, UCD_Types.FULL, UCD_Types.LOWER),
                        "\u2193");
        showForm(
                out,
                lower,
                null,
                null,
                Default.ucd().getCase(str, UCD_Types.FULL, UCD_Types.FOLD),
                "\u2194");

        final String dc = Default.ucd().getDecompositionMapping(lastCodePoint);
        final String nfd =
                showForm(out, dc, str, null, Default.nfd().normalize(lastCodePoint), "\u21DB");
        // String nfc = showForm(out, dc, null, Default.nfc().normalize(lastCodePoint), "\u21DB");
        final String nfkd =
                showForm(out, dc, str, nfd, Default.nfkd().normalize(lastCodePoint), "\u21DD");
        ...
        //                            + " "
        //                            + Default.ucd().getName(other, UCD.NORMAL, " +
        // ").toLowerCase()
        //                            // maybeNameStyle(showTextConvertingHex(upper, firstChar !=
        // '='), firstChar == '=')
        //                            + "</td></tr>");
        //                }
        //            }
            ...
            out.println(
                    "<tr><td>\u00A0</td><td>\u00A0</td><td class='c'>"
                            ...
                            + (UTF16.countCodePoint(transformed) != 1
                                    ...
                                    : " "
                                            + Default.ucd()
                                                    .getName(transformed, UCD_Types.NORMAL, " + ")
                                                    .toLowerCase())
                            // maybeNameStyle(showTextConvertingHex(upper, firstChar != '='),
                            // firstChar == '=')
                            + "</td></tr>");
    ...
    public static String getHeading(String name) {
    ...
    private static String nameStyle(String string) {
        ...
        String result =
                "<span class='name'>"
                        + Default.ucd().getCase(string, UCD_Types.FULL, UCD_Types.TITLE)
                        + "</span>";
    ...
    public static String checkName(String body, int cp, String name) {
        ...
        if (!name.equalsIgnoreCase(name2)) {
            System.out.println(
                    "Mismatch in name for " + body + " in " + Utility.hex(lastCodePoint));
    ...
    static String showChar(int cp, boolean addRlmIfNeeded) {
        ...
        if (usePicture.contains(cp)) {
            ...
            // return "<img alt='" + hex + "' src='http://www.unicode.org/cgi-bin/refglyph?24-" +
            // hex + "'>";
        }
    ...
    private static String checkCanonical(int codePoint, String body) {
        ...
        if (lastDecompType != UCD_Types.CANONICAL) {
            System.out.println(
                    "Mismatching Decomposition Type: " + body + " in " + Utility.hex(codePoint));
        }
    ...
    public static final String[][] LINE_MATCHER_VARIABLES = {
        {"$char", "[0-9A-F]{4,6}"},
        {
            "$name", "[0-9A-Z](?:[0-9A-Z\\- ]*[0-9A-Z])?"
        }, // alphanumeric, separated by spaces and '-'
        {
            "$lcname", "[0-9a-zA-Z](?:[0-9a-zA-Z \\-]*[0-9a-zA-Z])?"
        }, // lowercase alphanumeric, separated by spaces and '-'
        // NOTE: lcname can contain uppercase characters
        {
            "$comment", "\\([A-Za-z](?:[0-9A-Za-z, \\-]*[0-9A-Za-z])?\\)"
        }, // '(' alphanumeric (upper or lower) separated by spaces ')'
    };
    ...
    enum LineMatcher {
        ...
        NOTICE_LINE2("@\\+\t\\* (.*)"),
        NOTICE_LINE1("@\\+\t(.*)"),
        // NOTE: @+    Italic symbols already encoded in the Letterlike Symbols block are omitted
        // here to avoid duplicate encoding.
        // has TAB SP TAB
        // NOTE: out of order
        //      // Skip '@', output text as notice
