...
class ReorderingTokens {
    ...
    public int cardinality() {
        return reorderingToken.size() + types.size();
    }
    ...
    void addInfoFrom(long primary, String source) {
        ...
        if (!(script == UCD_Types.Unknown_Script || script == UCD_Types.COMMON_SCRIPT)
                && (cat == UCD_Types.OTHER_LETTER
                        || cat == UCD_Types.UPPERCASE_LETTER
                        || cat == UCD_Types.LOWERCASE_LETTER
                        || cat == UCD_Types.TITLECASE_LETTER)) {
    ...
    <T extends Appendable> T appendTo(T result, boolean categoriesAlso) {
        try {
            ...
            if (!categoriesAlso) {
                final String scriptNames =
                        reorderingToken.size() != 0
                                ? CollectionUtilities.join(reorderingToken.keySet(), " ")
                                : "Zyyy";
                result.append(scriptNames);
            } else {
                final String scriptNames =
                        reorderingToken.size() != 0 ? joinCounter(reorderingToken) : "";
                result.append(scriptNames);
                if (types.size() != 0) {
