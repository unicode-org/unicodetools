...
public class IcannMsr {
    ...
    static final SetMaker<String> SM =
            new SetMaker<String>() {
                public Set<String> make() {
                    return new LinkedHashSet<String>();
                }
            };
    static final SetMaker<Identifier_Type> ITM =
            new SetMaker<Identifier_Type>() {
    ...
    static final UnicodeRelation<Identifier_Type> DATA2TYPE =
            new UnicodeRelation<IdentifierInfo.Identifier_Type>(ITM);
    ...
    static {
        ...
        //        try {
        //            BufferedReader f = FileUtilities.openFile(ICANN_DIR,
        // "msr-wle-rules-04dec14-en.xml");
        //            int line = f.read();
        ...
        XMLFileReader.loadPathValues(ICANN_DIR + XML_DATA, data, false);
        for (Pair<String, String> datum : data) {
            String path = datum.getFirst();
            // (//lgr/data/range[@first-cp="20E0E"][@last-cp="20E0F"][@tag="sc:Hani"][@ref="4 ZH
            // IIC"],)
    ...
    private static Identifier_Type getIdentifierType(int cp, String dataLine) {
        try {
            ...
            throw new IllegalArgumentException(
                    Utility.hex(cp) + "\t" + getName(cp) + "\t" + dataLine);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        for (EntryRange x :
                new UnicodeSet("[[:age=6.3:]-[[:nd:][:cn:][:co:][:cs:][:cwcf:]]]").ranges()) {
            for (int i = x.codepoint; i <= x.codepointEnd; ++i) {
                ...
                Identifier_Status icannStatus =
                        icann == null
                                ? Identifier_Status.restricted
                                : icann.iterator().next().identifierStatus;
    ...
    static final Comparator<Pair<Identifier_Type, Identifier_Type>> IDPAIR =
            new Comparator<Pair<Identifier_Type, Identifier_Type>>() {

                @Override
                public int compare(
                        Pair<Identifier_Type, Identifier_Type> o1,
                        Pair<Identifier_Type, Identifier_Type> o2) {
    ...
    private static <T> void showValues(String file, UnicodeRelation<T> remap2, T skipValue) {
        try (PrintWriter out =
                FileUtilities.openUTF8Writer(Settings.Output.GEN_DIR + "icann/", file)) {
    ...
    private static String getGc(int cpStart) {
        return UCharacter.getPropertyValueName(
                ...
                UCharacter.getType(cpStart),
                UProperty.NameChoice.SHORT);
    }

    private static void showValues(
            String file, UnicodeMap<Pair<Identifier_Type, Identifier_Type>> diff) {
        try (PrintWriter out =
                FileUtilities.openUTF8Writer(Settings.Output.GEN_DIR + "icann/", file)) {
