...
public class VerifyUCD {
    ...
    public static void main(String[] args) throws IOException {
        ...
        final String y =
                Default.ucd()
                        .getCase(
                                Default.nfd().normalize("\u0130"), UCD_Types.FULL, UCD_Types.LOWER);
    ...
    private static void addMarks(
            UnicodeSet contents, UnicodeSet marks, int codepoint, Normalizer normalizer) {
    ...
    public static void doEquivalenceOfProperties(Equivalence equivalence) {
        ...
        final ToolUnicodePropertySource ups = ToolUnicodePropertySource.make(Default.ucdVersion());
        final UnicodeSet setToTest =
                ups.getSet(
                        equivalence == Equivalence.CANONICAL
                                ? "NFDQuickCheck=No"
                                : "NFKDQuickCheck=No");
        ...
        final Set<String> availablePropertyNames =
                new TreeSet<String>(
                        ups.getAvailableNames(
                                UnicodeProperty.BINARY_MASK
        ...
        final UnicodeSet forceNFC =
                new UnicodeSet()
                        ...
                        .addAll("\u1B3B\u1B3D\u1B43\u0CC0\u0CC7\u0CC8\u0CCA\u0CCB");
        final Set<String> singleCharOnly =
                new TreeSet<String>(
                        Arrays.asList(
                                new String[] {
                                    "ASCII_Hex_Digit",
        ...
        // ups.getAvailableNames(UnicodeProperty.STRING_OR_MISC_MASK));
        ...
        Log.getLog()
                .println("<tr><td><b>Testing:</b></td><td>" + availablePropertyNames + "</td><tr>");
        ...
        for (final UnicodeSetIterator it = new UnicodeSetIterator(setToTest); it.next(); ) {
            ...
            for (final String propertyName : availablePropertyNames) {
                if (UTF16.hasMoreCodePointsThan(normalized, 1)
                        && singleCharOnly.contains(propertyName)) {
                    continue;
                }
                final UnicodeProperty up = ups.getProperty(propertyName);
                final boolean isStringProp =
                        ((1 << up.getType()) & UnicodeProperty.STRING_OR_MISC_MASK) != 0;
                ...
                if (propertyName.contains("case_Mapping")
                        || propertyName.contains("Case_Folding")) {
    ...
    private static void showProperties(
            UnicodeMap results,
            Map<String, UnicodeMap> sidewaysResults,
            final Equivalence equivalence) {
        final UnicodeLabel nameLabel =
                new UnicodeLabel() {
                    @Override
                    public String getValue(int codepoint, boolean isShort) {
                        ...
                        return Default.ucd()
                                        .getCodeAndName(
                                                codepoint,
                                                UCD_Types.NORMAL,
                                                TransliteratorUtilities.toHTMLControl)
                                + "\t\u2192\t"
                                + Default.ucd()
                                        .getCodeAndName(
                                                nfd,
    ...
    private static String caseMapping(String source, String propertyName) {
        final byte operation =
                propertyName.contains("Uppercase")
                        ? UCD_Types.UPPER
                        : propertyName.contains("Lowercase")
                                ? UCD_Types.LOWER
                                : propertyName.contains("Titlecase")
                                        ? UCD_Types.TITLE
                                        : UCD_Types.FOLD;
    ...
    private static void addPropertyDifference(
            Map<String, UnicodeMap> sidewaysResults,
    ...
    static boolean equals(Object value1, Object value2, Equivalence equivalence) {
        ...
        if (equivalence == Equivalence.CANONICAL) {
            return Default.nfd()
                    .normalize(value1.toString())
                    .equals(Default.nfd().normalize(value2.toString()));
        } else if (equivalence == Equivalence.COMPATIBLITY) {
            return Default.nfkd()
                    .normalize(value1.toString())
                    .equals(Default.nfkd().normalize(value2.toString()));
        }
    ...
    public static void checkSegmentation() {
