...
public class ShowCharacters {
    ...
    static UnicodeMap<Age_Values> ages =
    ...
    public static void main(String[] args) {
        ...
        show(
                "Gendered",
                new UnicodeSet(
                        "[👲 👳 💂 🎅 👯 👰 🕴 ☃ ⛄ \\U0001F57A \\U0001F934 \\U0001F936 \\U0001F935]"));
        ...
        UnicodeSet modsWithVS =
                new UnicodeSet(mods).retainAll(EmojiData.EMOJI_DATA.getEmojiWithVariants());
        UnicodeSet modsWithVSAndDefaultEmoji =
                new UnicodeSet(modsWithVS)
                        .removeAll(EmojiData.EMOJI_DATA.getEmojiPresentationSet());
        show("basesWithVSAndDefaultEmoji", modsWithVSAndDefaultEmoji);

        UnicodeSet modsWithVSAndDefaultText =
                new UnicodeSet(modsWithVS).retainAll(EmojiData.EMOJI_DATA.getTextPresentationSet());
        show("basesWithVSAndDefaultText", modsWithVSAndDefaultText);
    }

    private static void show(String prop, UnicodeSet unicodeSet) {
        System.out.println(
                "# All omitted code points have "
                        + prop
                        + "=No\n"
                        + "# @missing: 0000..10FFFF  ; "
                        + prop
    ...
    public void test(String[] args) {
        ...
        for (final String okScript :
                Arrays.asList(
                        "Zyyy", "Armn", "Copt", "Dest", "Glag", "Cyrl", "Grek", "Geor", "Latn")) {
        ...
        for (final String s : source) {
            ...
            if (okScripts != null
                    && !okScripts.containsSome(s)
                    && !okScripts.containsSome(nfkdForm)) {
                continue;
            }
            ...
            final R5<String, String, String, String, String> row =
                    Row.of(scriptValue, dscriptValue, casing, dcasing, gc.getValue(ch, true));
        ...
        final PrintWriter log =
                Utility.openPrintWriter(
                        Settings.Output.GEN_DIR, "showChars.html", Utility.UTF8_WINDOWS);
        ...
        log.println(
                "<tr>"
                        ...
                        + "</tr>");
        int count = 0;
        for (final Entry<R5<String, String, String, String, String>, UnicodeSet> entry :
                data.entrySet()) {
            final R5<String, String, String, String, String> row = entry.getKey();
            for (final UnicodeSetIterator it = new UnicodeSetIterator(entry.getValue());
                    it.nextRange(); ) {
    ...
    private static String alphabeticStatus(
            String string,
