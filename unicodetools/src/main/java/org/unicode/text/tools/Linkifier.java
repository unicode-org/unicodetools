...
public class Linkifier {
    ...
    private class UrlScanner extends Scanner {
        ...
        Scanner schemeScanner =
                new SimpleScanner(new UnicodeSet("[-+.a-zA-Z0-9]"), UnicodeSet.EMPTY);
        ...
        Scanner queryScanner =
                new SimpleScanner(new UnicodeSet(NORMAL).add(QUERY_START), INCLUSIONS);
        Scanner fragmentScanner =
                new SimpleScanner(new UnicodeSet(NORMAL).add(FRAGMENT_START), INCLUSIONS);
        ...
        public ScanStatus scan(ScanData input) {
            ...
            if (input.source.regionMatches(
                    input.lastValidLimit, SCHEME_END, 0, SCHEME_END.length())) {
        ...
        for (String test : tests) {
            ...
            if (!found) {
                ...
                for (String name : GROUPS) {
                    ...
                    if (group != null) {
                        System.out.println(
                                "\t" + name + " " + group + "\t" + linkifier.check(name, group));
                    }
                }
            }
        }
