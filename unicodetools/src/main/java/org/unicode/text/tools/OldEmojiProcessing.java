...
public class OldEmojiProcessing {
    ...
    static {
        ...
        for (int i = 1; i <= 9; ++i) {
            addAndroidRemap(
                    (char) ('0' + i) + "" + Emoji.ENCLOSING_KEYCAP, 0xFE82D + i); // 1 => U+FE82E
        }
    ...
    public static Integer addAndroidRemap(String real, int replacement) {
        ...
        return ANDROID_REMAP.put(
                Row.of(first, real.codePointAt(Character.charCount(first))), replacement);
    }
    ...
    public static String androidPng(int firstCodepoint, int secondCodepoint, boolean first) {
        ...
        String filename =
                "android/emoji_u"
                        + Utility.hex(first ? firstCodepoint : secondCodepoint)
