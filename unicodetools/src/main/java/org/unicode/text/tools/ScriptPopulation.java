...
class ScriptPopulation {
    ...
    private static final UnicodeSet SINGLETONS_WITHOUT_DEFECTIVES =
            new UnicodeSet()
    ...
    public static void main(String[] args) {
        ...
        for (int cp = 0; cp <= 0x10FFFF; ++cp) {
            ...
            (isNoto ? notoCategoryToTopItems[scriptNum] : nonotoCategoryToTopItems[scriptNum])
        ...
        int count = 0;
        System.out.println(
                "№\t-log(%)\tCount\tScript (*Cat)\tUAX31 Status\t1st\t2nd\t3rd\t4th\t5th\t6th\t7th\t8th\t9th\t10th\t11th\t12th\t13th\t14th\t15th\t16th\t17th\t18th\t19th\t20th\t…");
        ...
        System.out.println("\nNOTO\n");
        System.out.println(
                "№\t-log(%)\tCount\tNoto?\tScript (*Cat)\tUAX31 Status\t1st\t2nd\t3rd\t4th\t5th\t6th\t7th\t8th\t9th\t10th\t11th\t12th\t13th\t14th\t15th\t16th\t17th\t18th\t19th\t20th\t…");

        count = 0;
        for (Pair<Boolean, Integer> entry :
                notoScriptFrequency.getKeysetSortedByCount(false, null)) {
            ...
            System.out.print(
                    ++count
                            ...
                            + Category.getUsageName(category));
            int max = 20;
            Counter2<Integer> topItems =
                    inNoto ? notoCategoryToTopItems[category] : nonotoCategoryToTopItems[category];
            showTop(max, topItems, frequ);
            System.out.println();
        }
        ...
        //    Relation<String, String> languagesWithoutScripts = Relation.of(new
        // TreeMap<String,Set<String>>(), TreeSet.class);
        ...
        //        PopulationData pop = supplemental.getLanguageAndTerritoryPopulationData(language,
        // territory);
        ...
        //        charCount = 1;
        //      }
        //      System.out.println(Category.getName(i) + "\t" + usage + "\t" + count + "\t" +
        // charCount);
        //    }
        ...
        //      //System.out.println(scriptName + "\t" + scriptNames + "\t" + Utility.hex(i) + "\t"
    ...
    //            //System.out.println(lang + "\t=>\t" + max + "\t" + scriptCode + "\t" +
    // UScript.getName(scriptCode));
    ...
    //                if (extra != Extra.Punctuation && extra != Extra.Letter && extra !=
    // Extra.Mark) continue;
    ...
    //        }
    //        // normalize
    //        Map<Integer, Counter2<Integer>> normcpToScriptFrequency = new TreeMap<Integer,
    // Counter2<Integer>>();
    ...
    //                System.out.print(extra + "\tU+" + Utility.hex(cp) + "\t" +
    // UCharacter.getName(cp));
    //                int max = 5;
    //                Counter<Integer> scriptFrequency = entry.getValue();
    //                double total = scriptFrequency.getTotal();
    //                for (R2<Long, Integer> x : scriptFrequency.getEntrySetSortedByCount(false,
    // null)) {
    //                    if (--max < 0) break;
    //                    int script = x.get1();
    //                    long count = x.get0();
    ...
    ////            System.out.println(UScript.getName(value) + "\t" +
    // remapped.getSet(value).toPattern(false));
    ////        }
    //
    ...
    private static void showTop(int max, Counter2<Integer> topItems, Double frequ) {
        ...
        for (Integer codePoint : sorted) {
            ...
            System.out.print((SHOW_FREQ ? "\t" + cFreq : "") + "\t" + str);
        }
        ;
        if (max < 0) {
            System.out.print("\t…");
    ...
    static final UnicodeSet SHOULD_BE_SYMBOL =
            new UnicodeSet("[@ * \\& # % ‰ ‱ † ‡ ※]").freeze(); // PRI 228
    ...
    static final UnicodeSet MAKE_FORMAT_FOR_CHART =
            new UnicodeSet(
                            "["
                                    + "[:variationselector:]"
                                    + "[\u034F]" // grapheme joiner
                                    + "]")
                    .freeze();
    static final UnicodeSet MAKE_SYMBOL_FOR_CHART =
            new UnicodeSet(
                            "["
                                    + "[:mark:]"
                                    ...
                                    + "]")
                    .freeze();
    static final UnicodeSet latinMark =
            new UnicodeSet("[[:scx=common:][:scx=inherited:]&[:mark:]]")
                    ...
                    .freeze();
    static final UnicodeSet latinLetter =
            new UnicodeSet("[[:scx=common:][:scx=inherited:]&[:letter:]]")
                    .removeAll(MAKE_SYMBOL_FOR_CHART)
                    .removeAll(MAKE_FORMAT_FOR_CHART)
                    .removeAll(SHOULD_BE_GREEK)
                    .removeAll(SHOULD_BE_COPTIC)
                    .removeAll(SHOULD_BE_DEVA);
    static final UnicodeSet SHOULD_BE_LATIN =
            new UnicodeSet(latinMark).addAll(latinLetter).freeze();
    static final UnicodeSet SHOULD_BE_HAN =
            new UnicodeSet(
                            "["
    ...
    static class Category {
        ...
        private static String getUsageName(Integer category) {
            ...
            return usage == ScriptUsage.UNKNOWN
                    ? "N/A"
                    : UCharacter.toTitleCase(usage.toString(), null);
        }
        ...
        private static boolean isVariant(int category) {
            ...
            return script == UScript.SIMPLIFIED_HAN
        ...
        public static int getCategory(int cp) {
            ...
            int defaultIgnorable =
                    UCharacter.getIntPropertyValue(cp, UProperty.DEFAULT_IGNORABLE_CODE_POINT);
    ...
    //        lastS = s;
    //      }
    //      for (UnicodeSetIterator it = new UnicodeSetIterator(fixedScripts.getSet(s));
    // it.nextRange();) {
    ...
    // UCharacter.getName(it.codepointEnd));
    //        } else {
    ...
    private static Extra getGeneralCategory(int cp) {
        ...
        switch (category) {
            ...
            case UCharacter.COMBINING_SPACING_MARK:
                return UCharacter.getIntPropertyValue(cp, UProperty.VARIATION_SELECTOR) != 0
                        ? Extra.Format
                        : Extra.Mark;
