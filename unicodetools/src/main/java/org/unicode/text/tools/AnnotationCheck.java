...
public class AnnotationCheck {
    static final Comparator<String> SIZE =
    ...
    public static void main(String[] args) {
        Relation<String, String> containerToContains =
                Relation.of(new TreeMap(SIZE), TreeSet.class);
        for (Entry<String, UnicodeSet> entry1 :
                EmojiAnnotations.ANNOTATIONS_TO_CHARS.getStringUnicodeSetEntries()) {
            final String key1 = entry1.getKey();
            final UnicodeSet value1 = entry1.getValue();
            if (value1.size() < 10) {
                continue;
            }
            for (Entry<String, UnicodeSet> entry2 :
                    EmojiAnnotations.ANNOTATIONS_TO_CHARS.getStringUnicodeSetEntries()) {
