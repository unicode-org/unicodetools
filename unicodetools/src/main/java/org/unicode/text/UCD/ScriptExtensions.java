...
public class ScriptExtensions {
    ...
    public static String getScriptID_fromIndex(int prop, int length) {
        return prop < 0 || prop >= UCD_Names.SCRIPT.length
                ? null
                : (length == UCD_Types.EXTRA_ALIAS && prop == UCD_Types.INHERITED_SCRIPT)
                        ? "Qaai"
                        : (length == UCD_Types.SHORT)
    ...
    public static ScriptExtensions make(String directory, String filename) {
        ...
        result.scriptSpecials =
                ((ScriptExtensions.MyHandler) new MyHandler().process(directory, filename))
                        .map.freeze();
        return result;
    }
    ...
    public static ScriptExtensions make(Class aClass, String filename) {
        final ScriptExtensions result = new ScriptExtensions();
        result.scriptSpecials =
                ((ScriptExtensions.MyHandler) new MyHandler().process(aClass, filename))
                        .map.freeze();
        return result;
    }
