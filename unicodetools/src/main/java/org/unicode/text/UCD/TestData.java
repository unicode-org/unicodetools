 ...
 * ******************************************************************************
 *
 * <p>$Source: /home/cvsroot/unicodetools/org/unicode/text/UCD/TestData.java,v $
 *
 * <p>******************************************************************************
 */
...
public class TestData implements UCD_Types {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (final Integer scriptCode : scriptCount.keySet()) {
            System.out.println(
                    UCD.getScriptID_fromIndex((byte) (int) scriptCode, UCD_Types.SHORT)
                            + "\t"
                            + scriptCount.get(scriptCode));
        }
        results.freeze();
        final BagFormatter bf =
                new BagFormatter(ToolUnicodePropertySource.make(Default.ucdVersion()));

        for (final Pair<String, String> value :
                results.values(new TreeSet<Pair<String, String>>())) {
        ...
        final UnicodeSet rr =
                new UnicodeSet(
                        "[[\u07C0-\u08FF \uFB1D-\uFB4F \\U00010840-\\U000109FF \\U00010A60-\\U00010FFF]-[\uFDD0-\uFDEF]]");
        ...
        Utility.showSetDifferences("Sentence_Terminal", sterm, "Empty", empty, true, Default.ucd());

        Utility.showSetDifferences(
                "Sentence_Terminal", sterm, "Terminal_Punctuation", term, true, Default.ucd());

        Utility.showSetDifferences(
                "Terminal_Punctuation", term, "Punctuation_Other", po, true, Default.ucd());
        ...
        final PrintWriter pw =
                Utility.openPrintWriterGenDir("log/Syntax.txt", Utility.UTF8_WINDOWS);
        showSet(pw, x, false);
        showSet(pw, "[[\\u2000-\\u205F]-" + x + "]", true);
        ...
        final PrintWriter log =
                Utility.openPrintWriterGenDir(
                        "log/TranslitSkeleton_" + script + ".txt", Utility.UTF8_WINDOWS);
        try {
    ...
    private static void tryConsole2() throws UnsupportedEncodingException {
        ...
        System.out.println(
                "Percent corrupted characters: "
                        + ((failures.size() + 0.0) / 0x110000 * 100.0 + "%"));
    ...
    public static void countChars() {
        ...
        for (byte i = -1; i < LIMIT_CATEGORY + 2; ++i) {
            switch (i) {
                ...
                default:
                    System.out.print(
                            UCD.getCategoryID_fromIndex(i, UCD_Types.LONG)
                                    + "\t"
                                    + UCD.getCategoryID_fromIndex(i));
                    break;
                case LIMIT_CATEGORY:
                    System.out.print("Noncharacter" + "\t" + "NCCP");
                    break;
                case LIMIT_CATEGORY + 1:
                    System.out.print("Alphabetic" + "\t" + "alpha");
                    break;
            }
            for (int j = 1; j < AGE_VERSIONS.length; ++j) {
                if (i < 0) {
    ...
    private static void checkForCaseStability(boolean mergeRanges) {
        ...
        for (int i = 0; i <= 0x10FFFF; ++i) {
            ...
            if (!str.equals(Default.nfkd().normalize(str))) {
                compat.add(i);
                // System.out.println(ucd.getCodeAndName(i) + "\t" + (u40.isAllocated(i) ? "already
                // in 4.0" : "new in 4.1"));
            }
        }
        ...
        final UnicodeSet allCased =
                new UnicodeSet().addAll(hasUpper).addAll(hasLower).addAll(hasTitle);
        ...
        System.out.println();
        System.out.println(
                "Bicameral Scripts: those with at least one functionally cased character.");
        System.out.println();
        ...
        printItems(
                bf,
                compat,
                "Bicameral Script: isAlpha or Symbol Modifier, but not isCased",
                new UnicodeSet(scriptSet).retainAll(otherAlphabetic).removeAll(allCased));
        printItems(
                bf,
                compat,
                "Bicameral Script: isCased, but not isAlpha or Symbol Modifier",
    ...
    private static void printItems(
            BagFormatter bf, UnicodeSet compat, String title, UnicodeSet temp) {
        System.out.println();
    ...
    public static void checkShaping() throws IOException {
        ...
        checkProperty(
                ...
                null,
                "General_Category",
                new String[] {"Spacing_Mark", "Enclosing_Mark", "Nonspacing_Mark"});
        log.close();
    }
        ...
        while (it.next()) {
            ...
            for (int i = 0; i < nfd.length(); i += UTF16.getCharCount(cp)) {
                ...
                if (ignoreValueSet.contains(possIgnValue)) {
                    // log.println("--- " + newValue + "\t" + Utility.hex(cp) + " " +
                    // name.getValue(cp));
                    continue;
                }
                // log.println("*** " + newValue + "\t" + Utility.hex(cp) + " " +
    ...
    private static UnicodeSet getSetForName(String regexPattern) {
    ...
    static String convertToBCD(String digits) {
        ...
        /**
         * // add a secondary weight // assume we don't care about more than too many leads/trails
         * leadZeros += 2; // make non-negative; might have padded by 2, for 0 trailZeros += 2; //
         * make non-negative; might have padded by 1 if (leadZeros > 7) leadZeros = 7; if
         * (trailZeros > 7) trailZeros = 7; int secondary = (removedNegative ? 0 : 0x80) // only for
         * zero | (leadZeros << 4) | (removedDecimal ? 0 : 0x08) | (trailZeros); result += ";" +
         * hexByte(secondary);
         */
        return result;
    }
    ...
    static void testFormatHack() {
        ...
        for (int i = 0; i < testLocales.length; ++i) {
            ...
            for (final String testCurrencie : testCurrencies) {
                ...
                nf = getCurrencyFormat(Currency.getInstance(testCurrencie), testLocales[i], false);
    ...
    static NumberFormat getCurrencyFormat(Currency currency, Locale displayLocale, boolean ICU26) {
        ...
        final DecimalFormat result =
                (DecimalFormat) NumberFormat.getCurrencyInstance(displayLocale);
