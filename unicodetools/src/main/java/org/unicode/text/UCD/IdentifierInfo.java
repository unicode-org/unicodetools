...
public class IdentifierInfo {
    ...
    private static final Set<Identifier_Type> SINGLETON_INCLUSION =
            Collections.singleton(Identifier_Type.inclusion);
    ...
    private static final UnicodeMap<General_Category_Values> GENERAL_CATEGORY =
            LATEST.loadEnum(UcdProperty.General_Category, General_Category_Values.class);

    private static final UnicodeSet NON_CHARACTERS =
            GENERAL_CATEGORY.getSet(General_Category_Values.Unassigned);
    private static final UnicodeSet DEPRECATED =
            LATEST.loadEnum(UcdProperty.Deprecated, Binary.class).getSet(Binary.Yes);
    private static final UnicodeSet DEFAULT_IGNORABLE =
            LATEST.loadEnum(UcdProperty.Default_Ignorable_Code_Point, Binary.class)
                    .getSet(Binary.Yes);
    private static final UnicodeSet WHITESPACE =
            LATEST.loadEnum(UcdProperty.White_Space, Binary.class).getSet(Binary.Yes);
    private static final UnicodeSet NOT_NFKC =
            LATEST.loadEnum(UcdProperty.NFKC_Quick_Check, NFKC_Quick_Check_Values.class)
    ...
    private final UnicodeMap<IdentifierInfo.Identifier_Type> removals =
            new UnicodeMap<IdentifierInfo.Identifier_Type>();
    private final UnicodeMap<Set<IdentifierInfo.Identifier_Type>> identifierTypesMap =
            new UnicodeMap<>();
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println(
                info.identifierTypesMap.getSet(
                        singleToSets.get(Identifier_Type.not_characters).iterator().next()));
    ...
    private IdentifierInfo() throws IOException {
        ...
        final UnicodeSet remainingInputSet1 =
                new UnicodeSet(GenerateConfusables.IDNInputSet)
        ...
        final UnicodeMap<String> nonstartingmap =
                new UnicodeMap<String>().putAll(nonstarting, "nonstarting");
        final UnicodeMap.Composer<String> composer =
                new UnicodeMap.Composer<String>() {
                    @Override
        ...
        removals2.putAll(
                UPS.getSet("XID_Continue=Yes").complement(),
    ...
    private void loadFileData() throws IOException {
        ...
        removals.putAll(
                0,
                0x10FFFF, // new
                // UnicodeSet("[^[:gc=cn:][:gc=co:][:gc=cs:][:gc=cc:]-[:whitespace:]]"),
            ...
            try {
                ...
                for (String s : sources) {
                    for (Identifier_Type reasons : reasonSet) {
                        ...
                        if (oldReason == Identifier_Type.inclusion || oldReason == reasons) {
                            continue; // always ok
                        }
                        if (override
                                || oldReason == null
                    ...
                    if (old == null) {
                        ...
                        if (old.replaceBy(status)) {
                            removalCollision.put(
                                    s, "REPLACING " + old + "\t!= (script metadata)\t" + status);
                            removals.put(s, status);
                        } else {
                            removalCollision.put(
        ...
        identifierTypesMap.putAll(
                DEFAULT_IGNORABLE, Collections.singleton(Identifier_Type.default_ignorable));
        identifierTypesMap.putAll(DEPRECATED, Collections.singleton(Identifier_Type.deprecated));
        identifierTypesMap.putAll(
                NON_CHARACTERS, Collections.singleton(Identifier_Type.not_characters));
        identifierTypesMap.putAll(
                GENERAL_CATEGORY.getSet(General_Category_Values.Private_Use),
                Collections.singleton(Identifier_Type.not_characters));
        identifierTypesMap.putAll(
                GENERAL_CATEGORY.getSet(General_Category_Values.Surrogate),
                Collections.singleton(Identifier_Type.not_characters));
        UnicodeSet controlNonWhitespace =
                new UnicodeSet(GENERAL_CATEGORY.getSet(General_Category_Values.Control))
                        .removeAll(WHITESPACE);
        identifierTypesMap.putAll(
                controlNonWhitespace, Collections.singleton(Identifier_Type.not_characters));

        // restore inclusions
        UnicodeSet inclusions2 = identifierTypesMap.getSet(SINGLETON_INCLUSION);
        System.out.println(
                "Restoring inclusions: " + new UnicodeSet(inclusions).removeAll(inclusions2));
        ...
        for (Set<Identifier_Type> value : identifierTypesMap.getAvailableValues()) {
            if (value.contains(Identifier_Type.exclusion)
                    && value.contains(Identifier_Type.obsolete)) {
    ...
    private void addToRemovalSets(
        ...
        } else if (identifierType == Identifier_Type.inclusion
                && !oldSet.contains(Identifier_Type.recommended)) {
            identifierTypesMap.put(codepoint, Collections.singleton(identifierType));
    ...
    private void writeIDReviewInternal() throws IOException {
        final BagFormatter bf =
                GenerateConfusables.makeFormatter()
    ...
    private void printModificationsInternal() throws IOException {
        ...
        //        .setMain("Removals", "GCB", UnicodeProperty.ENUMERATED, "1.0"));
        //
        //        final Set<String> fullListing = new HashSet<String>(Arrays.asList("technical
        // limited-use historic discouraged obsolete".split("\\s+")));
        //        //        final Set<String> sortedValues = new
        // TreeSet<String>(GenerateConfusables.UCAComparator);
        //        //        sortedValues.addAll(recastRemovals.values());
        //        //        if (GenerateConfusables.DEBUG) System.out.println("Restriction Values: "
        // + sortedValues);
        //        for (Identifier_Type value : Identifier_Type.values()) {
        ...
        final UnicodeMap.Composer<String> myComposer =
                new UnicodeMap.Composer<String>() {
                    @Override
                    public String compose(int codePoint, String string, String a, String b) {
                        if (b == null) {
        ...
        final UnicodeSet nonIDNA =
                new UnicodeSet(GenerateConfusables.IDNOutputSet)
                        .addAll(GenerateConfusables.IDNInputSet)
                        .complement();
        ...
        out =
                FileUtilities.openUTF8Writer(
                        GenerateConfusables.reformatedInternal, "draft-restrictions.txt");
        ...
        if (MAIN_CODE) {
            ...
            for (final Iterator<String> it = values.iterator(); it.hasNext(); ) {
                ...
                if (reason1.contains("recommended")) {
                    ...
                    UnicodeSet current =
                            GenerateConfusables.AGE.getSet(GenerateConfusables.VERSION_PROP_VALUE);
    ...
    private void printIdentifierTypes(Style status) throws IOException {
        ...
        final Comparator<String> tempComp =
        ...
        final ToolUnicodePropertySource properties =
        ...
        try (PrintWriter out2 =
            ...
            out2.println(
                    "#\n"
                            + "# For the purpose of regular expressions, the property "
                            + propName
                            + " is defined as\n"
    ...
    private void printIdentifierStatus() throws IOException {
        ...
        for (Set<Identifier_Type> value : identifierTypesMap.values()) {
            if (!value.contains(Identifier_Type.recommended)
                    && !value.contains(Identifier_Type.inclusion)) {
                UnicodeSet set = identifierTypesMap.getSet(value);
        ...
        try (PrintWriter out2 =
            ...
            bf2.setValueSource(
                    (new UnicodeProperty.UnicodeMapProperty() {})
                            .set(tempMap)
                            .setMain(propName, "IDS", UnicodeProperty.EXTENDED_MISC, "1.0"));
