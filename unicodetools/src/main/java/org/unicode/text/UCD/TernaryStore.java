...
public final class TernaryStore {
    ...
    static void test() throws java.io.IOException {
        ...
        try {
            ...
            for (i = 0; counter < tests.length && i <= 0x10FFFF; ++i) {
                ...
                final String temp = UCharacter.getName(i);
                if (temp != null) {
                    tests[counter++] = temp.trim();
                }
            }
    ...
    static void test(String title, String[] tests, int len) {
        ...
        dp.print(title, 0);
        dp.println();
        final TernaryStore.Builder builder = new TernaryStore.Builder();
        int charCount = 0;
        for (int i = 0; i < len; ++i) {
        ...
        final TernaryStore store = builder.build();
        store.showNodes();
        store.checkNodes();
        ...
        final Matcher matcher = store.getMatcher();
        for (int i = 0; i < len; ++i) {
            final int check = test(tests[i], matcher);
            if (check != i) {
    ...
    final static class Matcher {
        ...
        int next() {
            while (lastNode != null && position < s.length()) {
                final char ch = s.charAt(position++);
                do {
                    final String nodeString = lastNode.getString(store.stringStore);
                    final char first = nodeString.charAt(0);
                    if (ch == first) {
                        // now check the rest of the string
                        for (int i = 1; i < nodeString.length(); ++i) {
                            final char other = nodeString.charAt(i);
                        ...
                        final int result = lastNode.result;
    ...
    public Matcher getMatcher() {
        final Matcher result = new Matcher();
        result.store = this;
        return result;
    }
    ...
    public void checkNodes(Node n, NodeInfo nodeInfo) {
        nodeInfo.nodeCount++;
        if (n.result != NOT_FOUND) {
            nodeInfo.resultCount++;
        }
        if (n.less != null) {
            checkNodes(n.less, nodeInfo);
        } else {
            nodeInfo.nullLessCount++;
            if (n.greater == null && n.result == NOT_FOUND) {
                nodeInfo.nullSimpleCount++;
            }
        }
        if (n.next != null) {
    ...
    final static class DepthPrinter {
        ...
        void print(String s, int depth) {
            final int delta = depth - currentDepth;
            if (delta > 0) {
    ...
    final static class StringStore {
        ...
        private int addNoSplit(String s) {
            ...
            final String sTerm = s + TERMINATOR;
        ...
        public String get(int index) {
            ...
            if ((index & 0x8000) == 0) {
                final int end = buffer.indexOf(TERMINATOR, index);
    ...
    final static class Builder {
        ...
        public void replaceStrings(Node n) {
            ...
            n.setString(null);
            if (n.less != null) {
                replaceStrings(n.less);
            }
            if (n.next != null) {
                replaceStrings(n.next);
            }
            if (n.greater != null) {
                replaceStrings(n.greater);
            }
        }
