...
public abstract class UnicodePropertySource implements Cloneable {
    ...
    public abstract String getPropertyValue(int codepoint);

    public abstract Set getAvailablePropertyAliases(Set result);

    public abstract Set getAvailablePropertyValueAliases(Set result);

    public abstract String getPropertyAlias(int nameChoice);

    public abstract String getPropertyValueAlias(String valueAlias, int nameChoice);
        ...
        public String getPropertyValue(int codePoint) {
            if (propEnum < UProperty.INT_LIMIT) {
                ...
                switch (propEnum) {
                    ...
                    case NFD:
                        return Normalizer.normalize(codePoint, Normalizer.NFD);
                    case NFKC:
                        return Normalizer.normalize(codePoint, Normalizer.NFKC);
                    case NFKD:
                        return Normalizer.normalize(codePoint, Normalizer.NFKD);
                }
            return null;
        }

        static final List Extras = Arrays.asList(new String[] {"NFC", "NFD", "NFKC", "NKFD"});

        static final int NFC = 0x8000,
                NFD = 0x8001,
                NFKC = 0x8002,
                NFKD = 0x8003,
                EXTRA_START = NFC,
