...
public class GenerateNormalizeForMatch {
    ...
    private static final UnicodeSet DI =
            iup.loadEnumSet(UcdProperty.Default_Ignorable_Code_Point, Binary.Yes);
    ...
    private static final UnicodeMap<String> cpToSimpleLower =
            iup.load(UcdProperty.Simple_Lowercase_Mapping);
    private static final UnicodeSet UNASSIGNED = GC.getSet(General_Category_Values.Unassigned);
    private static final UnicodeMap<Decomposition_Type_Values> DT =
            iup.loadEnum(UcdProperty.Decomposition_Type, Decomposition_Type_Values.class);
    private static final UnicodeMap<Age_Values> AGE =
            iup.loadEnum(UcdProperty.Age, Age_Values.class);
    private static final UnicodeMap<Block_Values> BLOCK =
            iup.loadEnum(UcdProperty.Block, Block_Values.class);
    private static final UnicodeSet TAGS = BLOCK.getSet(Block_Values.Tags);

    private static final UnicodeSet NFKCCF_SET =
            iup.loadEnumSet(UcdProperty.Changes_When_NFKC_Casefolded, Binary.Yes);
    ...
    private static final NormalizeForMatch ADDITIONS_TO_NFKCCF =
            NormalizeForMatch.load(null, "XNFKCCF-Curated.txt", true);
    private static final NormalizeForMatch ADDITIONS_TO_NFKC =
            NormalizeForMatch.load(
                    Settings.UnicodeTools.DATA_DIR + "cldr/", "NFXC-Curated.txt", true);

    private static final UnicodeSet HANGUL_COMPAT_minus_DI_CN =
            new UnicodeSet(
                            iup.loadEnumSet(
                                    UcdProperty.Block, Block_Values.Hangul_Compatibility_Jamo))
                    .removeAll(DI)
    ...
    private static final UnicodeSet SPECIAL_DECOMP_TYPES =
            PropertyValueSets.getSet(
                    DT, Decomposition_Type_Values.Square, Decomposition_Type_Values.Fraction);
    //            new UnicodeSet("["
    //            + "[:dt=Square:]"
    //            + "[:dt=Fraction:]"
    //            + "]")

    private static final UnicodeSet NOCHANGE_DECOMP_TYPES =
            PropertyValueSets.getSet(
                    DT,
    ...
    private static final UnicodeSet COMBINING =
            PropertyValueSets.getSet(GC, PropertyValueSets.MARK);
    // new UnicodeSet("[:m:]").freeze();

    private static final UnicodeSet HIRAGANA =
            SC.getSet(Script_Values.Hiragana); // new UnicodeSet("[:sc=Hiragana:]").freeze();
    private static final UnicodeSet DECIMAL =
            PropertyValueSets.getSet(GC, PropertyValueSets.NUMBER);
    ...
    public static void main(String[] args) throws IOException {
        ...
        showSimpleData(
                ...
                REASONS,
                "XNFKCCF-NFKCCF.txt",
                "# Cases where XNFKCCF differs from NFKCCF.",
                cpToNFKCCF);
        showSimpleData(
                TRIAL_BASE,
                REASONS_BASE,
                "NFXC-NFKC.txt",
                "# Cases where NFXC differs from NFKC.",
                cpToNFKC);
        showSimpleData(
                TRIAL_BASE, REASONS_BASE, "NFXC-Curated.txt", "# Curated file of exceptions", null);

        showSimpleData(
                TRIAL,
                REASONS,
                "XNFKCCF-NFKC2.txt",
                "# Cases where XNFKCCF differs from NFKC.",
                cpToNFKC);

        showSimpleData(
                N4M, REASONS, "N4M-XNFKCCF.txt", "# Cases where N4M differs from XNFKCCF", TRIAL);
        NormalizeForMatch curated = NormalizeForMatch.load(null, "XNFKCCF-Curated.txt", true);

        showSimpleData(
                curated.getSourceToTarget(),
                curated.getSourceToReason(),
                "XNFKCCF-Curated.txt",
                "# Curated file of exceptions",
                null);
        NormalizeForMatch newCurated =
                NormalizeForMatch.load(
                        Settings.UnicodeTools.DATA_DIR + "n4m/9.0.0/", "XNFKCCF-Curated.txt", true);
        checkNewCurated(curated, newCurated);
        //    private static final NormalizeForMatch ADDITIONS_TO_NFKCCF =
        // NormalizeForMatch.load("XNFKCCF-Curated.txt");
    ...
    private static void checkNewCurated(NormalizeForMatch curated, NormalizeForMatch newCurated) {
        UnicodeSet sources =
                new UnicodeSet(curated.getSourceToTarget().keySet())
                        .addAll(newCurated.getSourceToTarget().keySet());
        int diffCount = 0;
        ...
        System.out.println(
                "Total diff from old to new Curated: " + diffCount + " out of " + sources.size());
    }

    private static void findExtraCaps() {
        HashMap<String, UnicodeSet> nameToCp =
                cpToName.addInverseTo(new HashMap<String, UnicodeSet>());
    ...
    private static void compareTrial(boolean ucaOnly) throws IOException {
        try (PrintWriter out =
                FileUtilities.openUTF8Writer(
                        Settings.Output.GEN_DIR + "n4m/",
                        "XNFKCCF" + (ucaOnly ? "-comp-uca" : "-comp") + ".txt")) {
            UnicodeSet interest =
                    new UnicodeSet(N4M.keySet())
                            .addAll(TRIAL.keySet())
                            .addAll(NFKCCF_SET)
            ...
            out.println(
                    "#Code\tAge\tGC\tDT\tName\tN4M\tTrial\tNFKCCF\tUCA\tΔ\tΔ\tΔ\tN4M Hex\tTrial Hex\tNFKCCF Hex\tUCA Hex\tTrial Reasons");
            TreeSet<Row.R4<Integer, Decomposition_Type_Values, String, String>> sorted =
                    new TreeSet<>();
            for (String s : interest) {
                ...
                String line =
                        "'"
                                ...
                                + "\t'"
                                + AGE.get(cp)
                                        .getShortName() // UCharacter.getAge(cp).getVersionString(2,
                                // 2)
                                + "\t'"
                ...
                Row.R4<Integer, Decomposition_Type_Values, String, String> row =
                        Row.of(
        ...
        Matcher nameCheck =
                Pattern.compile(
                                "FINAL|MEDIAL|INITIAL"
                                        ...
                                        + "|PARENTHESIS|PARENTHESIZED|BRACKET")
                        .matcher("");
    ...
    enum Status {
        ...
        static Status get(String source, String oldTarget, String newTarget) {
            return oldTarget == null
                    ...
                    : newTarget == null
                            ? Status.missing
                            : oldTarget.equals(newTarget) ? Status.same : Status.different;
        }
    }

    private static void removeString(
            final String name, String cp, boolean hack, final String... stringsToFind) {
        ...
        if (finalPos >= 0) {
            ...
            if (otherCode != null) {
                final String target =
                        HANGUL_COMPAT_minus_DI_CN.contains(otherCode)
                                ? otherCode
                                : Normalizer3.NFKCCF.normalize(otherCode);
    ...
    private static <T> void showSimpleData(
            UnicodeMap<String> mapping,
            UnicodeMap<T> reasons2,
            String filename,
            String header,
            UnicodeMap<String> skipIfSame)
            throws IOException {
        try (PrintWriter out =
                FileUtilities.openUTF8Writer(Settings.Output.GEN_DIR + "n4m/", filename)) {
            out.println(header);
            out.println("# Source; \tTarget; \tOther; \tReason(s); \tComments");
            UnicodeSet trialWithoutReason =
                    new UnicodeSet(mapping.keySet()).removeAll(reasons2.keySet());
            if (!trialWithoutReason.isEmpty()) {
                throw new IllegalArgumentException(
                        "Unexplained difference between TRIAL and REASONS: " + trialWithoutReason);
            }
            ...
            for (T reason : sorted) {
                ...
                String reasons =
                        reason instanceof Set
                                ? CollectionUtilities.join((Set) reason, " + ")
                                : reason.toString();
            ...
            if (skipIfSame != null) {
                showSimpleSet(
                        out,
                        new UnicodeSet(mapping.keySet()).complement(),
                        mapping,
                        "other",
                        skipIfSame);
            }
    ...
    private static <T> void showSimpleSet(
            PrintWriter out,
            UnicodeSet set,
            UnicodeMap<String> mapping,
            String reason,
    ...
    private static void computeTrial(
            Normalizer3 normalizer3,
            NormalizeForMatch additions,
            UnicodeMap<String> trial,
            UnicodeMap<Set<SpecialReason>> reasons) {

        UnicodeSet skipIfInMultiCodepointDecomp = new UnicodeSet("[\\u0020<>]");

        UnicodeMap<String> toSuper = new UnicodeMap<>();
        for (String s :
                DT.getSet(Decomposition_Type_Values.Super)) { // new UnicodeSet("[:dt=Super:]")
        ...
        for (int cp = 0; cp <= 0x10FFFF; ++cp) {
            ...
            subloop:
            {
                ...
                // if the target ends with a digit, and there are no other digits, superscript the
                // last
                ...
                //                        if (mod != null) {
                //                            String prefix = target.substring(0,target.length() -
                // Character.charCount(lastCp));
                ...
                if (NOCHANGE_DECOMP_TYPES.contains(cp)) {
                    reason.add(
                            SpecialReason.forString(
                                    "retain_" + DT.get(cp))); // "9 skip certain types";
                    break subloop;
                }
                if (TAGS.contains(cp)) {
                ...
                // length(value) ≠1 && contains any of  " ",  "(",  ".",  ",",  "〔" → no change
                // (discard mapping)
                ...
                if (target.codePointCount(0, target.length()) > 1
                    ...
                    //                                + new
                    // UnicodeSet().addAll(target).retainAll(skipIfInMultiCodepointDecomp)
                    //                                + "» (and isn't singleton)");
                    target = source;
                    ...
                    reason.add(
                            SpecialReason.forString(
                                    "nfkccf_" + DT.get(cp))); // "16. NFKC_CF-" + DT.get(cp);
                }
            }
            ...
            for (Entry<String, String> entry : trial.entrySet()) {
                ...
                if (!newTarget.equals(oldTarget)) {
                    if (newTarget.equals(source)) { // just in case
                        ...
                        LinkedHashSet<SpecialReason> reason =
                                new LinkedHashSet<>(reasons.get(source));
    ...
    private static void showMapping(NormalizeForMatch sourceMap, Normalizer3 nfkccf2) {
        ...
        System.out.println(
                "#source ; target ; nfkccf (if ≠) ; uca equiv (if ≠) # (source→target) names");
    ...
    //
    //        Set<Row.R5<Age, Difference, Decomposition_Type_Values, General_Category_Values,
    // String>> sorted = new TreeSet<>();
    ...
    //
    //            String reason = REASONS.get(source);
    //            General_Category_Values generalCategory = GC.get(sourceCodePoint); //
    // UCharacter.getIntPropertyValue(sourceCodePoint, UProperty.GENERAL_CATEGORY);
    //            Decomposition_Type_Values decompType = DT.get(sourceCodePoint); // int decompType
    // = UCharacter.getIntPropertyValue(sourceCodePoint, UProperty.DECOMPOSITION_TYPE);
    //
    ...
    //            }
    //            final R5<Age, Difference, Decomposition_Type_Values, General_Category_Values,
    // String> row
    ...
    //                );
    //
    //        for (R5<Age, Difference, Decomposition_Type_Values, General_Category_Values, String>
    // item : sorted) {
    ...
    //            }
    //            System.out.println(age + SEP + difference + SEP + decompType + SEP + cat + SEP +
    // info);
    //        }
    //        System.out.println();
    ...
    private static String hex(String n4mValue) {
        return n4mValue == null
                ? "<unchanged>"
                : n4mValue.isEmpty() ? "delete" : "U+" + Utility.hex(n4mValue, 4, ", U+");
    }
}
