...
/**
 * Class that encapsulates the data from confusables.txt. It currently generates its own
 * whole-script confusable data, because of omissions in the Unicode data file.
 *
 * @author markdavis
 */
public class Confusables {
    ...
    private final Map<Style, UnicodeMap<String>> style2RawMapToRepresentative;
    private final UnicodeSet hasConfusable = new UnicodeSet();
    private final UnicodeMap<EnumMap<Style, String>> char2data =
            new UnicodeMap<EnumMap<Style, String>>();

    private final Map<Script_Values, Map<Script_Values, CodepointToConfusables>>
            scriptToScriptToCodepointToUnicodeSet;

    /** Mapping from codepoint to name. */
    public static final UnicodeMap<String> CODEPOINT_TO_NAME =
            ScriptDetector.IUP.load(UcdProperty.Name);
    ...
    public Confusables(String directory) {
        try {
            EnumMap<Style, UnicodeMap<String>> _style2RawMapToRepresentative =
                    new EnumMap<Style, UnicodeMap<String>>(Style.class);
            for (String line : FileUtilities.in(directory, "confusables.txt")) {
                ...
                // {
                //                    addConfusable(style, target, source,
                // _style2RawMapToRepresentative);
                //                }
            }
            style2RawMapToRepresentative =
                    CldrUtility.protectCollection(_style2RawMapToRepresentative);
            char2data.freeze();
            hasConfusable.freeze();

            // patch, because the file doesn't contain X => common/inherited or the targetSet

            UnicodeMap<String> codepointToRepresentativeConfusable =
                    style2RawMapToRepresentative.get(Style.MA);
            Map<Script_Values, Map<Script_Values, CodepointToConfusables>>
                    _scriptToScriptToCodepointToUnicodeSet = new EnumMap<>(Script_Values.class);
            ...
            for (String representative : codepointToRepresentativeConfusable.values()) {
                UnicodeSet equivalents =
                        new UnicodeSet(codepointToRepresentativeConfusable.getSet(representative))
                                .add(representative);
                for (String a : equivalents) {
                    for (String b : equivalents) {
                        ...
                        for (Script_Values aScript : aScripts) {
                            Set<Script_Values> bScripts =
                                    scriptDetector.set(b).getSingleSetOrNull();
                            if (bScripts == null) {
                            ...
                            for (Script_Values bScript : bScripts) {
                                addToMap(
                                        ...
                                        aSingle,
                                        b,
                                        _scriptToScriptToCodepointToUnicodeSet);
                            }
                        }
                    }
                }
            ...
            scriptToScriptToCodepointToUnicodeSet =
                    CldrUtility.protectCollection(_scriptToScriptToCodepointToUnicodeSet);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }

    private void addConfusable(
            Style style,
            String source,
            String target,
            EnumMap<Style, UnicodeMap<String>> _style2map) {
     ...
     */
    public Map<Script_Values, Map<Script_Values, CodepointToConfusables>>
            getScriptToScriptToCodepointToUnicodeSet() {
    ...
    private int getSingleCodePoint(String a) {
        return a.length() == 1 || a.length() == 2 && Character.isHighSurrogate(a.charAt(0))
                ? a.codePointAt(0)
                : -1;
    }

    private void addToMap(
            Map<Script_Values, Map<Script_Values, UnicodeSet>> _scriptToScriptToUnicodeSet,
            int aSingle,
    ...
    //            for (int codepoint = range.codepoint; codepoint <= range.codepointEnd;
    // ++codepoint) {
    ...
    /**
     * Return whole-script confusables data. Augments the Unicode data by adding the set of
     * characters mapped to.
     *
     * @return null if no match for script1+script2
     */
    public CodepointToConfusables getCharsToConfusables(
            Script_Values sourceScript, Script_Values targetScript) {
        Map<Script_Values, CodepointToConfusables> map1 =
                scriptToScriptToCodepointToUnicodeSet.get(sourceScript);
        if (map1 == null) {
            return null;
        }
     ...
     * @author markdavis
     */
    public static class CodepointToConfusables
            implements Iterable<Entry<Integer, UnicodeSet>>, Freezable<CodepointToConfusables> {
        boolean isFrozen;
    ...
    public void print(Appendable out) {
        try {
            for (Entry<Script_Values, Map<Script_Values, CodepointToConfusables>>
                    scriptToCodepointToUnicodeSet :
    ...
    public static void main(String[] args) throws IOException {
        ...
        final Confusables CONFUSABLES = new Confusables(securityDir);
        try (PrintWriter pw =
                FileUtilities.openUTF8Writer(
                        Settings.Output.GEN_UCD_DIR, "confusablesWholeScript.txt")) {
