...
class ExtendedPictographic {
    ...
    static void load(File file) throws IOException {
        ...
        for (String line :
                FileUtilities.in(file.getParent(), file.getName())) { // Settings.DATA_DIR +
            // "cldr/","ExtendedPictographic.txt")
    ...
    enum MyOptions {
        destination(
                new Params()
                        .setHelp("File to read and change.")
                        .setMatch(".*")
                        .setDefault(
                                CLDRPaths.COMMON_DIRECTORY
                                        + "properties/ExtendedPictographic.txt")),
    ...
    public static void main(String[] args) throws IOException {
        ...
        try (PrintWriter _out = FileUtilities.openUTF8Writer(file.getParent(), file.getName())) {
            ...
            if (MyOptions.operations.option.doesOccur()) {
                UnicodeSet ops =
                        new UnicodeSet(
                                        "[[:s:][:p:]-[:sc:]-[:xidcontinue:]-[:nfkcqc=n:]&[:scx=Common:]]")
                                .removeAll(picto);
            ...
            if (doNormal || MyOptions.normal.option.doesOccur()) {
                ...
                for (Block_Values block : emojiBlocks) {
                    ...
                    UnicodeSet cnInBlock =
                            new UnicodeSet(blockSet).retainAll(Cn).removeAll(gazInBlock);
    ...
    private static String showRange(
            IndexUnicodeProperties iup, Age_Values v, int cpStart, int cpEnd) {
        return Utility.hex(cpStart)
                + ".."
                + Utility.hex(cpEnd)
