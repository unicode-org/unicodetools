...
public class EmojiFlagOrder {
        ...
        ImageInfo(String name, BufferedImage image) {
            ...
            for (int x = 1; x < xCount; ++x) {
                for (int y = 1; y < yCount; ++y) {
                    ...
                    //                        || x < xCount - 1 && base.distanceTo(colors[x+1][y]) >
                    // MIN
                    //                        ) {
                    //                    distance++;
                    //                }
                    int core =
                            Math.max(
                                    base.distanceTo(colors[x - 1][y - 1]),
                                    Math.max(
                                            base.distanceTo(colors[x - 1][y]),
                                            Math.max(
                                                    base.distanceTo(colors[x][y - 1]),
                                                    +(x < xCount - 1
                                                            ? base.distanceTo(colors[x + 1][y])
                                                            : 0))));
                    distance += core;
    ...
    public static HSB getHSB(
            int x, int y, final WritableRaster raster, final ColorModel colorModel) {
    ...
    static void getFlagOrder() throws IOException {
        ...
        for (String s : EmojiData.EMOJI_DATA.getChars()) {
            ...
            String sortString =
                    localeDisplayNames.regionDisplayName(Emoji.getRegionCodeFromEmoji(s));
            ...
            for (Pair<ImageInfo, String> colorChar : sorted) {
                ...
                out.print(
                        "<tr><td>"
                                + codePoints
                                + "</td><td>"
                                + Emoji.getFlagRegionName(codePoints)
                                + "</td><td padding='2px'>"
                                + getFlag(codePoints)
                                + "</td><td>"
                                + percent2.format(info.overall / base)
                                + "</td>");
                int limit = 15;
                    ...
                    for (R2<Long, HSB> item : info.colorDistribution) {
                        ...
                        out.println(
                                "<td style='background-color:#"
                                        ...
                                        + inverse.toString()
                                        + "'>"
                                        + percent.format(item.get0() / (double) info.count)
                                        + " "
                                        + item.get1()
                                        + "</td>");
                        if (--limit < 0) break;
    ...
    static String getFlag(String chars) {
        ...
        return "<img" + " alt='" + chars + "'" + " class='big'" + " src='" + getFile(chars) + "'>";
    }

    public static String getFile(String s) {
        String core = Emoji.buildFileName(s, "_");
        return Settings.Output.GEN_DIR + "emoji_images/country-flags/ref_" + core + ".png";
    }
