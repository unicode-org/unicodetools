...
public class GenerateOldestAnnotations {
    ...
    private static final Set<String> ANNOTATED_CHARS =
            new UnicodeSet(EmojiData.EMOJI_DATA.getSortingChars())
    ...
    private static void printText(OldAnnotationData data) throws IOException {
        try (PrintWriter outText =
                FileUtilities.openUTF8Writer(
                        Settings.Output.GEN_DIR + "emoji/tts/", data.locale + ".tsv")) {
            // #Code Image   TTS English TTS German  Annotations English Annotations German
            // Comments    INTERNAL
            // U+1F600   =vlookup(A2,Internal!A:B,2,0)   grinning face   Lachender Smiley    face;
            // grin  Lachender Smiley; Gesicht; lustig; lol
            int line = 1;
            outText.println(
                    "#Code"
                            + "\tImage"
            ...
            for (String s : ANNOTATED_CHARS) {
                ...
                final String englishAnnotationString =
                        CollectionUtilities.join(english.map.get(s), "; ");
                final String annotationString =
                        annotations == null ? MISSING : CollectionUtilities.join(annotations, "; ");
                ++line;
                outText.println(
                        "U+"
                                + Utility.hex(s, 4, " U+")
    ...
    private static LocaleData printXml(OldAnnotationData data, UnicodeSet missing)
            throws IOException {
        ...
        try (PrintWriter outText =
                FileUtilities.openUTF8Writer(
                        CLDRPaths.COMMON_DIRECTORY + "/annotations/", data.locale + ".xml")) {
            outText.append(
            ...
            for (String s : ANNOTATED_CHARS) {
                ...
                if (ttsString != null) {
                    outText.append("' tts='").append(fix(ttsString, ld));
                }
                outText.append(
                                "'"
    ...
    private static void showDiff(
            String label, String a, Set<String> aSet, String b, Set<String> bSet) {
        showDiff2(label, a, aSet, b, bSet, true);
        showDiff2(label, b, bSet, a, aSet, true);
        showDiff2(label, a, bSet, b, aSet, false);
    }

    private static void showDiff2(
            String label, String a, Set<String> aSet, String b, Set<String> bSet, boolean diff) {
        ...
        System.out.println(
                label
                        ...
                        + aMinusB.size()
                        + "\t"
                        + CollectionUtilities.join(aMinusB, " "));
    }
}
