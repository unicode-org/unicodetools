...
public class CountValidEmoji {
    ...
    private static void countInvalid(Validity validity, LstrType type) {
        ...
        if (type == null) {
            ...
            // idStatus="regular", "deprecated", or the "macroregion". However, for macroregions,
            // only UN and EU are valid.
            Multimap<Status, String> inverse =
                    Multimaps.invertFrom(Multimaps.forMap(codeToStatus), TreeMultimap.create());
        ...
        System.out.println("\n" + title);
        System.out.println(
                "Recommended:\t" + nf.format(recommendedSize) + "\t" + clip(recommended));
        System.out.println("Other Valid:\t" + nf.format(otherValidSize) + "\t" + clip(other_valid));
        System.out.println(
                "Invalid (but WF):\t"
                        + nf.format(syntactic_but_invalid_size)
                        + "\t"
                        + clip(invalid));
    }
