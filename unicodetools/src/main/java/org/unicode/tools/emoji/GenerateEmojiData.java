...
public class GenerateEmojiData {
    ...
    //        public static final String SV_WARNING = "Three characters used in emoji zwj sequences
    // with the emoji presentation selector (VS16 = FE0F) do not yet appear in
    // StandardizedVariants.txt.\n"
    //                        + "Implementations fully supporting these three characters as emoji
    // will need to allow for FE0F, by the following addition to that file:";
    //
    //        public static final String SV_CHARS = "2640 FE0E; text style; # FEMALE SIGN\n"
    //                        + "2640 FE0F; emoji style; # FEMALE SIGN\n" + "2642 FE0E; text style;
    // # MALE SIGN\n"
    //                        + "2642 FE0F; emoji style; # MALE SIGN\n" + "2695 FE0E; text style;  #
    ...
    public static final UnicodeSet genderBase =
            new UnicodeSet(
                            "[👲 👳  💂  👯  🕴   👱 👮  🕵 💆"
                                    + " 💇 👰 🙍 🙎 🙅 🙆 💁 🙋 🗣 👤 👥 🙇 🚶 🏃 🚴 🚵 🚣 🛀 🏄 🏊 ⛹ 🏋"
    ...
    public static final String EXPLICIT_GENDER_LIST =
            new StringBuffer()
    ...
    public static final UnicodeSet hairBase =
            new UnicodeSet(
                            "[👶 👮 👲 👳 👸 🕵 👼 💆 💇 👰 🙍 🙎 🙅 🙆 💁 🙋 🙇 "
                                    + "🚶 🏃 💃 🚣  🏄 🏊 ⛹ 🏋 "
                                    + "👦 👧  👩 👴 👵  "
                    ...
                    .freeze();
    public static final UnicodeSet directionBase =
            new UnicodeSet(
                            "[😘 🚶 🏃 ✌ ✋ 👋-👍 👏 💪 👀 🤘 💨 ✈ 🎷 🎺 🔨 ⛏ 🗡 🔫 🚬 "
    ...
    public static <T> void printData(EmojiDataSource emojiDataSource) throws IOException {
        ...
        try (TempPrintWriter outText2 =
            ...
            outText2.println(
                    Utility.getBaseDataHeader(
                            "emoji-internal", 51, "Emoji Data Internal", VERSION_STRING));

            int width =
                    maxLength(
                            "Emoji_Gender_Base", "Emoji_Explicit_Gender", "Multi_Person_Groupings");
        ...
        try (TempPrintWriter outText2 =
            ...
            outText2.println(
                    "# To reduce the number of lines, the strings are transformed to skeletons:\n"
        ...
        try (TempPrintWriter outText2 = new TempPrintWriter(ucdPathString, "emoji-data.txt")) {
            ...
            if (SHOW)
                System.out.println(
                        EmojiData.EMOJI_DATA_BETA.getExtendedPictographic().contains("🦰"));
            if (SHOW)
                System.out.println(
            ...
            int width =
                    Math.max(
                            "Emoji".length(),
                            Math.max(
                                    "Emoji_Presentation".length(),
                                    Math.max(
                                            "Emoji_Modifier".length(),
                                            // Math.max("Emoji_Regional_Indicator".length(),
                                            Math.max(
                                                    "Emoji_Component".length(),
            ...
            printer.show(
                    ...
                    true,
                    false);
            //                        printer.show(outText2, "Emoji_Regional_Indicator", null,
            // width, 14, emoji_regional_indicators, true, true,
            //                                        false);
            printer.show(
                    outText2,
                    "Emoji_Component",
        ...
        //            try (TempPrintWriter outText2 = new TempPrintWriter(OUTPUT_DIR,
        // "emoji-extended-data.txt")) {
        //                UnicodeSet emoji_pict = emojiDataSource.getExtendedPictographic();
        //                outText2.println(Utility.getBaseDataHeader("emoji-extended-data", 51,
        // "Emoji Data", Emoji.VERSION_STRING));
        //                int width = "Extended_Pictographic".length();
        //
        //                outText2.println("# Format: ");
        //                outText2.println("# codepoint(s) ; property(=Yes) # comments ");
        //                outText2.println(ORDERING_NOTE);
        //                printer.show(outText2, "Extended_Pictographic", null, width, 14,
        // emoji_pict, true, true, false);
        //                outText2.println("\n#EOF");
        //            }
        //        }
        ...
        try (Writer out = new TempPrintWriter(emojiPathString, "emoji-sequences.txt");
            ...
            outNonRgi.write(
                    Utility.getBaseDataHeader(
                                    "emoji-sequences-nonrgi",
                                    51,
                                    "Emoji Sequence Data — Non-RGI",
                                    VERSION_STRING)
                            + "\n");
            List<String> type_fields =
                    Arrays.asList(
                            "Basic_Emoji",
            ...
            printer.show(
                    ...
                    width,
                    14,
                    emojiDataSource.getBasicSequences(),
                    true,
                    false,
                    true);
            printer.show(
            ...
            //                if (emojiMultiPersonGroupings.containsSome(x) &&
            // !HOLDING_HANDS.containsSome(x)) {
            ...
            //
            //            printer.show(outNonRgi, "Emoji_Modifier_Sequence_Non_RGI", null, width,
            // 14,
            //                    nonRgiSequences,
            //                    false, false, true);
        ...
        try (Writer out = new TempPrintWriter(emojiPathString, "emoji-zwj-sequences.txt");
            ...
            outNonRgi.write(
                    Utility.getBaseDataHeader(
                                    "emoji-zwj-sequences-nonrgi",
                                    51,
                                    "Emoji ZWJ Sequences — Non-RGI",
                                    VERSION_STRING)
                            + "\n");
            ...
            printer.show(
                    out, REZS, "Family", width, 44, types.getSet("family"), false, false, true);
            printer.show(out, REZS, "Role", width, 44, types.getSet("role"), false, false, true);
            printer.show(
                    out, REZS, "Gendered", width, 44, types.getSet("gendered"), false, false, true);
            printer.show(out, REZS, "Hair", width, 44, types.getSet("hair"), false, false, true);
            printer.show(out, REZS, "Other", width, 44, types.getSet("other"), false, false, true);

            printer.show(
                    outNonRgi, REZS, "Non-RGI", width, 44, nonRgiZwjSequences, false, false, true);
        ...
        if (DO_TAGS) {
            ...
            try (PrintWriter out =
                    FileUtilities.openUTF8Writer(
                            ...
                            "emoji-tags.txt")) {
                out.println(
                        Utility.getBaseDataHeader("emoji-tags", 52, "Emoji Data", VERSION_STRING));
                List<String> type_fields =
                        Arrays.asList(
                                "Emoji_Flag_Base",
                                "Emoji_Gender_Base",
                ...
                printer.show(
                        out, "Emoji_Gender_Base", null, width, 6, genderBase, false, true, false);
        ...
        if (SHOW)
            System.out.println(
                    "Regional_Indicators ; " + Emoji.REGIONAL_INDICATORS.toPattern(false));
        if (SHOW)
            System.out.println(
                    "Emoji Combining Bases ; "
                            + EmojiData.EMOJI_DATA.getKeycapBases().toPattern(false));
        if (SHOW)
            System.out.println(
                    "Emoji All ; "
    ...
    static final UnicodeMap<String> NAMES =
            IndexUnicodeProperties.make(Settings.latestVersion).load(UcdProperty.Name);
    static final UnicodeMap<Age_Values> AGES =
    ...
    private static String getVariationComment(String s) {
        ...
        return "("
                + (age == null
                        ? DATA_VERSION_TO_GENERATE.getVersionString(2, 2)
                        : age.getShortName())
                + ") "
                + name;
    }
    ...
    private static void showTypeFieldsMessage(
        ...
        out.write(
                "#\n"
                        ...
                        + (type_fields.size() != 1
                                ? "each of the type fields"
                                : "the above type field")
                        + " defines the name of\n"
        ...
        //                        out.write("#\n" + "# In display and processing, sequences should
        // be supported both with and without FE0F."
        //                                        + "\n# " + SV_WARNING.replace("\n", "\n# ") +
        // "\n#" + "\n# " + SV_CHARS.replace("\n", "\n# "));
        //                }
    }
    ...
    static final String EXCEPTION_ZWJ =
            new StringBuilder()
    ...
    static class PropPrinter {
        ...
        void show(
                ...
                String comments,
                int maxTitleWidth,
                int maxCodepointWidth,
                UnicodeSet emojiChars,
                boolean addVariants,
                boolean showMissingLine,
                boolean showName) {
            try {
                Tabber tabber =
        ...
        private String getAgeString(VersionInfo value) {
            return Emoji.getShortName(
                    value); // value != Age_Values.Unassigned ? value.getShortName() :
            // CandidateData.CANDIDATE_VERSION;
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        EmojiDataSource emojiDataSource =
                MyOptions.Version.option.doesOccur()
    ...
    private static void showExtraInfo() {
            ...
            if (s.codepoint == s.codepointEnd) {
                System.out.println(
                        Utility.hex(s.codepoint)
                                + " ;\tEmoji_Gender_Base ;\t"
                                + iup.getName(s.codepoint));
            } else {
                System.out.println(
                        Utility.hex(s.codepoint)
                                + ".."
                                + Utility.hex(s.codepointEnd)
                                + " ;\tEmoji_Gender_Base ;\t"
                                + iup.getName(s.codepoint)
                                + ".."
                                + iup.getName(s.codepointEnd));
            }
        }
        ...
        all.removeAll(emojiDataSource.getSingletonsWithoutDefectives())
                .removeAll(emojiDataSource.getEmojiComponents());
        for (String s : all) {
    ...
    static final UnicodeMap<General_Category_Values> gcMap =
