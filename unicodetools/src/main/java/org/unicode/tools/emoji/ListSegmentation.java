...
public class ListSegmentation {
    private static final UnicodeSet SINGLETONS_WITHOUT_DEFECTIVES =
            EmojiData.EMOJI_DATA.getSingletonsWithoutDefectives();
    static final IndexUnicodeProperties iup = IndexUnicodeProperties.make("9.0.0");
    static final UnicodeMap<Line_Break_Values> lb =
            iup.loadEnum(UcdProperty.Line_Break, Line_Break_Values.class);
    static final UnicodeMap<Word_Break_Values> wb =
            iup.loadEnum(UcdProperty.Word_Break, Word_Break_Values.class);
    static final UnicodeMap<Grapheme_Cluster_Break_Values> gcb =
            iup.loadEnum(UcdProperty.Grapheme_Cluster_Break, Grapheme_Cluster_Break_Values.class);
    ...
    public static void main(String[] args) {
        ...
        for (List<String> value : items.values()) {
            ...
            System.out.println(
                    CollectionUtilities.join(value, "\t") + "\t" + us.size() + "\t" + show(us));
        }
        ...
        rest = new UnicodeSet("[⎈ ✀ ✁ ✃ ✄ ✑ ❦ ❧ 🀰-🂓 🂠-🂮 🂱-🂿 🃁-🃎 🃑-🃵 🕏]");
        System.out.println(
                "\nOther possible pictographs"
                        + "\t"
                        + rest.size()); //  + "\t" + rest.toPattern(false));
        showList(rest);
    }
    ...
    private static void showList(UnicodeSet rest) {
        ...
        for (String s : sorted) {
            System.out.println(
                    "U+" + Utility.hex(s) + "\t" + UCharacter.getName(s, ", ") + "\t" + s);
        }
    }
