...
public class EmojiAnnotations extends Birelation<String, String> {
    ...
    private final UnicodeMap<String> shortNames = new UnicodeMap<>();
    private final UnicodeMap<Status> statusValues = new UnicodeMap<>();
    ...
    //    private static final Set<String> STOP_WORDS = new HashSet<>(Arrays.asList("the", "of",
    // "for", "a", "and", "state",
    //            "c�te", "verde▪cape", "dhekelia", "akrotiri", "comros", "pdr", "jamahiriya",
    // "part",
    ...
    public EmojiAnnotations(
        ...
        // Loader loader = new Loader(CLDRConfig.getInstance().getCLDRFile(localeString, true),
        // annotationData);
        ...
        for (String s : EmojiData.EMOJI_DATA.getChars()) {
            ...
            if (outShortName.value == null
                    || outShortName.value.contains(Annotations.ENGLISH_MARKER)) {
        ...
        //            }
        //        }
        // Output<Set<String>> lastLabel = new Output<Set<String>>(new
        // TreeSet<String>(codepointCompare));
        //        for (String filename : filenames) {
        //            int lineCount = 0;
        //            int lineNumber = 0;
        //            EmojiIterator ei = new
        // EmojiIterator(EmojiData.of(Emoji.VERSION_LAST_RELEASED), true);
        //
        ...
        //                        UnicodeSet overlap = new
        // UnicodeSet().addAll(string).retainAll(Emoji.ASCII_LETTERS);
        //                        String withPosition = line.replaceAll("("+overlap+")", "###$1");
        //                        throw new IllegalArgumentException(lineNumber + "\tStrange line
        // with ASCII emoji: " + overlap + "; "+ withPosition);
        //                    }
        ...
        //                }
        //            }
        //            if (SHOW) System.out.println(lineCount + "\tannotation lines from " +
        // filename);
        //        }
        ...
        //                String emoji = new
        // StringBuilder().appendCodePoint(cp1).appendCodePoint(cp2).toString();
        ...
        // UnicodeSet().addAll(EmojiData.EMOJI_DATA.getChars()).removeAll(annotationCharacters);
    ...
    static class Loader {
        ...
        static final UnicodeSet FAMILY_PLUS =
                new UnicodeSet(Emoji.FAMILY_MARKERS)
                        .add(Emoji.JOINER)
            ...
            this.KEYCAP_PATTERN =
                    SimpleFormatter.compile(
                            cldrFile.getStringValue(
                                    "//ldml/characterLabels/characterLabelPattern[@type=\"keycap\"]"));
            this.COMBINE_PATTERN =
                    SimpleFormatter.compile(
                            cldrFile.getStringValue(
        ...
        private Status getNameAndAnnotations(
                String s, final Set<String> keywordsToAppendTo, Output<String> outShortName) {
            if (s.equals("💃")) {
                int debug = 0;
            }
            ...
            if (datum != null) {
                ...
                //                    outShortName.value =
                // sep.format(outShortName.value.toLowerCase(Locale.ENGLISH), type);
                ...
                if (s.contains(KISS)) {
                    ...
                    Annotations familyDatum = data.get("👪"); //        <annotation cp="👪"
                    // type="tts">Familie</annotation>
                    if (familyDatum != null) {
                        base = familyDatum.getShortName();
                        keywordsToAppendTo.addAll(familyDatum.getKeywords());
                        status = Status.found;
                    ...
                    Annotations familyDatum =
                            data.get(rem.contains("♂") ? "👨" : "👩"); //        <annotation cp="👪"
                    // type="tts">Familie</annotation>
                    if (familyDatum != null) {
                        outShortName.value =
                                fix(s, sep, outShortName.value, familyDatum.getShortName());
                        keywordsToAppendTo.addAll(familyDatum.getKeywords());
                        status = Status.gender;
                    } else {
                        status = Status.missing;
                ...
                for (int cp : CharSequences.codePoints(s)) {
                    ...
                    if (partDatum != null) {
                        outShortName.value =
                                fix(s, sep, outShortName.value, partDatum.getShortName());
