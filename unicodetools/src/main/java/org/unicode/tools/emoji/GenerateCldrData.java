...
public class GenerateCldrData {
    ...
    public static void main(String[] args) throws IOException {
        ...
        //        final String OUTPUT_DIR = CLDRPaths.GEN_DIRECTORY + "cldr/emoji/" + Emoji.VERSION_BETA_STRING;

        //        UnicodeSet temp = new UnicodeSet(onlyNew).addAll(found);

        //GenerateEmojiTestFile.showLines(EmojiOrder.BETA_ORDER, temp, Target.propFile, OUTPUT_DIR);
    ...
    private static void getEmojiDataLines() {
        Set<String> found = getSortedFiltered(betaPlusCandidateData.getAllEmojiWithDefectives(), candidateData.comparator, false); // releaseData, betaPlusCandidateData
        if (false) {
            Set<String> found2 = getSortedFiltered(betaPlusCandidateData.getAllEmojiWithDefectives(), EmojiOrder.STD_ORDER.codepointCompare, false); // releaseData, betaPlusCandidateData
        ...
        System.out.println("# Generated lines from Emoji Data v" + versionString + ", using GenerateCldrData.java");
        ...
        for (String s : found) {
            ...
            if (!minorCat.equals(oldMinorCat)) {
                if (minorSeen.contains(minorCat)) {
                    throw new ICUException("Bad ordering for " + minorCat + "\t" + s
            ...
            addDataLine(withoutVS.replace(Emoji.EMOJI_VARIANT_STRING,""), name, doneAlready, "unqualified", tabber);
        }
    }

    private static void addDataLine(String s, String name, Set<String> doneAlready, String classification, Tabber tabber) {
    ...
    private static Set<String> getSortedFiltered(UnicodeSet onlyNew, Comparator<String> comparator, boolean filtered) {
    ...
    private static void getLines(Set<String> found, boolean isRoot, UnicodeMap<Annotations> cldrData) {
        int counter = 0;
            ...
            if (annotations != null) {
                if (!isRoot) {
                    ...
                    if (!Objects.equals(candidateName, cldrName)) {
                        warnings.add(rootCode + " Names Differ:"
                                + "\tcldr:\t" + cldrName
                    ...
                    if (!Objects.equals(candidateKeywords, cldrKeywords)) {
                        warnings.add(rootCode + " Keywords Differ:"
                                + "\tcldr:\t" + cldrKeywords
                                + "\tcandidate:\t" + candidateKeywords 
                                );
                    }
                }
                continue;
        ...
        if (!isRoot) {
            if (!warnings.isEmpty()) {
                System.out.println("Differences from candidate to cldr"
            ...
            UnicodeSet extraInRoot = new UnicodeSet().addAll(found)
