    ...
    private static void showSet(String title, UnicodeSet set) {
        ...
        final String compact =
                getCompact(
                        ...
                        possibleBridge,
                        true,
                        60); // "[[:Cn:][:nfkcqc=n:][:XID_Continue=n:]]");
        System.out.println(
                "public static final UnicodeSet " + title + " = new UnicodeSet(" + compact + ");");
    }

    private static String getCompact(
            UnicodeSet original, String possibleBridge, boolean escape, int width) {
        ...
        if (!possibleBridge.isEmpty()) {
            ...
            if (dontCare.containsNone(original)) {
                ...
                final String compactString =
                        "[" + compact.toPattern(escape) + "-" + possibleBridge + "]";
