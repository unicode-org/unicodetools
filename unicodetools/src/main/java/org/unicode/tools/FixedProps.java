...
public class FixedProps {
    private static final Set<Script_Values> SINGLETON_INHERITED =
            singleton(Script_Values.Inherited);
    ...
    // private static final UnicodeMap<Script_Values> sc = iup.loadEnum(UcdProperty.Script,
    // Script_Values.class);
    private static final UnicodeMap<Set<Script_Values>> scx =
    ...
    private static final String GEN_FIXED_DIR = Settings.Output.GEN_DIR + "fixed/";
    ...
    public static final class FixedGeneralCategory {
        ...
        static {
            ...
            for (String s : allButOther) {
                ...
                // 〔S〕
                EnumSet<General_Category_Values> gcSet =
                        getSetValues(nfkded, generalCategoryRev, temp);
                if (gcSet.size() == 1) {
    ...
    private static <T, U extends Collection<T>> U getSetValues(
            String source, UnicodeMap<T> map, U target) {
        target.clear();
    ...
    private static Script_Values getScript(final String scriptCode) {
        ...
        if (scriptValue == null) {
            switch (scriptCode) {
                case "Punc":
                    scriptValue = Script_Values.Japanese;
                    break;
                case "Symm":
                    scriptValue = Script_Values.Math_Symbols;
                    break;
                case "Syme":
                    scriptValue = Script_Values.Emoji_Symbols;
                    break;
                case "Symo":
                    scriptValue = Script_Values.Other_Symbols;
                    break;
                default:
                    throw new IllegalArgumentException();
            }
    ...
    public static void main(String[] args) throws IOException {
        System.out.println("\n# Fixed Nfkd.\n");
        {
            ...
            try (PrintWriter out = FileUtilities.openUTF8Writer(GEN_FIXED_DIR, "FixedNfkd.txt")) {
                showmap(out, FixedNfkd.changes, temp, literalTemp);
            }
            try (PrintWriter out =
                    FileUtilities.openUTF8Writer(GEN_FIXED_DIR, "FixedNfkdDiff.txt")) {
        ...
        System.out.println("\n# Fixed GC.\n");
        {
            ...
            try (PrintWriter out = FileUtilities.openUTF8Writer(GEN_FIXED_DIR, "FixedGc.txt")) {
                showmap(
                        out,
                        FixedGeneralCategory.changes,
                        FixedGeneralCategory.generalCategoryRev,
                        FixedGeneralCategory.generalCategoryRev);
            }
            try (PrintWriter out = FileUtilities.openUTF8Writer(GEN_FIXED_DIR, "FixedGcDiff.txt")) {
        ...
        System.out.println("\n# Fixed Script.\n");
        {
            final UnicodeMap<Set<Script_Values>> scx =
                    iup.loadEnumSet(UcdProperty.Script_Extensions, Script_Values.class);
            UnicodeMap<String> diff = new UnicodeMap<>();
            ...
            try (PrintWriter out = FileUtilities.openUTF8Writer(GEN_FIXED_DIR, "FixedScx.txt")) {
                showmap(
                        out,
                        FixedScriptExceptions.changes,
                        FixedScriptExceptions.scriptRev,
                        FixedScriptExceptions.scriptRev);
            }
            try (PrintWriter out =
                    FileUtilities.openUTF8Writer(GEN_FIXED_DIR, "FixedScxDiff.txt")) {
                showmap(out, diff.keySet(), diff, diff);
            }
        }
    ...
    private static void showOld() {
        ...
        UnicodeSet us =
                new UnicodeSet(FixedScriptExceptions.getSet(SINGLETON_COMMON))
                        .addAll(FixedScriptExceptions.scriptRev.getSet(SINGLETON_INHERITED))
                        .removeAll(
                                FixedGeneralCategory.getSet(
                                        General_Category_Values.Space_Separator))
                        .removeAll(
                                FixedGeneralCategory.getSet(General_Category_Values.Line_Separator))
                        .removeAll(
                                FixedGeneralCategory.getSet(
                        ...
                        .removeAll(
                                iup.loadEnum(UcdProperty.Variation_Selector, Binary.class)
                                        .getSet(Binary.Yes));
        ...
        UnicodeSet[] diffs =
                new UnicodeSet[] {new UnicodeSet(a).removeAll(b), new UnicodeSet(b).removeAll(a)};
    ...
    private static <T> void showmap(
            PrintWriter printWriter,
            UnicodeSet inclusion,
            UnicodeMap<T> unicodeMap,
            UnicodeMap<T> literalMap) {
    ...
    private static void showSet(String value, UnicodeSet us) {
        Tabber tabber =
                new Tabber.MonoTabber().add(19, Tabber.LEFT).add(value.length() + 1, Tabber.LEFT);
