...
public class UnicodeUtilities {
    ...
    public static UnicodeMap<String> getIdnaDifferences(UnicodeSet remapped, UnicodeSet skip) {
        ...
        final UnicodeSet valid2008 = GenerateIdnaTest.getIdna2008Valid();

        final VersionInfo empty = VersionInfo.getInstance(0);
        ...
        for (int i = 0; i <= 0x10FFFF; ++i) {
            ...
            final boolean isNew = birth.compareTo(VersionInfo.UNICODE_3_2) > 0 || birth.equals(empty);
            final String age = isNew ? ">3.2!!!" : "v3.2";
            final IdnaType idna2003 = Idna2003.getIDNA2003Type(i);
            final IdnaType tr46 = Uts46.SINGLETON.getType(i);
            if (isNew) {// skip
            ...
            final IdnaType idna2008 = valid2008.contains(i) ? IdnaType.valid : IdnaType.disallowed;
            final String iClass = age
    ...
    static String getXStringPropertyValue(int propertyEnum, int codepoint, int nameChoice, Normalizer.Mode compat) {
        ...
        final String s = Common.MyNormalize(codepoint, compat);
        int cp;
        String lastPart = null;
        for (int i = 0; i < s.length(); i += UTF16.getCharCount(cp)) {
            cp = UTF16.charAt(s, i);
            final String part = Common.getXStringPropertyValue(propertyEnum, cp, nameChoice);
            if (lastPart == null) {
                lastPart = part;
    ...
    static int getXPropertyEnum(String propertyAlias) {
        final int extra = Common.XPROPERTY_NAMES.indexOf(propertyAlias
    ...
    public static void showSet(String grouping, UnicodeSet a, CodePointShower codePointShower, Appendable out) throws IOException {
        ...
        final String[] props = grouping.split("[;,\\s]\\s*");
        ...
        final boolean getShortest = false;
        final UnicodeProperty[] properties = new UnicodeProperty[length];
        ...
        final UnicodeMap<String> map = new UnicodeMap<String>();
        final StringBuilder builder = new StringBuilder();
        for (final UnicodeSetIterator it = new UnicodeSetIterator(a); it.next();) {
            final int s = it.codepoint;
            if (s == UnicodeSetIterator.IS_STRING) {
                final String ss = it.string;
                builder.setLength(0);
                for (int i = 0; i < length; ++i) {
                    if (i != 0) {
                        builder.append("; ");
        ...
        final TreeSet<String> sorted = new TreeSet<String>(Collator.getInstance(ULocale.ENGLISH));
        sorted.addAll(map.values());
        final String[] propsOld = new String[length];
        for (int i = 0; i < propsOld.length; ++i) {
            propsOld[i] = "";
        }
        int lastLevel = -1;
        for (final String s : sorted) {
            final String[] props2 = s.split("; ");
            final int level = getFirstDiff(propsOld, props2);
            ...
            lastLevel = level;
            final UnicodeSet items = map.getSet(s);
    ...
    public static void showSet(UnicodeSet inputSetRaw, CodePointShower codePointShower, Appendable out) throws IOException {
        ...
        if (inputSetRaw.getRangeCount() > 10000) {
            ...
            final LinkedHashMap<String,UnicodeSet> items = new LinkedHashMap();
            final String specials = "Unassigned, Private use, or Surrogates";

            final UnicodeSet specialSet = new UnicodeSet(inputSetRaw).retainAll(UnicodeProperty.getSPECIALS());
            final UnicodeSet inputSet = specialSet.size() == 0 ? inputSetRaw : new UnicodeSet(inputSetRaw).removeAll(UnicodeProperty.getSPECIALS());
            if (specialSet.size() != 0) {
                items.put(specials, specialSet);
            }

            for (final UnicodeSetIterator it = new UnicodeSetIterator(inputSet); it.next();) {
                final int s = it.codepoint;
                if (s == UnicodeSetIterator.IS_STRING) {
                    final String newBlock = "Strings";
                    UnicodeSet set = items.get(newBlock);
                    if (set == null) {
                        items.put(newBlock, set = new UnicodeSet());
                    }
                    set.add(it.string);
                } else {
                    final String block = UCharacter.getStringPropertyValue(BLOCK_ENUM, s, UProperty.NameChoice.LONG).replace('_', ' ');
            ...
            for (final String newBlock : items.keySet()) {
                final UnicodeSet set = items.get(newBlock);
                if (codePointShower.doTable) {
                    out.append("<tr><td colSpan='4'>");
                }
    ...
    public static String getIdentifier(String script) {
        ...
        final UnicodeProperty scriptProp = factory.getProperty("sc");
        ...
        if (scriptSet.size() == 0) {
            ...
            String last = null;
            final TreeSet<String> sorted = new TreeSet<String>(col);
        ...
        try {
            final UnicodeSet allowed = new UnicodeSet(scriptSet).retainAll(XIDModifications.allowed);
            final UnicodeSet restricted = new UnicodeSet(scriptSet).removeAll(XIDModifications.allowed);
            ...
            if (restricted.size() == 0) {
                ...
                for (final String reason : XIDModifications.reasons.values()) {
                    final UnicodeSet shard = XIDModifications.reasons.getSet(reason);
                    final UnicodeSet items = new UnicodeSet(restricted).retainAll(shard);
                    if (items.size() != 0) {
    ...
    private static String showCodePoint(String s) {
        final String literal = getLiteral(s);
    ...
    static class CodePointShower {
        ...
        public CodePointShower(boolean abbreviate, boolean ucdFormat, boolean identifierInfo) {
            ...
            this.identifierInfo = doTable = identifierInfo;
        }
        ...
        private void showString(final String string, String separator, Appendable out) throws IOException {
            ...
            if (name == null || name.length() == 0) {
                ...
                final boolean special = name.indexOf('<') >= 0;
            ...
            if (identifierInfo) {
                ...
                final StringBuilder confusableString = displayConfusables(cp);
                if (doTable) {
        ...
        private void showAbbreviated(UnicodeSet a, Appendable out) throws IOException {
            final UnicodeUtilities.CodePointShower codePointShower = this;

            for (final UnicodeSetIterator it = new UnicodeSetIterator(a); it.nextRange();) {
                final int s = it.codepoint;
                if (s == UnicodeSetIterator.IS_STRING) {
                    out.append(UnicodeUtilities.showCodePoint(it.string)).append("<br>\n");
                } else {
                    final int end = it.codepointEnd;
                    if (end == s) {
                        codePointShower.showCodePoint(s, out);
                    } else if (end == s + 1) {
                        codePointShower.showCodePoint(s, out);
    ...
    private static String getHex(int codePoint, boolean ucdFormat) {
        final String hex = com.ibm.icu.impl.Utility.hex(codePoint, 4);
    ...
    public static Transliterator getTransliteratorFromFile(String ID, String file, int direction) {
        try {
            final BufferedReader br = FileUtilities.openFile(UnicodeUtilities.class, file);
    ...
    public static String showTransform(String transform, String sample) {
        ...
        final Map<String, UnicodeSet> mapping = new TreeMap<String,UnicodeSet>(pp.getOrdering());

        for (final UnicodeSetIterator it = new UnicodeSetIterator(set); it.next();) {
            final String s = it.getString();
            final String mapped = trans.transform(s);
            if (!mapped.equals(s)) {
        ...
        final StringBuilder result = new StringBuilder();
        for (final String mapped : mapping.keySet()) {
            final UnicodeSet source = mapping.get(mapped);
    ...
    public static String listTransforms() {
        if (TRANSFORMLIST == null) {
            final StringBuilder result = new StringBuilder();
            final Set<StringPair> pairs = new TreeSet<StringPair>();
            final Set<String> sources = append(new TreeSet<String>(col), Transliterator.getAvailableSources());
            for (final String source : sources) {
                final Set<String> targets = append(new TreeSet<String>(col), Transliterator.getAvailableTargets(source));
                for (final String target : targets) {
                    final Set<String> variants = append(new TreeSet<String>(col), Transliterator.getAvailableVariants(source, target));
    ...
    static class FilteredStringTransform implements StringTransform {
        ...
        public String transform(String source) {
            ...
            while (start < source.length()) {
                final int end = toExclude.findIn(source, start, false);
    ...
    public static void getDifferences(String setA, String setB,
        ...
        final String setAr = toHTML.transliterate(UtfParameters.fixQuery(setA));
        final String setBr = toHTML.transliterate(UtfParameters.fixQuery(setB));
        ...
        final String[] aMessage = new String[1];
        final String[] bMessage = new String[1];

        final UnicodeSet a = UnicodeUtilities.parseSimpleSet(setA, aMessage);
        final UnicodeSet b = UnicodeUtilities.parseSimpleSet(setB, bMessage);
    ...
    public static void showProperties(int cp, Appendable out) throws IOException {
        final String text = UTF16.valueOf(cp);
        ...
        final boolean allowed = XIDModifications.allowed.contains(cp);
        ...
        final String hex = com.ibm.icu.impl.Utility.hex(cp, 4);
        ...
        for (final String propName : sortedProps) {
            final UnicodeProperty prop = factory.getProperty(propName);
            if (prop.getName().equals("confusable")) {
                continue;
            }

            final boolean isDefault = prop.isDefault(cp);
            if (isDefault) {
                continue;
            }
    ...
    private static StringBuilder displayConfusables(int codepoint) {
        final StringBuilder confusableString = new StringBuilder();
        final Set<String> skip = new HashSet<String>();
        final String same = UTF16.valueOf(codepoint);
        final String nfd = Normalizer.normalize(same, Normalizer.NFD);

        skip.add(same);
        skip.add(nfd);

        // get basic confusables
        final Set<String> list = Confusables.getEquivalents(same);
        if (list != null) {
            for (final String s: list) {
                ...
                skip.add(s);
                final String nfd2 = Normalizer.normalize(same, Normalizer.NFD);
                skip.add(nfd2);
            }
        }
        ...
        if (UTF16.countCodePoint(nfd) > 1) {
            ...
            final List<Confusables> combos = new ArrayList<Confusables>();
            ...
            for (int i = 0; i < nfd.length(); i += Character.charCount(cp)) {
                ...
                final Confusables currentCombos = new Confusables(UTF16.valueOf(cp)).setNormalizationCheck(Normalizer.NFKC);
                combos.add(currentCombos);
                confusableString.append("<div class='char'>");
                for (final String s : currentCombos) {
                    getBoxedCharacters(s, confusableString);
                }
        ...
        final Confusables confusables = new Confusables(same).setNormalizationCheck(Normalizer.NFKC);
        for (final String s: confusables) {
            if (skip.contains(s)) {
                continue;
            }
            final String nfd2 = Normalizer.normalize(same, Normalizer.NFD);
    ...
    private static void addToSkip(String prefix, int i, List<Confusables> combos, Set<String> skip) {
        if (i >= combos.size()) {
            ...
            for (final String s : combos.get(i)) {
    ...
    private static void showPropertyValue(String propName, String propValue, boolean isDefault, Appendable out) throws IOException {
        final String defaultClass = isDefault ? " class='default'" : "";
        if (propValue == null) {
    ...
    public static void showPropsTable(Appendable out, String propForValues, String myLink) throws IOException {
        ((RuleBasedCollator)col).setNumericCollation(true);
        final Map<String, Map<String, String>> alpha = new TreeMap<String, Map<String, String>>(col);
        final Map<String, String> longToShort = new HashMap<String, String>();

        final Set<String> showLink = new HashSet<String>();
        ...
        for (final R4<String, String, String, String> propData : PropertyMetadata.CategoryDatatypeSourceProperty) {
            final String propName = propData.get3();
            final UnicodeProperty prop = factory.getProperty(propName);
            if (prop == null) {
                continue;
            }
            final String propHtml = toHTML.transform(propName);
            String shortName;
            try {
            ...
            final String title = shortName == null || shortName.equals(propName) ? "" : " title='" + toHTML(shortName) + "'";
    ...
    private static void getHtmlPropValues(UnicodeProperty prop, String propHtml, StringBuilder propValues) {
        final List<String> availableValues = prop.getAvailableValues();
        final TreeSet<String> sortedList = Builder.with(new TreeSet<String>(col)).addAll(availableValues).get();
        int count = 500;
        int lastFirstChar = 0;
        for (final String valueName : sortedList) {
            if (--count < 0) {
                propValues.append("\n<i>too many values to show</i>");
                break;
            }
            final int firstChar = valueName.codePointAt(0);
            if (lastFirstChar != 0) {
            ...
            final String valueHtml = toHTML.transform(valueName);
            final String shortValue = prop.getFirstValueAlias(valueName);
        ...
        final String propExp =
                propValue == "T" ? propName
                        : propValue == "F" ? "^" + propName
                                : propName + "=" + propValue;
        final String title = shortName == null ? "" : " title='" + toHTML(shortName) + "'";
    ...
    static void addCell(StringBuilder resultLines, Transliterator hex, String tr46, String attributes, String confusableChoice) {
        if (tr46 == null) {
            ...
            final String escaped = showEscaped(tr46);
    ...
    private static String showEscaped(String line) {
        ...
        final String escaped = ESCAPER.transform(line);
    ...
    public static String showBidi(String str, int baseDirection, boolean asciiHack) {
        ...
        final BidiCharMap bidiCharMap = new BidiCharMap(asciiHack);
    ...
    private static String getList(final UnicodeSet uset) {
        final StringBuffer codePointString = new StringBuffer();
    ...
    private static void showBidiLine(String str, int baseDirection, PrintWriter writer, BidiCharMap bidiCharMap) {
        final byte[] codes = new byte[str.length()];
        ...
        final BidiReference bidi = new BidiReference(codes, (byte)baseDirection);
        ...
        for (int i = 0; i < str.length(); ++i) {
            ...
            final String title = toHTML.transform(getName(s, "", true));
        ...
        for (int k = 0; k < str.length(); ++k) {
            ...
            final String td = bidiChar.length() == 0 ? "<td class='bxcell'>" : "<td class='bcell'>";
        ...
        for (int k = 0; k < str.length(); ++k) {
            final int i = reorder[k];
            final String bidiChar = getBidiChar(str, i, codes[i]);
            final String title = bidiChar.length() == 0 ? "deleted" : toHTML.transform(getName(bidiChar, "", true));
            final String td = bidiChar.length() == 0 ? "bxcell" : "bccell";
    ...
    public static String testIdnaLines(String lines, String filter) {
        ...
        try {
            ...
            final StringBuilder resultLines = new StringBuilder();
            //UnicodeUtilities.getIdna2008Tester();

            final Predicate<String> verifier2008 = new Predicate<String>() {
                @Override
            ...
            boolean first = true;
            final boolean[] errorOut = new boolean[1];

            for (final String line : lines.split("\\s+")) {
                if (first) {
                    first = false;
                } else {
                ...
                final String rawPunycode = UnicodeUtilities.processLabels(line, IdnaTypes.DOTS, true, new Predicate() {
                    @Override
                ...
                final String idna2003unic = Idna2003.SINGLETON.toUnicode(line, errorOut, true);
                addCell(resultLines, hex, idna2003unic, getIdnaClass("cn i2003", errorOut[0]), "IDNA2003");

                final String uts46unic = Uts46.SINGLETON.toUnicode(line, errorOut, true);
                addCell(resultLines, hex, uts46unic, getIdnaClass("cn i46", errorOut[0]), "UTS46%2BUTS39");

                final String idna2008unic = UnicodeUtilities.processLabels(line, IdnaTypes.DOT, false, verifier2008);
                ...
                final String idna2003puny = Idna2003.SINGLETON.toPunyCode(line, errorOut);
                addCell(resultLines, hex, idna2003puny, getIdnaClass("cn mono i2003", errorOut[0]), null);

                final String uts46puny = Uts46.SINGLETON.toPunyCode(line, errorOut);
                addCell(resultLines, hex, uts46puny, getIdnaClass("cn mono i46", errorOut[0]), null);

                final String idna2008puny = UnicodeUtilities.processLabels(line, IdnaTypes.DOT, true, verifier2008);
    ...
    static String processLabels(String inputLabels, Pattern dotPattern, boolean punycode, Predicate<String> verifier) {
        final StringBuilder result = new StringBuilder();
        for (final String label : dotPattern.split(inputLabels)) {
            if (result.length() != 0) {
                result.append('.');
            }
            try {
                ...
                if (!punycode || IdnaTypes.ASCII.containsAll(label)) {
                    ...
                    final StringBuffer puny = Punycode.encode(new StringBuffer(label), null);
                    if (puny.length() == 0) {
                        throw new IllegalArgumentException();
                    }
