...
public class XPropertyFactory extends UnicodeProperty.Factory {
    ...
    {
        final ICUPropertyFactory base = ICUPropertyFactory.make();
        for (final String propertyAlias : (List<String>)base.getInternalAvailablePropertyAliases(new ArrayList())) {
            add(base.getProperty(propertyAlias));
        }
        for (int i = Common.XSTRING_START; i < Common.XSTRING_LIMIT; ++i) {
            final XUnicodeProperty property = new XUnicodeProperty(i);
            add(property);
        }
        ...
        add(new StringTransformProperty(new StringTransform() {
            @Override
            public String transform(String source) {
                final String result = NFM.nfm.get(source);
        ...
        final UnicodeProperty scriptProp = base.getProperty("sc");
        final UnicodeMap<String> specialMap = new UnicodeMap<String>();
        ...
        if (DEBUG_CHARSET_NAMES) {
            System.out.println(charsets.keySet());
        }
        final Matcher charsetMatcher = Pattern.compile("ISO-8859-\\d*|GB2312|Shift_JIS|GBK|Big5|EUC-KR").matcher("");
        for (final String name : charsets.keySet()) {
            if (!charsetMatcher.reset(name).matches()) {
                continue;
            }
            final Charset charset = charsets.get(name);
            final EncodingProperty prop = new EncodingProperty(charset);
            prop._setType(UnicodeProperty.STRING);
            prop._setName("enc_" + name);

            final EncodingPropertyBoolean isProp = new EncodingPropertyBoolean(charset);
    ...
    private void addCollationProperty() {
        final RuleBasedCollator c = (RuleBasedCollator) Collator.getInstance(ULocale.ROOT);

        final UnicodeMap<String> collationMap = new UnicodeMap<String>();
        final UnicodeMap<String> collationMap2 = new UnicodeMap<String>();
        final RawCollationKey key = new RawCollationKey();
        ...
        final UnicodeSet stuff = new UnicodeSet(ALL).addAll(contractions).addAll(expansions);
        for (final String s : stuff) {
            ...
            for (int i = 0; i < key.size; ++i) {
                ...
                final String hex = com.ibm.icu.impl.Utility.hex(0xFF&b, 2);
                // look at both
    ...
    private static class Usage extends XEnumUnicodeProperty {
        ...
        public static UsageValues getValue(int codepoint) {
            if (UnicodeProperty.getSPECIALS().contains(codepoint)) {
                return na;
            }
            if (UnicodeUtilities.DEPRECATED.contains(codepoint)) {
                return deprecated;
            }
            if (UnicodeUtilities.LITURGICAL.contains(codepoint)) {
                return liturgical;
            }
            if (ScriptCategoriesCopy.ARCHAIC.contains(codepoint)) {
                return historic;
            }
            //if (UnicodeUtilities.LIM.contains(codepoint)) return archaic;
            if (UnicodeUtilities.COMMON_USE_SCRIPTS.contains(codepoint)) {
                if (UnicodeUtilities.SYMBOL.contains(codepoint)) {
                    return symbol;
                }
                if (UnicodeUtilities.PUNCTUATION.contains(codepoint)) {
                    return punctuation;
                }
                return common;
            }
    ...
    public static class EncodingProperty extends SimpleProperty {
        ...
        @Override
        protected String _getValue(int codepoint) {
            final int len = encoder.getValue(codepoint, temp, 0);
            if (len < 0) {
                return ERROR;
            }
        ...
        private Object hex(byte b) {
            final String result = Integer.toHexString(0xFF&b).toUpperCase(Locale.ENGLISH);
