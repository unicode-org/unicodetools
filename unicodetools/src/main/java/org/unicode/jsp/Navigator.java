...
final public class Navigator {
    ...
    public static void main(String[] args) {
        final double latitude1 = 0; // toRadians(37.0, 28.0, 48.0, false); // N = +
        final double longitude1 = 0; // toRadians(180-122.0, 8.0, 39.0, true); // W = -
        final Tester tester = new Tester(latitude1, longitude1);
        ...
        for (final double[] test : tests) {
            tester.testItem(0, test[0], test[1]);
        }
        // exhaustive
    ...
    private static class Tester {
        ...
        private void test(double lat0, double lon0, double latMin, double latMax, double lonMin, double lonMax, double inc) {
            for (double dLat = latMin; dLat <= latMax; dLat += inc) {
                for (double dLon = lonMin; dLon <= lonMax; dLon += inc) {
                    counter++;
                    final double lat2a = lat0 + dLat*DEGREE;
                    final double lon2a = lon0 + dLon*DEGREE;
        ...
        boolean testItem(int counter, double lat2a, double lon2a) {
            ...
            lat2a = a.getLat2();
            lon2a = a.getLon2();
            final double distance2 = a.getDistance();
            final double course2 = a.getCourse();
            b.setDistanceCourse(distance2, course2);
            final double lat2b = b.getLat2();
            final double lon2b = b.getLon2();
            final boolean success = areClose(lat2b, lat2a) && areClose(lon2a, lon2b);
    ...
    public Navigator setLat2Lon2(double lat2, double lon2) {
        ...
        // compute distance
        final double halfLatDiff = Math.sin((lat1-lat2)/2);
        final double halfLonDiff = Math.sin((lon1-lon2)/2);
    ...
    public static double wrap(double aa, double low, double high) {
        final double a = aa - low;
        final double span = high - low;
        if (a >= 0 && a < span) {
            return aa;
        }
        final double intQuotient = Math.floor(a / span);
