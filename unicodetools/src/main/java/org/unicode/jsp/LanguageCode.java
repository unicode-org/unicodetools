...
public class LanguageCode {
    ...
    public static String validate(String input, ULocale ulocale) {
        ...
        final Matcher m = languageID.matcher(input);
        if (!m.matches()) {
            int i = input.length();
            for (; ; --i) {
                ...
                if(i == 0 || m.hitEnd()) {
                    final int posBefore = input.lastIndexOf('-', i-1) + 1;
        ...
        final StringBuilder builder = new StringBuilder().append("<table>\n").append(getLine("th", "Type", "2.1", "Code", "Name", "Replacement?"));

        String languageCode = Subtag.language.get(m);
        if (languageCode != null) {
            String languageAndLink = languageCode = languageCode.toLowerCase(Locale.ENGLISH);
            final String originalCode = languageCode;
            String fixed;
            String languageName;
            final String[] parts = languageCode.split("[-_]");
            if (parts.length == 1) {
                final boolean invalidLanguageCode = !names.containsKey(languageCode);
                ...
                final String extlang = parts[1];
                final String extLangName = names.get(extlang);
                final boolean invalidLanguageCode = !names.containsKey(languageCode);
        ...
        if (script != null) {
            final String scriptCode = script = UCharacter.toTitleCase(Locale.ENGLISH, script, null);
            String scriptName;
        ...
        if (region != null) {
            final String regionCode = region = region.toUpperCase(Locale.ENGLISH);
            String regionName;
        ...
        if (variantList != null) {
            ...
            final Set<String> variants = new TreeSet<String>(Arrays.asList(variantList.split("[-_]")));
        ...
        if (extensionList != null) {
            ...
            final Matcher m2 = extensionID.matcher(extensionList);
            final Set<String> extensions = new TreeSet<String>();
            while (m2.find()) {
                final String extension = m2.group();
        ...
        final String canonicalString = canonical.toString();
    ...
    private static void addFixed(StringBuilder canonical, String code, String fixed) {
        ...
        final int spacePos = fixed.indexOf(' ');
        if (spacePos >= 0) {
    ...
    private static String getCodeAndLink(Subtag subtag, String codes, ULocale ulocale) {
        ...
        for (final String code : codes.split("\\s+")) {
            final String value = getCodeAndLink2(subtag, code, ulocale);
            if (buffer.length() != 0) {
                buffer.append(" ");
            }
            buffer.append(value);
    ...
    public static String getLanguageOptions(ULocale toLocalizeInto) {
        ...
        final ULocale[] list = ULocale.getAvailableLocales();
        final Map<String, String> sorted = new TreeMap<String, String>(Collator.getInstance(toLocalizeInto));
        for (final ULocale ulocale : list) {
            ...
            final String name = getLocaleName(ulocale, toLocalizeInto);
            sorted.put(name, ulocale.toString());
        }
        for (final String name : sorted.keySet()) {
            final String code = sorted.get(name).toString();
            final String selected = code.equals(toLocalizeInto.toString()) ? " selected" : "";
    ...
    public static String getLocaleName(ULocale toBeLocalized, ULocale toLocalizeInto) {
        ...
        final String test = toBeLocalized.getDisplayName(ULocale.ROOT);
        final String englishName = toBeLocalized.getDisplayName(ULocale.ENGLISH);
