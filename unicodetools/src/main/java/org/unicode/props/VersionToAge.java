...
public enum VersionToAge {
    ...
    ucd(
            ImmutableMap.<VersionInfo, Long>builder()
                    ...
                    .put(VersionInfo.getInstance(10, 0), getDate(2017, 6))
                    .put(VersionInfo.getInstance(9, 0), getDate(2016, 6))
                    .put(VersionInfo.getInstance(8, 0), getDate(2015, 6))
    ...
    public VersionInfo getVersionInfo(long requestedDate) {
        ...
        for (Entry<VersionInfo, Long> entry : versionToDate.entrySet()) {
            ...
            if (entry.getValue() <= requestedDate && entry.getKey().compareTo(best) > 0) {
                best = entry.getKey();
            }
        }
        return best;
    ...
    private static void show(VersionToAge v2a) {
        for (VersionInfo version : v2a.getVersions()) {
            ...
            System.out.println(
                    v2a + "\t" + version + "\t" + year + "\t" + v2a.getVersionInfoForYear(year));
        }
    }
}
