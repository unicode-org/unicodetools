...
public class BagFormatter {
    ...
    public void showSetDifferences(
            PrintWriter pw, String name1, Collection set1, String name2, Collection set2) {
    ...
    /**
     * Returns a list of items in the collection, with each separated by the separator. Each item
     * must not be null; its toString() is called for a printable representation
     *
    ...
    public String getAbbreviatedName(String src, String pattern, String substitute) {

        int matchEnd = NameIterator.findMatchingEnd(src, pattern);
        int sdiv = src.length() - matchEnd;
        int pdiv = pattern.length() - matchEnd;
        StringBuffer result = new StringBuffer();
        addMatching(src.substring(0, sdiv), pattern.substring(0, pdiv), substitute, result);
        addMatching(src.substring(sdiv), pattern.substring(pdiv), substitute, result);
        return result.toString();
    }

    public abstract static class Relation {
        public abstract String getRelation(String a, String b);
    }
    ...
    */
    ...
    public static class NameLabel extends UnicodeLabel {
        ...
        public String getValue(int codePoint, boolean isShort) {
            String hcp = !isShort ? "U+" + Utility.hex(codePoint, 4) + " " : "";
            String result = nameProp.getValue(codePoint);
            if (result != null) return hcp + result;
    ...
    private void addMatching(String src, String pattern, String substitute, StringBuffer result) {
        ...
        while (true) {
            ...
            first = false;
            if (s1.equals(s2)) result.append(substitute);
    ...
    private class MyVisitor extends Visitor {
        ...
        public void toOutput(String s) {
            ...
            output.print(s);
            if (isHtml) output.println("</p>");
            else output.print(lineSeparator);
        }
        ...
        public void doAt(Object c, PrintWriter out) {
            ...
            valueSize =
                    maxWidthOverride > 0
                            ? maxWidthOverride
                            : getValueSource().getMaxWidth(shortValue);
        ...
        protected void doSimpleAt(Object o) {
            if (o instanceof Map.Entry) {
                ...
                String label =
                        getLabelSource(true) == UnicodeLabel.NULL
                                ? ""
                                : getLabelSource(true).getValue(thing, ",", true);
    ...
    private class RangeFinder {
        ...
        boolean next() {
            if (limit >= veryLimit) return false;
            ...
            for (; limit < veryLimit; limit++) {
                ...
                if (!equalTo(s, label) || !equalTo(v, value) || !equalTo(b, breaker)) {
                    break;
                }
            }
    ...
    public static final Transliterator hex =
            Transliterator.getInstance("[^\\u0009\\u0020-\\u007E\\u00A0-\\u00FF] hex");
