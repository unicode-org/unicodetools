...
/**
 * A command-line option. A UOption specifies the name of an option and whether or not it takes an
 * argument. It is a mutable object that later contains the option argument, if any, and a boolean
 * flag stating whether the option was seen or not.
 *
 * <p>The static method parseArgs() takes an array of command-line arguments and an array of
 * UOptions and parses the command-line arguments.
 *
 * <p>This deliberately resembles the icu4c file uoption.[ch].
 */
public class UOption {
     ...
     *
     * <p>This function takes the argv[] command line and a description of the program's options in
     * form of an array of UOption structures. Each UOption defines a long and a short name (a
     * string and a character) for options like "--foo" and "-f".
     *
     * <p>Each option is marked with whether it does not take an argument, requires one, or
     * optionally takes one. The argument may follow in the same argv[] entry for short options, or
     * it may always follow in the next argv[] entry.
     *
     * <p>An argument is in the next argv[] entry for both long and short name options, except it is
     * taken from directly behind the short name in its own argv[] entry if there are characters
     * following the option letter. An argument in its own argv[] entry must not begin with a '-'
     * unless it is only the '-' itself. There is no restriction of the argument format if it is
     * part of the short name options's argv[] entry.
     *
     * <p>The argument is stored in the value field of the corresponding UOption entry, and the
     * doesOccur field is set to 1 if the option is found at all.
     *
     * <p>Short name options without arguments can be collapsed into a single argv[] entry. After an
     * option letter takes an argument, following letters will be taken as its argument.
     *
     * <p>If the same option is found several times, then the last argument value will be stored in
     * the value field.
     *
     * <p>For each option, a function can be called. This could be used for options that occur
     * multiple times and all arguments are to be collected.
     *
     * <p>All options are removed from the argv[] array itself. If the parser is successful, then it
     * returns the number of remaining non-option strings. (Unlike C, the Java argv[] array does NOT
     * contain the program name in argv[0].)
     *
     * <p>An option "--" ends option processing; everything after this remains in the argv[] array.
     *
     * <p>An option string "-" alone is treated as a non-option.
     *
     * <p>If an option is not recognized or an argument missing, then the parser returns with the
     * negative index of the argv[] entry where the error was detected.
     *
     * @param argv this parameter is modified
     * @param start the first argument in argv[] to examine. Must be 0..argv.length-1. Arguments
     *     from 0..start-1 are ignored.
     * @param options this parameter is modified
     * @return the number of unprocessed arguments in argv[], including arguments 0..start-1.
     */
    public static int parseArgs(String argv[], int start, UOption options[]) {
        ...
        while (i < argv.length) {
            ...
            if (!stopOptions && arg.length() > 1 && arg.charAt(0) == '-') {
                ...
                if (option != null
                        && option.optionFn != null
                        && option.optionFn.handle(option) < 0) {
