...
public class MessageFormatCheck extends TestFmwkMinusMinus {
    ...
    public static void init() throws Exception {
        format =
                new MessageFormat(
                        "The directory {directory} contains {file_count,number,#,##0.0}KB.");
        args = new HashMap();
    ...
    public void TestDuration() {
        ...
        for (final long duration :
                new Long[] {2L, 2 * SECOND, 2 * MINUTE, 2 * HOUR, 2 * DAY, 2 * MONTH, 2 * YEAR}) {
            System.out.println();
            System.out.println(
                    "formatDurationFrom: " + durationFormat.formatDurationFrom(duration, now));
            System.out.println(
                    "formatDurationFromNowTo: "
                            + durationFormat.formatDurationFromNowTo(new Date(now + duration)));
            System.out.println(
                    "formatDurationFromNow: " + durationFormat.formatDurationFromNow(duration));
    ...
    public void TestSettingFormats() {
        final MessageFormat format2 =
                new MessageFormat("The number {0} is formatted as {0,number,#,##0.0}.");
        assertEquals(
                "Resetting format",
                "The number 1.234 is formatted as 1.2.",
                format2.format(new Object[] {new Double(1.2345)}));
    }
    ...
    public void TestNamedArguments() {
        ...
        format.setFormatByArgumentName(
                "file_count", NumberFormat.getIntegerInstance(ULocale.FRENCH));
        assertEquals(
                "Resetting format", "The directory foo/bar/ contains 3KB.", format.format(args));
    }
    ...
    public void TestNicerSyntax() {
        final MessageFormat format2 =
                new MessageFormat("The number {0} is formatted as {0,number,#,##0.0} with {1}.");
        assertEquals(
                "Resetting format",
                "The number 1.234 is formatted as 1.2.",
