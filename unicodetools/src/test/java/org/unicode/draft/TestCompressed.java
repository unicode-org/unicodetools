...
public class TestCompressed extends TestFmwkMinusMinus {
    ...
    public void TestSetStreamer() throws IOException {
        final String[] tests = {
            "[:Ll:]", "[a-c]", "[]", "[a-ce-fq-z]", "[{abc}]", "[a-cmq-z{abc}]"
        };
    ...
    public void TestMapStreamer() throws IOException {
        final String[] tests = {
            "[a-c]", "[]", "[a-ce-fq-zA-BD-F]", "[{abc}]", "[a-cmq-z{abc}{huh?}]"
        };
    ...
    private void verifyMap(
            ...
            UnicodeDataOutput unicodeDataOutput,
            UnicodeDataInput unicodeDataInput,
            String test,
            UnicodeMap<String> item)
            throws IOException {
        int totalLocal = 0;
        for (final boolean compressed : new boolean[] {false, true}) {
            final ByteArrayOutputStream outBytes = new ByteArrayOutputStream();
            ...
            final ByteArrayInputStream inBytes = new ByteArrayInputStream(bytes);
            final DataInputStream in = new DataInputStream(inBytes);
            final UnicodeMap<String> newItem =
                    unicodeDataInput.set(in, compressed).readUnicodeMap(stringReader);
            checkAtEnd("Stream not read completely: " + test, in);
            in.close();
            final String comp =
                    compressed ? "\t" + percent.format(bytes.length / (double) totalLocal - 1) : "";
        ...
        if (item.equals(newItem)) {
            logln(
                    item
                            ...
                            + "\t"
                            + percent.format(bytes.length / (double) oldBytes.length - 1)
                            + "\t"
                            + showBytes(bytes, 20)
                            + "\tOld\t"
                            + showBytes(oldBytes, 20));
        } else {
            errln(
                    "No round trip "
                            + "\t"
    ...
    public void TestUnicodeProperties() throws IOException {
        ...
        for (final Entry<String, UnicodeMap<String>> propertyPair : cache.entrySet()) {
            ...
            verifyMap(
                    stringReader, stringWriter, unicodeDataOutput, unicodeDataInput, property, map);
        }
        logln("Total:\t" + total);
        logln(
                "Total-Compressed:\t"
                        + nf.format(totalCompressed)
                        + "\t"
                        + percent.format(totalCompressed / (double) total - 1));
    }
}
