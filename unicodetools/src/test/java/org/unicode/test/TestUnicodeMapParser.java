...
public class TestUnicodeMapParser extends TestFmwkMinusMinus {
    ...
    private static final IndexUnicodeProperties INDEX_PROPS =
            IndexUnicodeProperties.make(Settings.latestVersion);
    private static final IndexUnicodeProperties LAST_INDEX_PROPS =
    ...
    private UnicodeMapParser<String> ump =
            UnicodeMapParser.create(
                    UnicodeMapParser.STRING_VALUE_PARSER, INDEX_PROPS, LAST_INDEX_PROPS);

    @Test
    public void testBasic() {
        UnicodeMap<String> expected =
    ...
    public void testUnihan() {
        UnicodeMapParser<String> ump =
                UnicodeMapParser.create(UnicodeMapParser.STRING_VALUE_PARSER, INDEX_PROPS);
        UnicodeMap<String> expected =
                new UnicodeMap<String>()
    ...
    private <V> void check(
            UnicodeMapParser<V> ump, String test, UnicodeMap<V> expected, int expectedError) {
    ...
    private <V> void areEqual(UnicodeMapParser<V> ump, String test1, String test2) {
        ...
        if (!um1.equals(um2)) {
            ...
            //            final UnicodeMap um1MinusUm2 = UnicodeMapParser.removeAll(new
            // UnicodeMap().putAll(um1), um2);
            //            errln("In " + test1 + " but not " + test2 + "\n\t"
            //            + um1MinusUm2.toString());
            //            errln("In " + test1 + " and in " + test2 + "\n\t"
            //                    + UnicodeMapParser.retainAll(new UnicodeMap().putAll(um1),
            // um2).toString());
            //            errln("Not in " + test1 + " but in " + test2 + "\n\t"
            //                    + UnicodeMapParser.removeAll(new UnicodeMap().putAll(um2),
            // um1).toString());
        }
    }
