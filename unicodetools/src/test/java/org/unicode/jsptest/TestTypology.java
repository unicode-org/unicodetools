...
public class TestTypology extends TestFmwk {
    ...
    public void TestSimple() throws IOException {
        ...
        final UnicodeSet archaic = new UnicodeSet();
        //        System.out.println();
        ...
        for (final String label : list) {
            final UnicodeSet uset = Typology.label_to_uset.get(label);
            final UnicodeSet[] usets = { uset };
        ...
        for (final String label : list) {
            final Map<String, UnicodeSet> lists = Typology.label_parent_uset.get(label);
            if (lists.size() < 2) {
                continue;
            }
            String oldLabel = "";
            for (final Entry<String, UnicodeSet> parentAndSet : lists.entrySet()) {
                final String parent = parentAndSet.getKey();
                final UnicodeSet uset = parentAndSet.getValue();
        ...
        final Set<PropData> props = getProps();
        final Set<R3<Double, String, TreeSet<Row.R2<Double, PropData>>>> matches = new TreeSet<R3<Double, String, TreeSet<Row.R2<Double, PropData>>>>();

        for (final String label : list) {
            final UnicodeSet uset = Typology.label_to_uset.get(label);
            final TreeSet<Row.R2<Double, PropData>> close = new TreeSet<R2<Double, PropData>>();
            for (final PropData item : props) {
                final double closeness = getCloseness(item.getSet(), uset);
                if (closeness == 0d) {
                    continue;
                }
                if (close.size() > 4) {
                    final R2<Double, PropData> lowest = close.iterator().next();
            ...
            for (final R2<Double, PropData> row : close) {
                closestValue = row.get0();
            }

            final R3<Double, String, TreeSet<Row.R2<Double, PropData>>> match = Row.of(closestValue, label, close);
            matches.add(match);
        }
        ...
        for (final R3<Double, String, TreeSet<Row.R2<Double, PropData>>> match : matches) {
            final String label = match.get1();
            final UnicodeSet uset = Typology.label_to_uset.get(label);
            boolean first = true;
            for (final R2<Double, PropData> matchData : InverseIterator.of(match.get2())) {
                final double closeness = matchData.get0();
                if (closeness < 0.75 && !first) {
                    break;
                }
                final PropData propData = matchData.get1();
                final UnicodeSet propSet = propData.getSet();
                final UnicodeSet label_AND_propset = new UnicodeSet(uset).retainAll(propSet);
                final UnicodeSet label_propset = new UnicodeSet(uset).removeAll(propSet);
                final UnicodeSet propset_label = new UnicodeSet(propSet).removeAll(uset);
    ...
    private double getCloseness(UnicodeSet get2, UnicodeSet uset) {
        ...
        final UnicodeSet intersect = new UnicodeSet(get2).retainAll(uset);
    ...
    private Set<PropData> getProps() {
        final Set<PropData> props = new HashSet<PropData>();
        final int[][] ranges = {
        ...
        final UnicodeSet skip = new UnicodeSet("[[:cn:][:cs:][:co:]]");
        for (final int[] range : ranges) {
            for (int propEnum  = range[0]; propEnum < range[1]; ++propEnum) {
                final String alias = UCharacter.getPropertyName(propEnum, UProperty.NameChoice.LONG);
                ...
                for (int valueEnum = UCharacter.getIntPropertyMinValue(propEnum); valueEnum <= max; ++valueEnum) {
                    try {
                        final UnicodeSet foo = new UnicodeSet().applyIntPropertyValue(propEnum, valueEnum);
                        final String valueAlias = UCharacter.getPropertyValueName(propEnum, valueEnum, UProperty.NameChoice.LONG);
        ...
        for (final String subhead : subheader) {
            final UnicodeSet uset = subheader.getUnicodeSet(subhead);
            final String alias = "subhead";
    ...
    private void addProps(Set<PropData> props, UnicodeSet skip, String subhead, UnicodeSet uset, String alias) {
        final UnicodeSet foo = new UnicodeSet(uset);
        foo.removeAll(skip);
        if (foo.size() != 0) {
            final PropData r = new PropData(foo, alias, subhead);
            props.add(r);
            if (DEBUG) {
    ...
    private void showLabel(PrintWriter html, PrintWriter printStream, UnicodeSetPrettyPrinter ppx,
        ...
        if (subhead == LabelStyle.title) {
            ...
            cell = "td";
            final Map<String,Double> subheads = getSubheadInfo(usets[0]);
    ...
    private String formatUnicodeSet(UnicodeSet... usets) {
        String result = "";
        for (final UnicodeSet uset : usets) {
            if (result.length() != 0) {
                result += "\t";
            }
    ...
    private String formatUnicodeSet(UnicodeSet uset) {
        ...
        final String uset2 = uset.toPattern(false);
        final String href = "<a href='http://unicode.org/cldr/utility/list-unicodeset.jsp?a=" + uset2 + "'>";
        return "<span class='b'>" + href + setString + "</a></span>";
    }


    private Map<String,Double> getSubheadInfo(UnicodeSet uset) {
        final Map<String,Double> subheads = new TreeMap<String,Double>();
        for (final String s : uset) {
            ...
            if (subheadString == null) {
                ...
                final UnicodeSet other = subheader.getUnicodeSet(subheadString);
                final UnicodeSet overlap = new UnicodeSet(other).retainAll(uset);
    ...
    public static String join(Map<String,Double> map, String label) {
        final StringBuffer result = new StringBuffer("<b>OL:</b> ");
        final Set<String> lists = Typology.labelToPaths.getAll(label);
        final TreeSet<String> otherLabels = new TreeSet<String>();
        ...
        if (map.size() < 3) {
            ...
            for (final Entry<String, Double> entry : map.entrySet()) {
                ...
                if (first) {
                    ...
                    final char newFirstChar = Character.toLowerCase(item.charAt(0));
    ...
    private static void joinItems(StringBuffer result, String label, TreeSet<String> otherLabels) {
        ...
        boolean first = true;
        for (final String otherLabel : otherLabels) {
            if (first) {
                first = false;
            } else {
                result.append(", ");
            }
            final UnicodeSet other = Typology.label_to_uset.get(otherLabel);
            if (other == null) {
                throw new IllegalArgumentException();
            }
            UnicodeSet overlap;
    ...
    private static void appendWithCoverage(StringBuffer result, String item, Double coverage) {
        final String pc = coverageToClass(coverage);
    ...
    private static String coverageToClass(double coverage) {
        final String pc = coverage >= 0.999d ? "p100"
                : coverage > 0.666d ? "p66"
                        : coverage > 0.333 ? "p33"
                                : "p0";
                        return pc;
