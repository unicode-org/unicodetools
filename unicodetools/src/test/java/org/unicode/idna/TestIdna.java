...
public class TestIdna extends TestFmwkMinusMinus {
    ...
    public void TestExtract() throws IOException {
        ...
        logln(
                "sourceNotAllowed: "
                        + handler.sourceNotAllowed.size()
                        + "\t"
                        + handler.sourceNotAllowed);
        logln(
                "targetNotAllowed: "
                        + handler.targetNotAllowed.size()
                        + "\t"
                        + handler.targetNotAllowed);
        ...
        for (String source : handler.diffIdnaBis.keySet()) {
            ...
            logln(
                    "\t"
                            ...
                            + source
                            + " ) => "
                            + Utility.hex(targetChars)
                            + " ( "
                            + targetChars
                            + " ) # "
                            + UCharacter.getName(source.codePointAt(0)));
        }
    }
