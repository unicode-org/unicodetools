...
/** Parse the t extension, because that isn't yet supported by ICU. Results are immutable. 
 * Internally it is a single map of key-values,
 * with the attributes represented by <u,attributes> and the tlang by <t,tLang> (both attributes and tLang may be ""
 * as may be the values of other keys like kc.
 * */
public class LocaleExtensions {
    public static final Pattern TKEY = Pattern.compile("[a-zA-Z][0-9]");
    public static final Pattern TKEY_SPLITTER = Pattern.compile("(?:^|[-])(?=[a-zA-Z][0-9](?:$|[-]))");

    /** Puts keys in the order <a, ... s, t, a0, a1 ... z9, u, aa, ab, .. zz, v, w, y, z, x>*/
    public static final Comparator<? super String> EXTENSION_ORDER = new Comparator<String>() {
    ...
    static final int 
    a_sBucket = 0, tBucket = 1, tkeyBucket = 2, uBucket = 3, 
    ukeyBucket = 4, vwyzBucket = 5, xBucket = 6;
        ...
        if (extensions != null) {
            ...
            for (String part : tExtensions) {
                if (first & !TKEY.matcher(part).lookingAt()) {
                    ...
                    _tKeyValues.put(keyValue.get(0), LocaleCanonicalizer.SEP_JOIN.join(keyValue.subList(1, keyValue.size())));
                }
            }
        }
    ...
    public void set(Builder builder) {
        ...
        for (Entry<String, String> entry : tKeyValues.entrySet()) {
            ...
            case xBucket:
                ...
                // fall through
            default: builder.setExtension(key.charAt(0), value);
            break;
            }
        } 
    }
