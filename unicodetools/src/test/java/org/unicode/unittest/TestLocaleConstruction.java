...
public class TestLocaleConstruction extends TestFmwkMinusMinus {
    ...
    public void TestLocalesJava() {
        CheckLocaleState jLocaleTester =
                new CheckLocaleState(new JLocaleShim(Locale.class, Locale.Builder.class));
        int lineCount = 0;
    ...
    public void TestLocalesICU() {
        CheckLocaleState uLocaleTester =
                new CheckLocaleState(new JLocaleShim(ULocale.class, ULocale.Builder.class));
        int lineCount = 0;
        for (String line : tests) {
        ...
        public void handleCommand(int lineCount, String commandString) {
            ...
            switch (OperatorType.from(operator)) {
                ...
                case test:
                    ...
                    assertEquals(
                            lineCount + ") " + getBuildArgs(), parameter, shim.getValue(operator));
                    break;
                case compare:
                    if (!tested) {
                        tested = true;
                    ...
                    assertEquals(
                            lineCount + ") " + getBuildArgs(),
                            shim.getStored(parameter),
                            shim.getStored(""));
                    break;
            }
        }
    ...
    enum OperatorType {
        ...
        static OperatorType from(String operator) {
            return "arg".equals(operator)
                            || operator.startsWith("set")
                            || operator.startsWith("add")
                    ? OperatorType.build
                    : operator.equals("store")
                            ? OperatorType.store
                            : operator.equals("equals")
                                    ? OperatorType.compare
                                    : operator.equals("title")
                                            ? OperatorType.title
                                            : OperatorType.test;
        }
    }
                ...
                if (!argValues.isEmpty()) {
                    ...
                    for (Entry<String, Collection<String>> entry : map.entrySet()) {
                        ...
                        switch (key) {
                            case "setExtension":
                                builderClass
                                        .getMethod(key, Character.class, String.class)
    ...
    //    static final LocaleValidityChecker validity = new
    // LocaleValidityChecker(EnumSet.allOf(Datasubtype.class));
    //
    ...
    //    }
    //
    //    public static final Map<Datatype, Map<Datasubtype, ValiditySet>> validityInfo =
    // ValidIdentifiers.getData();
    //
    //    public static void TestAliases() {
    //        LocaleDisplayNames eng = LocaleDisplayNames.getInstance(ULocale.ENGLISH);
    //        Set<String> isoLanguages = new
    // LinkedHashSet<>(Arrays.asList(ULocale.getISOLanguages()));
    ...
    //            String dummy3 = name.length() > 2 ? name.substring(0,3).toLowerCase(Locale.ROOT) :
    // "und";
    ...
    ////                        dataType == Datatype.region || dataType == Datatype.variant?
