...
public class TestLocaleMatching  extends TestFmwkMinusMinus {
    ...
    public void TestBasic() {
        String[][] tests = {
                ...
                //                {"de-CH-u-co-phonebk, sv-u-co-reformed", "en, sv", "sv", "sv-u-co-reformed", "="},
                //                {"de-CH-u-co-phonebk, sv-u-cf-account", "sv, de", "sv", "sv-u-cf-account", "="},
                //                {"en-IN-u-cu-eur, sv", "en, en-GB, fr", "en-GB", "en-IN-u-cu-eur", "="},
                //                //                {"en-GB-u-cu-eur, sv", "en, en-IN", "en", "en-GB-u-cu-eur", "="},
                //                {"es-MX, sv", "es-ES, es-419, fr", "es-419", "es-MX", "="},
                //                {"sr-ME-u-cu-eur, sv", "sr, sr-Latn, fr", "sr-Latn", "sr-ME-u-cu-eur", "="},
                //                {"sr-ME-u-cu-eur, sv", "sr, fr", "sr", "sr-ME-u-cu-eur", "sr-Cyrl-ME-u-cu-eur"},
                // {"preferred locales", "supported locales", "best supported (ui)", "best preferred", "best services"},
        ...
        for (String[] test : tests) {
            ...
            ULocale expected_best_services = ULocale.forLanguageTag(!test[4].trim().equals("=") ? test[4] : test[3]);
...
//            ULocale actual_best_supported = matcher.getBestMatch(preferred_locales, actual_best_preferred);
//
//            String message = item + ") pref: " + preferred_locales + ", supp: " + supported_locales;
//            assertEquals(message + " => best-supp", expected_best_supported, actual_best_supported);
//
//            assertEquals(message + " => best-pref", expected_best_preferred, actual_best_preferred.value);
//
//            ULocale actual_best_services = combine(expected_best_supported, expected_best_preferred);
//            assertEquals(item + ") pref: " + expected_best_preferred + ", supp: " + expected_best_supported + " => best-serv", expected_best_services, actual_best_services);
        }

    }

    /** Combine features of the desired locale and supported locale for the best services locale, and return result. */
    public static ULocale combine(ULocale supported, ULocale desired) {
        ...
        Builder builder = new ULocale.Builder().setLocale(desired);  // gets region, variants, extensions, etc.
        ...
        ULocale maxService = new ULocale.Builder()
                .setLocale(maxSupp)
                .setRegion(desiredCountry)
                .build();
