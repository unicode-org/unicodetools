...
public class TestUnicodeSet extends TestFmwkMinusMinus {
    ...
    public void TestAge() {
        ...
        if (U32.contains(0x0221) || U32.contains(0x0560)) {
        ...
        for (String[] test : tests) {
            ...
            if (test[0].startsWith("[") && parsed != null) {
                ...
                assertEquals(
                        test[1],
                        expectedString,
                        error == null ? parsed.toPattern(false) : error.getMessage());
            }
        }
    }
    ...
    // \p{prop} matches if the input has the property prop, while \P{prop} does not match if the
