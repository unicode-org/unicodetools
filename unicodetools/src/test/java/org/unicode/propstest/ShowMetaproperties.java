...
public class ShowMetaproperties {
    public static void main(String[] args) {
        final Relation<EnumSet<PropertyStatus>, UcdProperty> map =
                Relation.of(
                        new TreeMap<EnumSet<PropertyStatus>, Set<UcdProperty>>(
                                new EnumSetComparator<PropertyStatus>()),
                        TreeSet.class);
        for (final UcdProperty prop : UcdProperty.values()) {
            final EnumSet<PropertyStatus> propertyStatusSet =
                    PropertyStatus.getPropertyStatusSet(prop);
            if (propertyStatusSet.contains(PropertyStatus.Normative)
                            && propertyStatusSet.contains(PropertyStatus.Informative)
                    || propertyStatusSet.contains(PropertyStatus.Normative)
                            && propertyStatusSet.contains(PropertyStatus.Contributory)
                    || propertyStatusSet.contains(PropertyStatus.Informative)
                            && propertyStatusSet.contains(PropertyStatus.Contributory)) {
                throw new IllegalArgumentException(
                        "Conflicting values: " + prop + ",\t" + propertyStatusSet);
            }
            map.put(propertyStatusSet, prop);
            // System.out.println(prop + " ;\t" +
            // IndexUnicodeProperties.getPropertyStatusSet(prop));
        }
        System.out.println(
                "# Table 9 Status+\n"
