...
public class TestImmutableUnicodeMap extends TestFmwkMinusMinus {
    ...
    static final UnicodeMap<Binary> WHITESPACE_PROP =
            iup.loadEnum(UcdProperty.White_Space, Binary.class);
    static final UnicodeMap<Block_Values> BLOCK_PROP =
    ...
    private <T> void checkGetRange(UcdProperty prop, UnicodeMap<T> umap, Type type) {
        ...
        for (EntryRange<T> range : umap.entryRanges()) {
            ...
            if (range.codepoint >= 0) {
                ...
                assertEqualHex(
                        prop + ", " + type + ", Range Start: ",
                        range.codepoint,
                        cpRange.getStart());
                assertEqualHex(
                        prop + ", " + type + ", Range End: ", range.codepointEnd, cpRange.getEnd());
    ...
    private void checkEquivalent(final int expected, final ValueWidth valueWidth) {
        ...
        assertEquals(
                "Check Exception for " + Utility.hex(expected) + " with " + valueWidth,
                expectedException,
                actualException);
    }
    ...
    public void testRanges() {
        ...
        ValueWidth valueWidth =
                UnicodeCPMap.getValueWidth(
                        outputValueMap.size() + 1); // leave room for -1. Would be nice utility
    ...
    private void assertCodePointMapEquals(
            String message, CodePointMap cpMap1, CodePointMap cpMap2) {
        // First by range
        ...
        for (int cp = 0; ok && cp <= 0x01FFFF; cp = cpMapRange1.getEnd() + 1) {
            ...
            ok &=
                    assertEqualHex(
                            message + ": Start: ", cpMapRange1.getStart(), cpMapRange2.getStart());
    ...
    static class UnicodeCPMap<T> {
        ...
        private static ValueWidth getValueWidth(int itemCount) {
            return itemCount < 0x100
                    ? ValueWidth.BITS_8
                    : itemCount < 0x10000 ? ValueWidth.BITS_16 : ValueWidth.BITS_32;
        }

        private static <T> MutableCodePointTrie fromUnicodeMap(
                UnicodeMap<T> source,
                LinkedHashMap<T, Integer> outputValueMap,
                Map<String, T> outputStringData) {
            MutableCodePointTrie builder = new MutableCodePointTrie(-1, -1);
