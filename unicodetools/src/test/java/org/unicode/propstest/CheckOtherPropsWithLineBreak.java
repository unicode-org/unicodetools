...
public class CheckOtherPropsWithLineBreak {
    ...
    public static void main(String[] args) {
        UnicodeMap<Line_Break_Values> lineBreakProperty =
                UnicodeProperties.loadEnum(UcdProperty.Line_Break, Line_Break_Values.class);
        UnicodeSet hebrewScript =
                UnicodeProperties.loadEnum(UcdProperty.Script, Script_Values.class)
                        .getSet(Script_Values.Hebrew);
        ...
        for (Line_Break_Values lineBreakValue : Line_Break_Values.values()) {
            ...
            UnicodeSet difference =
                    new UnicodeSet(lineBreakPropertyValueSet).removeAll(hebrewScript);
            UnicodeSet intersection =
                    new UnicodeSet(lineBreakPropertyValueSet).retainAll(hebrewScript);
            if (difference.isEmpty() || intersection.isEmpty()) {
                addFirstCharacter(
                        lbvName + "\t " + otherPropName, lineBreakPropertyValueSet, testCharacters);
            } else {
    ...
    private static void addFirstCharacter(
            String message, UnicodeSet intersection, UnicodeSet testCharacters) {
        ...
        System.out.println(
                message
                        + "\t"
                        + Utility.hex(testChar)
                        + "\t"
                        + UnicodeProperties.getName(testChar, ", "));
