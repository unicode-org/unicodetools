...
/**
 * Simple tool for checking the stability of a property+value A property value
 * is stable in a version when the only change is when a new character is
 * created. In this listing, those are called "new". The unstable changes are
 * when an existing character changes value: here listed as either "Removed" or
 * "Changed".
 * 
 * <p>The first argument is the property name, and the second is the
 * property value. Aliases work for both. The default value if not supplied is "Yes".
 * 
 * <p>For example, General_Category = Letter_Number (as of V14) was last unstable
 * in V5.0 2006, with the following: 
 * <br>REMOVED: 2183 V3_0 ROMAN NUMERAL REVERSED ONE HUNDRED 
 * <br>CHANGED: 10341 V3_1 GOTHIC LETTER NINETY
 */
public class CheckPropertyStability {
    ...
    static UnicodeMap<General_Category_Values> latestGc = latest.loadEnum(UcdProperty.General_Category,
    ...
    public static void main(String[] args) {
        ...
        UcdProperty prop = args.length < 1 ? UcdProperty.Default_Ignorable_Code_Point : UcdProperty.forString(args[0]);
        if (prop == null) {
        ...
        if (uset.isEmpty()) {
            System.out.println("No current code points have the value " + prop + " = " + propValueRaw);
        ...
        for (Age_Values ucdVersion : Age_Values.values()) {
            ...
            final UnicodeMap<General_Category_Values> gc = iup.loadEnum(UcdProperty.General_Category,
                    General_Category_Values.class);
            ...
            if (lastDi != null) {
                if (!di.equals(lastDi)) {
                    ...
                    UnicodeSet newDi = new UnicodeSet(di).removeAll(lastDi).retainAll(lastUndefined).freeze();
                    UnicodeSet changedDi = new UnicodeSet(di).removeAll(lastDi).removeAll(lastUndefined).freeze();

                    System.out.println(ucdVersion + "\t" + VersionToAge.ucd.getYear(ucdVersion) + "\tnew:\t"
                            + newDi.size() + "\tREMOVED:\t" + removedDi.size() + "\tCHANGED:\t" + changedDi.size());
                    show("new:", newDi);
    ...
    private static void show(String string, UnicodeSet newDi) {
        ...
        for (String s : newDi) {
            System.out.println(
                    "\t\t" + string + "\t" + Utility.hex(s) + "\t" + birth.get(s) + "\t" + latest.getName(s, "+"));
        }
    }

    private static UnicodeSet removeCommon(String string, UnicodeSet newDi, String nameOfSet, UnicodeSet unicodeSet) {
