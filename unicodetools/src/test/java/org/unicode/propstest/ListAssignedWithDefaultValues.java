    ...
    public static void main(String[] args) {
        ...
        UnicodeSet unassigned =
                new UnicodeSet(cn)
                        ...
                        .freeze();
        UnicodeSet assigned = new UnicodeSet(unassigned).complement().freeze();
        UnicodeSet mark =
                new UnicodeSet(gc.getSet(General_Category_Values.Nonspacing_Mark))
            ...
            switch (prop.getType()) {
                ...
                default:
                    ...
                    UnicodeSet assignedWithDefaultValues =
                            new UnicodeSet(defaultValues).retainAll(assigned);
                    UnicodeSet unassignedWithoutDefaultValues =
                            new UnicodeSet(cn).removeAll(defaultValues);
                    System.out.println(
                            prop.getType()
                                    + "\t"
                                    + prop
                                    ...
                                    + "\t"
                                    + (!Objects.equals(defaultValue, defaultValue2)
                                            ? defaultValue2
                                            : ""));
                    if (prop == UcdProperty.Script) {
                        for (String s : assignedWithDefaultValues) {
                            System.out.println(
                                    Utility.hex(s)
                                            + "; "
                                            + (mark.contains(s) ? "Inherited" : "Common")
                                            + "\t"
                                            + latest.getName(s, ", "));
                        }
                    }
