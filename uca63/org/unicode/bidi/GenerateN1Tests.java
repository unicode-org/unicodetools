...
public class GenerateN1Tests {
  enum Sample {AL("\u0627\u062F\u0630\u0631"), R("\u05D0\u05D1\u05d2\u05d3"), L("abcd"), AN("\u0660\u0661\u0662\u0663"), EN("6789"), N("!&?@");
    ...
    Sample(String instr) {
      this.str[0] = instr.substring(0,2);
      this.str[1] = instr.substring(2);
    }
  }
  static class SampleEnumerator {
    ...
    boolean next() {
      for (int i = items.size()-1; i >= 0; --i) {
        Sample oldValue = items.get(i);
        Sample newValue = oldValue.ordinal() < Sample.values().length - 1 ? Sample.values()[oldValue.ordinal()+1] : null; // next value
        if (newValue != null) {
    ...
    public String toString(boolean codes) {
      EnumSet<Sample> seen = EnumSet.noneOf(Sample.class);
  ...
  public static void main(String[] args) {
    SampleEnumerator samples = new SampleEnumerator(5);
    System.out.println("\uFEFF");
    int count = 1;
    while (samples.next()) {
      if (samples.getItems().size() != 5) continue;
      if (samples.getItems().get(2) != Sample.N) continue;
      if (samples.getItems().get(1) == Sample.N
              || samples.getItems().get(3) == Sample.N
              ) continue;
      if (!Sample.STRONG.contains(samples.getItems().get(0))) continue;
      if (!Sample.STRONG.contains(samples.getItems().get(4))) continue;
      if (!(Sample.NUMERIC.contains(samples.getItems().get(1))
              || Sample.NUMERIC.contains(samples.getItems().get(3)))) continue;
      if (Sample.STRONG.contains(samples.getItems().get(1))
              && samples.getItems().get(1) != samples.getItems().get(0)) continue;
      if (Sample.STRONG.contains(samples.getItems().get(3))
              && samples.getItems().get(3) != samples.getItems().get(4)) continue;
