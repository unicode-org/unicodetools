...
public final class TestCompatibilityCharacters {
    ...
    static void testCompatibilityCharacters(UCA uca) throws IOException {
        ...
        PrintWriter testLog = Utility.openPrintWriter(UCA.getUCA_GEN_DIR() + File.separator + "log", fullFileName, Utility.UTF8_WINDOWS);
        ...
        for (int i = 0; i < 0xFFFF; ++i) {
            ...
            int kenLen = uca.getCEs(s, decompType, kenCes); // true
            int markLen = fixCompatibilityCE(uca, s, true, markCes, false);

            if (!arraysMatch(kenCes, kenLen, markCes, markLen)) {
                int kenCLen = fixCompatibilityCE(uca, s, true, kenComp, true);
        ...
        while (it.hasNext()) {
            ...
            int kenLen = uca.getCEs(s, decompType, kenCes);
