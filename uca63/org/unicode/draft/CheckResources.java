...
public class CheckResources {
  ...
  private static void printData(Counter<String> counter2, boolean showKeys) throws UnsupportedEncodingException {
    ...
    for (String key : counter2.getKeysetSortedByCount(false)) {
      final long count = counter2.getCount(key);
      if (showKeys) {
        String trunc = key.length() < 20 ? key : key.substring(0,19) + "...";
        System.out.println(count + "\t" + trunc);
      }
      long utf16Length = (key.length()+1) * 2;
      uniqueUtf16Size += utf16Length;
      totalUtf16Size += utf16Length * count;

      long utf8Length = key.getBytes("utf-8").length + 1;
      totalUtf8Size += utf8Length * count;
      uniqueUtf8Size += utf8Length;

      long scsuLength = UnicodeCompressor.compress(key).length + 1;
  ...
  private static void printDataCompressed(Counter<String> counter2, boolean show) throws UnsupportedEncodingException {
    ...
    Counter<Character> charCount = new Counter();

    Set<String> count_key = new TreeSet<String>(REVERSE);
    count_key.addAll(counter2.keySet());
    String lastKey = "";
    for (String key : count_key) {
      final long count = counter2.getCount(key);
      String trunc = key.length() < 20 ? key : key.substring(0,19) + "...";
      if (show) System.out.print(count + "\t" + trunc);
    ...
    int i = 0;
    for (Character key : charCount.getKeysetSortedByCount(false)) {
  ...
  static class SortReverseLengthFirst implements Comparator<String> {
    public int compare(String o1, String o2) {
      ...
      int p2 = o2.length();
      while (true) {
        int ch1 = p1 <= 0 ? 0x10000 : o1.charAt(--p1);
        int ch2 = p2 <= 0 ? 0x10000 : o2.charAt(--p2);
        int diff = ch1 - ch2;
        if (diff != 0) {
          return diff;
        }
