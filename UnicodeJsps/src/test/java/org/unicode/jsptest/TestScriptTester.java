...
public class TestScriptTester extends TestFmwkMinusMinus {
    ...
    public void TestBasic() {
        ...
        String[] bad = {
            "gー", "ä\u0308", "1٦", "٦۶", "1aᎠ", "aᎠ1", "aᎠ", // Cherokee
    ...
    private void checkFilter(String testChar) {
        List<Set<String>> listTrial =
                Builder.with(new ArrayList<Set<String>>())
                        .add(
    ...
    public void TestConfusables() {
        TreeSet<String> expected =
                Builder.with(new TreeSet<String>()).addAll("google", "goog1e", "googIe").get();
    ...
    private void checkScriptCheck(String string, TreeSet<String> expected) {
        ...
        if (expected != null) {
            assertEquals("Confusables for '" + string + "'", expected, items);
        }

        Confusables confusables2 =
                new Confusables(string)
                        .setNormalizationCheck(Normalizer.NFKC)
                        .setAllowedCharacters(XIDModifications.getAllowed());
        ...
        assertEquals("Confusables for '" + string + "'", items, filteredDifferently);
    }
}
