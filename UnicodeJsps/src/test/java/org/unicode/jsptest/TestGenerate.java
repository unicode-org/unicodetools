...
public class TestGenerate extends TestFmwk {
    ...
    static final UnicodeSet OVERALL_ALLOWED =
            new UnicodeSet().applyPropertyAlias("age", AGE == null ? "5.2" : AGE).freeze();
            ...
            if (!uts46.equals(idna2003)) {
                System.out.println(
                        Utility.hex(s)
                                ...
                                + Utility.hex(uts46)
                                + "\t"
                                + Utility.hex(idna2003)
                                + "\t"
                                + name.transform(s)
                                + "\t"
                                + name.transform(uts46)
                                + "\t"
                                + name.transform(idna2003));
            }
        }
    }
    ...
    public void TestGenerateDataFile() throws IOException {
        ...
        for (int cp = 0; cp <= 0x10FFFF; ++cp) {
            ...
            if (Uts46.SINGLETON.getType(cp)
                    == IdnaType.deviation) { //  Uts46.SINGLETON.DEVIATIONS.contains(cp)
                status = "deviation";
            }
        ...
        //    String names = (sourceName != null && targetName != null) ? "#\t" + sourceName + "
        // \u2192 " + targetName : "";
    ...
    //  private void writeIdnaDataFile(final UnicodeMap<String> hex_results, BagFormatter bf, String
    // normalizationForm2, String filenameStem) throws IOException {
    //    String filename = filenameStem + "-" + AGE + ".0.txt";
    //    PrintWriter writer =
    // BagFormatter.openUTF8Writer("/Users/markdavis/Documents/workspace/draft/reports/tr46/",
    // filename);
