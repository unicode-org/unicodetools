 ...
 * </ol>
 *
 * Note that for conformance, algorithms are only required to generate correct reordering and
 * character directionality (odd or even levels) over a line. Generating identical level arrays over
 * a line is not required. Bidi explicit format codes (LRE, RLE, LRO, RLO, PDF) and BN can be
 * assigned arbitrary levels and positions as long as the other text matches.
 *
 ...
 * visual order (e.g. to the left of their base character in RTL runs) and that it adjust the glyphs
 * used to render mirrored characters that are in RTL runs so that they render appropriately.
 *
 * @author Doug Felt
 */
public final class BidiReference {
     ...
     * level. The embedding level may be -1, 0, or 1. -1 means to apply the default algorithm (rules
     * P2 and P3), 0 is for LTR paragraphs, and 1 is for RTL paragraphs.
     *
     * @param types the types array
     ...
     * <p>Rules P2, P3.
     *
     * <p>At the end of this function, the member variable paragraphEmbeddingLevel is set to either
     * 0 or 1.
     */
     ...
     *
     * <p>Calls processEmbeddings to generate an embedding array from the explicit format codes. The
     * embedding overrides in the array are then applied to the result types, and the result levels
     * are initialized.
     *
    ...
    /**
     * Rules X9. Remove explicit codes so that they may be ignored during the remainder of the main
     * portion of the algorithm. The length of the resulting text is returned.
     *
    ...
    /**
     * Reinsert levels information for explicit codes. This is for ease of relating the level
     * information to the original input data. Note that the levels assigned to these codes are
     * arbitrary, they're chosen so as to avoid breaking level runs.
     *
     ...
     *
     * <p>The interaction of these rules makes handling them a bit complex. This examines
     * resultTypes but does not modify it. It returns embedding and override information in the
     * result array. The low 7 bits are the level, the high bit is set if the level is an override,
     * and clear if it is an embedding.
     */
            ...
            switch (t) {
                ...
                case LRO:
                    ...
                    if (overflowCounter == 0) {
                        ...
                        if (t == RLE || t == RLO) {
                            newLevel =
                                    (byte) ((currentEmbeddingLevel + 1) | 1); // least greater odd
                        } else { // t == LRE || t == LRO
                            newLevel =
                                    (byte) ((currentEmbeddingLevel + 2) & ~1); // least greater even
                        }
                        ...
                        if (newLevel < EXPLICIT_LEVEL_LIMIT) {
                            ...
                            // Adjust level of format mark (for expositional purposes only, this
                            // gets
                            // removed later).
                            embeddings[i] = currentEmbeddingValue;
                            break;
                        ...
                        // Otherwise new level is invalid, but a valid level can still be achieved
                        // if this
                ...
                case PDF:
                    // The only case where this did not actually overflow but may have almost
                    // overflowed
                    // is when there was an RLE or RLO on level 60, which would result in level 61.
                    // So we
    ...
    /**
     * Return the start of the run including index that includes only resultTypes in validSet. This
    ...
    /** Algorithm validation. Assert that all values in resultTypes are in the provided set. */
    ...
    public static interface BidiTraceHook {
         ...
         *
         * <p>The data supplied to the display method represents the current internal state of the
         * implementation. Note that some phases of the algorithm operate on the data as it appears
         * when the explicit formatting codes and BN have been removed. When this is the case, start
         * and limit do not correspond directly to the original direction type codes that were
         * passed to the constructor. However, the values in embeddings, resultTypes, and
         * resultLevels are consistent.
         *
         * <p>
         *
         ...
         * @param embeddings the embeddings and override information resulting from explicit
         *     formatting codes
         ...
         */
        public abstract void displayLineLevels(
                byte paragraphEmbeddingLevel,
                byte[] initialTypes,
                byte[] embeddings,
                int[] linebreaks,
    ...
    public static String getHtmlTypename(int value) {
        return "<a target='list' href='list-unicodeset.jsp?a=[:bc="
                + typenames[value]
                + ":]'>"
                + typenames[value]
                + "</a>";
