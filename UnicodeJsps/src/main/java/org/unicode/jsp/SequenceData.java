...
public class SequenceData {
    ...
    static final UnicodeSet MODIFIER = new UnicodeSet("[:emoji_modifier:]").freeze();
    static final UnicodeSet TAGS = new UnicodeSet(0xe0020, 0xe007F).freeze();
    ...
    static final UnicodeSet EMOJI_TAG_SEQUENCES = new UnicodeSet();
    ...
    static {
        for (String file : Arrays.asList("emoji-sequences.txt", "emoji-zwj-sequences.txt")) {
            ...
            for (String line : FileUtilities.in(SequenceData.class, file)) {
                ...
                if (MODIFIER.containsSome(source)) {
                    ...
                    if (REGIONAL_INDICATORS.contains(first)) {
                        ...
                        // TODO fix data
                        final StringBuilder sourceWithVs =
                                new StringBuilder()
                    ...
                    } else if (TAGS.containsSome(source)) {
                        EMOJI_TAG_SEQUENCES.add(source);
                    } else {
                        int[] codepoints = CharSequences.codePoints(source);
                        throw new IllegalArgumentException(
                                "internal error" + Arrays.asList(codepoints));
                    }
                }
            }
        }
        for (String line : FileUtilities.in(SequenceData.class, "StandardizedVariants.txt")) {
            ...
            if (selectors == null) {
                ...
                VARIATION_BASE_TO_SELECTORS.put(
                        first, selectors = new UnicodeSet(selectors).add(second));
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(
                "EMOJI_MODIFIER_SEQUENCES: " + EMOJI_MODIFIER_SEQUENCES.toPattern(false));
        System.out.println("EMOJI_DEFECTIVES: " + EMOJI_DEFECTIVES.toPattern(false));
        System.out.println("EMOJI_TAG_SEQUENCES: " + EMOJI_TAG_SEQUENCES.toPattern(false));
