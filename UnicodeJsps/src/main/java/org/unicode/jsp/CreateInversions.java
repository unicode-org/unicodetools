...
public class CreateInversions {
    ...
    public static void main(String[] args) {
        UnicodeSet ignorables =
                new UnicodeSet("[[:Cn:][:Cs:][:Co:]]")
                        .freeze(); // exclude unassigned, surrogates, and private use
        CreateInversions createInversions =
                new CreateInversions().setIgnorables(ignorables).setDelta(true);
        ...
        UnicodeMap[] testProperties = {
            ...
            propFactory
                    .getProperty("grapheme_cluster_break")
                    .getUnicodeMap()
                    .putAll(new UnicodeSet(0xAC00, 0xD7A3), "LVT"),
        ...
        // further compaction can be done by assigning each property value to a number, and using
        // that instead.
        UnicodeMap source =
                propFactory
                        .getProperty("grapheme_cluster_break")
    ...
    public Appendable create(String name, UnicodeSet source, Appendable target) throws IOException {
        initShortestForm();
    ...
    public Appendable create(String name, UnicodeMap source, Appendable target) throws IOException {
        initShortestForm();
        target.append("var " + name + " = new Inversion([\n");
