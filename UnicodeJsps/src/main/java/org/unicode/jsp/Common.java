...
public class Common {
    static final Normalizer2 NFKC_CF_ =
            Normalizer2.getInstance(null, "nfkc_cf", Normalizer2.Mode.COMPOSE);

    //    public static class NFKC_CF implements StringTransform {
    //        private static final UnicodeSet DEFAULT_IGNORABLES = new
    // UnicodeSet("[:di:]").freeze();
    //
    //        // static Matcher DI =
    //        // Pattern.compile(UnicodeRegex.fix("[:di:]")).matcher("");
    //        // UnicodeMap<String> DI2 = new
    //        // UnicodeMap<String>().putAll(DEFAULT_IGNORABLES, "");
    //        public String transform(String source) {
    //
    //            // String di = DI2.transform(source);
    //            String di = DEFAULT_IGNORABLES.stripFrom(source, true);
    //            String cf = Common.myFoldCase(di);
    //            String nfcf = Common.MyNormalize(cf, Normalizer.NFKC);
    //            String nfcfnf = Common.myFoldCase(nfcf);
    //            // String s3 = DI.reset(s2).replaceAll("");
    //            String result = Common.MyNormalize(nfcfnf, Normalizer.NFKC);
    //            return result;
    //        }
    //    }

    public static Transform<String, String> NFKC_CF =
            new StringTransform() {
                public String transform(String source) {
                    return NFKC_CF_.normalize(source);
                }
            };
    ...
    public static String getXStringPropertyValue(int propertyEnum, int codepoint, int nameChoice) {
        ...
        switch (propertyEnum) {
            case Common.TO_NFC:
                return Common.MyNormalize(codepoint, Normalizer.NFC);
            case Common.TO_NFD:
                return Common.MyNormalize(codepoint, Normalizer.NFD);
            case Common.TO_NFKC:
    ...
    static String myFoldCase(String source) {
        return UCharacter.foldCase(source, true);
    }

    public static HanTypeValues getValue(int codepoint) {
        if (Common.simpOnly.contains(codepoint)) return HanTypeValues.Hans;
        if (Common.tradOnly.contains(codepoint)) return HanTypeValues.Hant;
        if (Common.bothSimpTrad.contains(codepoint)) return HanTypeValues.Han;
        return HanTypeValues.na;
    }
    ...
    public static final UnicodeSet simpOnly =
            new UnicodeSet(
                    ...
                    .freeze();
    public static final UnicodeSet tradOnly =
            new UnicodeSet(
                    ...
                    .freeze();
    public static final UnicodeSet bothSimpTrad =
            new UnicodeSet("[:sc=han:]").removeAll(simpOnly).removeAll(tradOnly).freeze();

    static String MyNormalize(String string, Mode mode) {
        return Normalizer.normalize(string, mode);
    }

    static final int TO_NFC = UProperty.STRING_LIMIT;
    ...
    //            if (cat == UCharacter.UNASSIGNED || cat == UCharacter.PRIVATE_USE || cat ==
    // UCharacter.SURROGATE) {
    ...
    //        Common.isCased = new
    // UnicodeSet(Common.isLowercase).retainAll(Common.isUppercase).retainAll(
