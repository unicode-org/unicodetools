...
public class Uts46 extends Idna {
  ...
  /* http://datatracker.ietf.org/doc/draft-ietf-idnabis-bidi/, version 07
  ...
  public static boolean hasContextJError(String domain, Set<Errors> errors) {
    ...
    for (int i = 0; i < domain.length(); i += Character.charCount(cp)) {
      ...
      if (cp == NON_JOINER) {
        ...
        // the way the following code is set up, we continue if we *don't* get an error
  ...
  public enum IdnaChoice {transitional, nontransitional}
  ...
  private String processMap(String domainName, IdnaChoice idnaChoice, Set<Errors> errors) {
    ...
    for (int i = 0; i < domainName.length(); i+= Character.charCount(cp)) {
      ...
      case mapped: 
        ...
        //    For Nontransitional Processing, leave the code point unchanged in the string.
      case deviation: 
  ...
  public String toASCII(String domainName, IdnaChoice idnaChoice, Set<Errors> errors) {
    //      Apply the appropriate processing. This may record an error. The appropriate processing is either:
