...
5. Prohibited Output
  ...
  public static void getIdna2003Tables(UnicodeMap<String> mappings, UnicodeMap<IdnaType> types) {
    ...
    for (int i = 0; i <= 0x10FFFF; ++i) {
      ...
      boolean isProhibited = mapping == null ? isProhibited(i, allowed) : isProhibited(mapping,allowed);
      IdnaType status;
  ...
  private static String getMapping(int cp, EnumSet<Idna2003Table> allowed) {
    ...
    String normalizedMapping = mapping != null ? Normalizer.normalize(mapping, Normalizer.NFKC) : Normalizer.normalize(cp, Normalizer.NFKC);
  ...
  static EnumSet<Idna2003Table> PROHIBITED = EnumSet.range(Idna2003Table.C_1_1, Idna2003Table.C_9);
  ...
  static class DataSet {
    ...
    public DataSet add(boolean myisProhibited, String mymapping, String mycomment) {
      ...
      if (mymapping == null) {
        ...
        throw new IllegalArgumentException("Conflicting mapping " + Utility.hex(mapping) + ", " + Utility.hex(mymapping));
      }
      if (mycomment == null) {
        mycomment = comment;
    ...
    public DataSet fix(int codepoint, UnicodeMap<DataSet> data) {
      if (mapping != null) {
        ...
        if (DEBUG) System.out.println("Changing value for " + Utility.hex(codepoint) + ":\t[" + this + "] => [" + newDataSet + "]");
        return newDataSet;
      }
      return null;
    }
  ...
  static {
    ...
    try {
      ...
      for (int count = 1; ; ++count) {
        ...
        //          if (DEBUG) System.out.println(count + ")\t" + table + "\tSKIPPING line:\t" + line);
        //          continue;
        //        } else {
        //          if (DEBUG) System.out.println(count + ")\t" + table + "\tDoing line:\t" + line);
        //        }
    ...
    for (int i = 'A'; i <= 'Z'; ++i) {
      ...
      if (tableSet == null) {
        tableSet = new DataSet(PROHIBITED.contains(Idna2003Table.B_1), UTF16.valueOf(i-'A'+'a'), (String)null);
      } else {
        tableSet = tableSet.add(PROHIBITED.contains(Idna2003Table.B_1), UTF16.valueOf(i-'A'+'a'), (String)null);
      }
      data.put(i, tableSet);
    }
  ...
  private static void addMapping(int startCode, int endCode, StringPrepData.Idna2003Table type, String mapping, String comment) {
    for (int i = startCode; i <= endCode; ++i) {
      addData(i, type, mapping, comment);
    }
  }

  private static void addData(int i, StringPrepData.Idna2003Table type, String mapping, String comment) {
    try {
      if (i == 0x200c) {
        System.out.print("");
      }
