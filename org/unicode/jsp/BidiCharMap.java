...
class BidiCharMap {
	...
	static {
		...
		for (byte i = 0; i < mapIcuToRefNum.length; ++i) {
			final int icuValue = UCharacter.getPropertyValueEnum(UProperty.BIDI_CLASS, BidiReference.typenames[i]);
		...
		for (final UnicodeSetIterator it = new UnicodeSetIterator(new UnicodeSet("[[:ascii:]-[[:cc:]-[:whitespace:]]]")); it.next();) {
	...
	public static byte getBidiClass(int codepoint, boolean asciiHack2) {
		if (asciiHack2) {
			final Byte result = (Byte) asciiHackMap.getValue(codepoint);
			if (result != null) {
				return result;
			}
		}
