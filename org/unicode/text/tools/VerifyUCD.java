...
public class VerifyUCD {
  ...
  public static void doEquivalenceOfProperties(Equivalence equivalence) {
    ...
    ToolUnicodePropertySource ups = ToolUnicodePropertySource.make(Default.ucdVersion());
    UnicodeSet setToTest = ups.getSet(equivalence == Equivalence.CANONICAL ? "NFDQuickCheck=No" : "NFKDQuickCheck=No");
    ...
    for (UnicodeSetIterator it = new UnicodeSetIterator(setToTest); it.next();) {
      int codepoint = it.codepoint;
      String normalized = (forceNFC.contains(codepoint) 
  ...
  private static void showProperties(UnicodeMap results, Map<String, UnicodeMap> sidewaysResults, final Equivalence
    ...
    UnicodeLabel nameLabel = new UnicodeLabel() {
      public String getValue(int codepoint, boolean isShort) {
        String nfd = (equivalence == Equivalence.CANONICAL ? Default.nfd() : Default.nfkd()).normalize(codepoint);
  ...
  static public void checkSegmentation() {
    ...
    for (String type : new String[] {"GraphemeClusterBreak", "WordBreak", "LineBreak", "SentenceBreak"}) {
      ...
      for (int cp = 0; cp <= 0x10FFFF; ++cp) {
        ...
        if (cat == UCD_Types.Cs || cat == UCD_Types.Cn || cat == UCD_Types.Co) continue;
        String nfd = Default.nfd().normalize(cp);
        if (!UTF16.hasMoreCodePointsThan(nfd, 1)) continue;
        for (int i = 1; i < nfd.length() - 1; ++i) {
          boolean b = seg.breaksAt(nfd, i);
          if (b) {
            seg.breaksAt(nfd, i);
