...
public class GenerateCeMapping {
  ...
  public static void main(String[] args) throws Exception {
    ...
    UnicodeSet charsToTest = new UnicodeSet("[[:assigned:]-[:ideographic:]-[:hangul:]-[:c:]]")
    .addAll(contractions)
    .addAll(expansions)
    .addAll(rbc.getTailoredSet());

    CollationElementIterator cei = rbc.getCollationElementIterator("");
    
    System.out.println("Items to check: " + charsToTest.size());
    // not the most efficient way, but for testing...
    
    for (UnicodeSetIterator it = new UnicodeSetIterator(charsToTest); it.next();) {
      final String item = it.getString();
      cei.setText(item);
      CeList ceList = new CeList(cei, item);
    ...
    for (Iterator<Integer> it = ceToCharsContaining.keySet().iterator(); it.hasNext();) {
      Integer item = it.next();
      Set<String> set = ceToCharsContaining.get(item);
      int maxSize = 0;
      int minSize = Integer.MAX_VALUE;
      for (Iterator<String> it2 = set.iterator(); it2.hasNext();) {
        String string = it2.next();
        CeList ceList = charsToCeList.get(string);
    ...
    // print stats
    if (true)
    for (int item : ceToCharsContaining.keySet()) {
      Set<String> set = ceToCharsContaining.get(item);
    ...
    for (String example : examples) {
      CeList exampleCeList = new CeList(cei, example);
  ...
  public static int minLengthInChars(CeList ceList, int offset, StringBuilder debug) {
    ...
    while (offset < ceList.size()) {
      int ce = ceList.get(offset);
      int bestLength = Integer.MIN_VALUE;
      String bestString = null;
      int bestCeLength = 0;
      for (String s : ceToCharsStartingWith.get(ce)) {
        CeList ceList2 = charsToCeList.get(s);
        if (ceList.matchesAt(offset, ceList2)) {
          int length = ceList2.size() - s.length();
