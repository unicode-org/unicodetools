...
U+01D5 LATIN CAPITAL LETTER U WITH DIAERESIS AND MACRON
 => U+00DC LATIN CAPITAL LETTER U WITH DIAERESIS, U+0304 COMBINING MACRON
    ...
    static void addStringX(String s, byte option) {
        ...
        addStringY(s + 'A', option);
        addStringY(s + '!', option);
        if (option == SHIFTED && collator.isVariable(firstChar)) {
    ...
    static void writeCategoryCheck() throws IOException {
        ...
        for (int i = 0; i < 0x10FFFF; ++i) {
            ...
            if (cat == Cc || cat == Cs || cat == Cf || ucd.isNoncharacter(i)) {
        ...
        for (int i = 1; i < len; ++i) {
            ...
            if (map.get(tail) != null) {
                ...
                if (DEBUG_SHOW_OVERLAP) {
                    System.out.println(Utility.repeat("**", depth) + "  Match tail at " + i + ": " + tail + ", " + ucd.getCodeAndName(map.get(tail).toString()));
                }
    ...
    static String getHTML_NameSet(Collection set, Map m, boolean useName) {
        ...
        while (it.hasNext()) {
            ...
            if (useName) {
                item = ucd.getCodeAndName(item.toString());
            }
            result.append(item);
        }
        return result.toString();
...
2FA1D;CJK COMPATIBILITY IDEOGRAPH-2FA1D;Lo;0;L;2A600;;;;N;;;;;
    ...
    static void checkImplicit() {
        ...
        showImplicit("# First CJK_C", CJK_C_BASE);
        showImplicit("# Last CJK_C", CJK_C_LIMIT-1);
        showImplicit("# First CJK_D", CJK_D_BASE);
        showImplicit("# Last CJK_D", CJK_D_LIMIT-1);
    ...
    static final boolean needsCaseBit(String x) {
        ...
        if (!ucd.getCase(s, FULL, LOWER).equals(s)) {
            return true;
        }
        if (!toSmallKana(s).equals(s)) {
            return true;
        }
        return false;
    }
...
308F;HIRAGANA LETTER WA;Lo;0;L;;;;;N;;;;;
    ...
    static int fixPrimary(int x) {
        int result = 0;
        if ((x & MARK_CODE_POINT) != 0) {
            result = getImplicitPrimary(x & ~MARK_CODE_POINT);
        } else {
...
# 0158: (EE4D) 2776 [0858.0158.0006] DINGBAT NEGATIVE CIRCLED DIGIT ONE
            ...
            if (type < FIXED_CE) {
                ...
                if (scripts[script] == 0 || scripts[script] > primary) {
                    ...
                    if (cat <= ucd.OTHER_LETTER && cat != ucd.Lm) {
                        ...
                        if (script == ucd.GREEK_SCRIPT) {
                            System.out.println("*" + Utility.hex(primary) + ucd.getName(ch));
                        }
                    }
                }
    ...
    private static void checkUnassigned() {
        ...
        // the invariants are that there must be exactly 2 ces; the ces will match codepointToImplicit
...
A4C6;YI RADICAL KE;So;0;ON;;;;;N;;;;;
    ...
    static boolean containsCombining(String s) {
        for (int i = 0; i < s.length(); ++i) {
            if ((ucd.getCategoryMask(s.charAt(i)) & ucd.MARK_MASK) != 0) {
                return true;
            }
        }
        return false;
