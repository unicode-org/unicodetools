...
public final class Fractional {
    ...
    /**
     * Top of the byte range for compressing a sequence of
     * common secondary weights into a single sort key byte.
     *
     * <p>The bytes {@link #COMMON_SEC_TOP}+1 .. {@link #FIRST_SEC_ASSIGNED}-1
     * provide a large gap for tailoring secondary-after
     * a primary CE with a common secondary.
     */
    ...
    /**
     * First secondary weight byte of any secondary CE (primary ignorable).
     * Not actually assigned in FractionalUCA.txt.
     *
     * <p>The bytes {@link #FIRST_IGNORABLE_SEC} .. {@link #FIRST_IGNORABLE_SEC_ASSIGNED}-1
     * provide a gap for tailoring secondary-before the first secondary CE.
     */
     ...
     *
     * <p>The bytes {@link #FIRST_IGNORABLE_SEC_RESERVED} .. 0xFF
     * provide a gap for tailoring secondary-after the last secondary CE.
     */
    public static final int FIRST_IGNORABLE_SEC_RESERVED = 0xFC;

    /**
     * Common (default) tertiary weight.
     * It has been the same as {@link #COMMON_SEC} but need not be.
     *
     * <p>The bytes {@link #COMMON_TER}+1 .. {@link #FIRST_TER_ASSIGNED}-1
     * provide a gap for tailoring tertiary-after
     * a primary or secondary CE with a common tertiary.
     */
    ...
    /**
     * First non-common tertiary weight byte used in FractionalUCA.txt
     * for primary+secondary with very few distinct tertiary weights.
     */
    public static final int FIRST_TER_FEW_ASSIGNED = 0x20;
    /**
     * First tertiary weight byte of any tertiary CE (secondary ignorable).
     * Not actually assigned in FractionalUCA.txt.
     *
     * <p>The bytes {@link #FIRST_IGNORABLE_TER} .. {@link #FIRST_IGNORABLE_TER_ASSIGNED}-1
     * provide a gap for tailoring tertiary-before the first tertiary CE.
     */
    public static final int FIRST_IGNORABLE_TER = 0x3C;
    /**
     * First tertiary weight byte of any tertiary CE in FractionalUCA.txt.
     * There are none in UCA 6.3 or earlier.
     */
     ...
     *
     * <p>The bytes {@link #FIRST_IGNORABLE_TER_RESERVED} .. 0x3F
     * provide a gap for tailoring tertiary-after the last tertiary CE.
     */
    ...
    public static class WeightIterator {
         ...
         * For a mix of weight lengths, the second byte is incremented by about desiredInc2.
         * If there are very few weights,
         * then they are all single-byte weights with at least a one-byte gap.
         * If there are very many weights,
         * then they are all two-byte weights with at least a two-byte gap.
         *
        ...
        private WeightIterator(
                int b1, int limitByte1, int limitByte2,
                int numWeights, int desiredInc2) {
            this.b1 = b1;
            ...
            int maxWeights = numBytes1 * numBytes2;
        ...
        /**
         * Returns a single-byte or two-byte weight and increments the iterator state.
         * Returns 0 after all requested weights have been returned.
         */
    ...
    public static WeightIterator assignSecondaryWeightsForPrimaryCEs(int numWeights) {
        return new WeightIterator(
                FIRST_SEC_ASSIGNED, FIRST_IGNORABLE_SEC, 0x100,
                numWeights, 32);
    }

    public static WeightIterator assignSecondaryWeightsForSecondaryCEs(int numWeights) {
        return new WeightIterator(
                FIRST_IGNORABLE_SEC_ASSIGNED, FIRST_IGNORABLE_SEC_RESERVED, 0x100,
                numWeights, 32);
    }
    ...
    public static WeightIterator assignTertiaryWeightsForSecondaryCEs(int numWeights) {
        return new WeightIterator(
                ...
                FIRST_IGNORABLE_TER, 0x40,
                numWeights, 20);
    }

    public static WeightIterator assignTertiaryWeightsForTertiaryCEs(int numWeights) {
        return new WeightIterator(
                FIRST_IGNORABLE_TER_ASSIGNED, FIRST_IGNORABLE_TER_RESERVED, 0x40,
                numWeights, 20);
    }
