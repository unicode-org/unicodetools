...
public class CompareDucetToCldr {
    ...
    private static final String BASE_DIR = Utility.DATA_DIRECTORY + "/UCA/6.1.0/";
    ...
    public static void main(String[] args) throws IOException {
        Birelation<WeightList, String> cldr = getData(BASE_DIR + "CollationAuxiliary/", "allkeys_CLDR.txt");
        writeValues(cldr, "cldr.txt", false);
        writeValues(cldr, "cldr_weights.txt", true);
        Birelation<WeightList, String> ducet = getData(BASE_DIR, "allkeys.txt");
    ...
    public static void writeValues(Birelation<WeightList, String> cldr, String filename, boolean showWeights) throws IOException {
        PrintWriter out = BagFormatter.openUTF8Writer(UCD_Types.GEN_DIR + "uca_COMP/", filename);
        out.println("#Date: " + DATE);
        for (Entry<WeightList, Set<String>> kvs : cldr.keyValuesSet()) {
            WeightList weights = kvs.getKey();
            for (String source : kvs.getValue()) {
                out.println (
    ...
    public static Birelation<WeightList, String> getData(String directory, String filename) {
        Birelation<WeightList, String> cldr = Birelation.of(
                new TreeMap<WeightList, Set<String>>(), TreeSet.class, 
                new TreeMap<String, Set<WeightList>>(), TreeSet.class);
        for (String line : FileUtilities.in(directory, filename)) {
            ...
            if (parts != null) {
                String string = Utility.fromHex(parts[0]);
    ...
    private static boolean isCanonical(String string) {
        ...
        for (int cp : With.codePointArray(string)) {
            int dt = UCharacter.getIntPropertyValue(cp, UProperty.DECOMPOSITION_TYPE);
    ...
    private static class WeightList implements Comparable<WeightList> {
        ...
        public int compareTo(WeightList other) {
            for (int i = 0; i < list.size() && i < other.list.size(); ++i) {
                int result = list.get(i).compareTo(other.list.get(i));
    ...
    private static class Weight implements Comparable<Weight> {
        ...
        public int compareTo(Weight other) {
            int result;
            if (0 != (result = primary - other.primary)) return result;
            if (0 != (result = secondary - other.secondary)) return result;
            if (0 != (result = tertiary - other.tertiary)) return result;
