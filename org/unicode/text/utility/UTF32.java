...
public final class UTF32 {
		...
		try { // use exception to catch out-of-bounds
			...
			if (isLeadSurrogate(single)) {
				...
				if (isTrailSurrogate(trail)) {
					return (single << SURROGATE_SHIFT) + trail + SURROGATE_OFFSET;
				}
			} else { // isTrailSurrogate(single), so
				final char lead = source.charAt(--offset16);
				if (isLeadSurrogate(lead)) {
					return (lead << SURROGATE_SHIFT) + single + SURROGATE_OFFSET;
				}
			}
	...
	public static int append32(char[] output, int oPosition, int oEnd, int cp) {
		if (oPosition >= oEnd) {
			throw new ArrayIndexOutOfBoundsException(oPosition);
		}
	...
	public static void setChar32At(StringBuffer b, int position, int codePoint) {
		final int type = bounds32(b, position);
		...
		case LEAD:
			...
			b.setCharAt(position++, getLead(codePoint));
			b.setCharAt(position, getTrail(codePoint));
			return;
		case TRAIL:
			if (!isSupplementary(codePoint)) {
				--position;
	...
	public static boolean isLegal(int char32) {
		...
		if ((char32 & PLANE_MASK) >= NON_CHARACTER_BASE) {
			return false;
		}
	...
	public static String valueOf32(int char32) {
		if (char32 < 0 || MAX_UNICODE < char32) {
	...
	public static int bounds32(char[] source, int oStart, int oEnd, int offset16) {
		...
		if (isSurrogate(ch)) {
			if (isLeadSurrogate(ch)) {
				...
				if (--offset16 >= oStart
						&& isLeadSurrogate(source[offset16])) {
					return TRAIL;
				}
			}
		}
	...
	public static void append32(StringBuffer target, int char32) {
		...
		if (char32 < 0 || char32 > MAX_UNICODE) {
			char32 = REPLACEMENT_CHAR;
		}
	...
	public static final class StringComparator implements java.util.Comparator {
		...
		public int compare(Object a, Object b) {
			...
			final int lena = sa.length();
			final int lenb = sb.length();
			int len = lena;
			if (len > lenb) {
				len = lenb;
			}
