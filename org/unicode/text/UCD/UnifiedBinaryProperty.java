...
public final class UnifiedBinaryProperty extends UCDProperty {
	...
	static private boolean isDefined(int propMask, UCD ucd) {
		...
		switch (majorProp) {
		case CATEGORY>>8: if (propValue >= LIMIT_CATEGORY) {
			break;
		}
		...
		return true;
		case COMBINING_CLASS>>8: if (propValue >= LIMIT_COMBINING_CLASS) {
			break;
		}
		return ucd.isCombiningClassUsed((byte)propValue);
		case BIDI_CLASS>>8: if (propValue >= LIMIT_BIDI_CLASS) {
			break;
		}
		...
		return true;
		case DECOMPOSITION_TYPE>>8: if (propValue >= LIMIT_DECOMPOSITION_TYPE) {
			break;
		}
		return true;
		case NUMERIC_TYPE>>8: if (propValue >= LIMIT_NUMERIC_TYPE) {
			break;
		}
		return true;
		case EAST_ASIAN_WIDTH>>8: if (propValue >= LIMIT_EAST_ASIAN_WIDTH) {
			break;
		}
		return true;
		case LINE_BREAK>>8:  if (propValue >= LIMIT_LINE_BREAK) {
			break;
		}
		return true;
		case JOINING_TYPE>>8: if (propValue >= LIMIT_JOINING_TYPE) {
			break;
		}
		return true;
		case JOINING_GROUP>>8: if (propValue >= LIMIT_JOINING_GROUP) {
			break;
		}
		return true;
		case BINARY_PROPERTIES>>8: if (propValue >= LIMIT_BINARY_PROPERTIES) {
			break;
		}
		return true;
		case SCRIPT>>8: if (propValue >= LIMIT_SCRIPT) {
			break;
		}
		...
		return true;
		case HANGUL_SYLLABLE_TYPE>>8: if (propValue >= HANGUL_SYLLABLE_TYPE_LIMIT) {
			break;
		}
		return true;
		/*
