...
public class GenerateLanguageNames {
	...
	static void getNames() {
		...
		for (final String[] pair : corrections) {
			if ("XXXX".equals(pair[1])) {
				continue;
			}
		...
		for (final ULocale locale : ULocale.getAvailableLocales()) {
			...
			final String name = locale.getDisplayNameWithDialect(ULocale.ENGLISH);
			nameToLocale.put(name, lang);
			localeToName.put(lang, name);
		}
		...
		for (final String miss : missing) {
			System.out.println("{\"" + miss + "\", \"XXXX\"},");
		}

		for (final String name : nameToLocale.keySet()) {
			final String locale = nameToLocale.get(name);
