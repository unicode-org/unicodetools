...
public class GenerateCasedPairs {
  public static void main(String[] args) {
    UnicodeSet set = new UnicodeSet("[[:script=Cyrillic:]" +
            "-[:block=Phonetic_Extensions:]" +
            "-[:block=Cyrillic_Extended_A:]" +
            "-[:block=Cyrillic_Extended_B:]" +
            "-[:block=Cyrillic_Supplement:]" +
    		"]");
    Set<String> lower = new TreeSet(Collator.getInstance(ULocale.ENGLISH));
    Set<String> blocks = new TreeSet();
    for (UnicodeSetIterator it = new UnicodeSetIterator(set); it.next();) {
      lower.add(UCharacter.toLowerCase(ULocale.ENGLISH, it.getString()));
      final String block = UCharacter.getStringPropertyValue(UProperty.BLOCK, it.codepoint, UProperty.NameChoice.LONG);
      blocks.add(block);
    }
    System.out.println("Blocks: " + blocks);
    for (String s : lower) {
      int ch = UTF16.charAt(s, 0);
