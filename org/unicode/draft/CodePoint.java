...
public class CodePoint implements Iterator<Integer>, Iterable<Integer> {
  ...
  public Integer next() {
    int cp = Character.codePointAt(charSequence, position);
  ...
  public static void main(String[] args) {
    ...
    String[] tests = {"In a hole in the ground there lived a hobbit.", 
  ...
  private static void timeMethods(CharSequence s, int ITERATIONS) {
    ...
    if (doSomethingWith2 != doSomethingWith) throw new IllegalArgumentException();

    // easy code (but slower)
    doSomethingWith2 = 0;
    timer.start();
    for (int iteration = ITERATIONS; iteration > 0; --iteration) {
      for (CodePointIterator it = new CodePointIterator(s); it.next();) {
