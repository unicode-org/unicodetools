...
public class CldrUtility {
    ...
    public static class VariableReplacer {
        ...
        private final Map<String, String> m = new TreeMap<String, String>(Collections.reverseOrder());
        /**
         * Add a new variable
         * @param variable
         * @param value
    ...
    public static class SimpleLineComparator implements LineComparer {
        ...
        private final Matcher[] CVS_TAGS = {Pattern.compile("[$](Revision)[^$]*[$]").matcher(""), Pattern.compile("[$](Date)[^$]*[$]").matcher("")};
    ...
    public static void registerExtraTransliterators() {
        final String tzadir = UTIL_DATA_DIR + File.separatorChar; // work around bad pattern (dir+filename)
    ...
    public static Map asMap(Object[][] source, Map target, boolean reverse) {
        ...
        for (final Object[] element : source) {
            if (element.length != 2) {
                throw new IllegalArgumentException("Source must be array of pairs of strings: " + Arrays.asList(element));
            }
    ...
    public static String toRegex(UnicodeSet source, Transliterator escaper, boolean onlyBmp) {
        ...
        final Map<UnicodeSet,UnicodeSet> lastToFirst = new TreeMap<UnicodeSet,UnicodeSet>(new UnicodeSetComparator());
     ...
     * @param readUntilPattern pattern to stop at (or null to copy to end). Uses matches(), not find()
     ...
     * @throws IOException
     */
    public static String copyUpTo(BufferedReader oldFile, final Pattern readUntilPattern,
     ...
     * Copy up to matching line (or rest of file, or skip).
     * @param oldFile file to copy from
     * @param readUntil Matcher for line to stop at (or null to copy to end). Uses matches(), not find(). After returning, the matcher has the groupings set.
     * @param output output, or null to just skip
     * @param includeMatchingLine if true, the matching line is copied also
     * @return the matching line if there is a match, null if the end of the file is reached
     * @throws IOException
     */
    public static String copyUpTo(BufferedReader oldFile, Matcher readUntil, final PrintWriter output, boolean includeMatchingLine) throws IOException {
        while (true) {
