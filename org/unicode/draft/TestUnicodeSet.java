...
public class TestUnicodeSet {
  ...
  public static void main(String[] args) {
    String testdi = "abc\u0000def";
    String filtered = replace(testdi, POSSIBLY_INVISIBLE, "");
    System.out.println(Utility.hex(filtered));
    
    UnicodeSet foo = new UnicodeSet("[a-b d-g {ch} {zh}]");
    for (String s : foo) {
    ...
    UnicodeSet fii = new UnicodeSet("[a d-g {ch} {zh}]");
    ...
    int max = 1000;
    Set<String> test1 = new HashSet<String>(max);
    Set<String> test2 = new HashSet<String>(max);
    for (int i = 0; i <= max; ++i) {
      test1.add("a" + i);
      test2.add("a" + (max - i)); // add in reverse order
    }
    System.out.println("expect equal?\t" + UnicodeSet.compare(test1,test2));
    List<String> test1a = new ArrayList<String>(test1);
    List<String> test2a = new ArrayList<String>(test2);
