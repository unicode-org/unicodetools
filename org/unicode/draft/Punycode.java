...
final public class Punycode {
  ...
  public StringBuilder encode(CharSequence src, StringBuilder dest) throws StringPrepParseException{
    ...
    for(handledCPCount=basicLength; handledCPCount<srcCPCount; /* no op */) {
      ...
      if (showProgress) System.out.println("Next larger: " + Integer.toString(nextLargerCodePoint, 16));
      ...
      if (showProgress) System.out.println("\tChars: " + dest.substring(startWriting));
  ...
  static String show(int[] source) {
    String result = "";
    for (int item : source) result += item + ",";
    return result;
  }
}
