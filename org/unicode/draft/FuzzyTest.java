...
public class FuzzyTest {
  public static void main(String[] args) {
    ...
    FuzzyNumber aaa = FuzzyNumber.parse("1.");
    ...
    for (int k = 0; k < 1000; ++k) {
      FuzzyNumber a = new FuzzyNumber(r.nextInt(200) / 10.0D - 10, 0.5);
      FuzzyNumber b = new FuzzyNumber(r.nextInt(200) / 10.0D - 10, 0.5);
      FuzzyNumber added = a.add(b);
      FuzzyNumber invert = a.invert();
      FuzzyNumber subtracted = a.subtract(b);
      FuzzyNumber multiplied = a.multiply(b);
      FuzzyNumber divided = a.divide(b);
      if (false) {
      ...
      for (int i = 1; i < 10; ++i) {
        double foo = (a.lower * i + a.upper * (10 - i)) / 10;
        for (int j = 1; j < 10; ++j) {
          double bar = (b.lower * j + b.upper * (10 - j)) / 10;
          if (!assertCovers("invert", invert, 1 / foo)) {
            FuzzyNumber invert2 = a.invert();
