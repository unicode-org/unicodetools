...
public class GenerateIdnaTest {
    ...
    int generateTests(int lines) throws IOException {
        ...
        int labelLength = NEW_FORMAT ? 2 : 3;
    ...
    private boolean alreadyDone(String source) {
        ...
        String canonical = getCanonical(source);
    ...
    String getCanonical(String source) {
        ...
        Matcher m = labelSeparator.reset(source);
        for (String label : Regexes.split(m, source, true)) {
            if (IdnaTypes.LABEL_ASCII.containsAll(label)) {
                String folded = Idna.CASEFOLD.transform(label);
    ...
    private void showLine(String source, String type, String ascii, Set<Errors> asciiErrors, String unicode, Set<Errors> toUnicodeErrors, PrintWriter out) {
        ...
        Set<Errors> extraErrors = EnumSet.noneOf(Errors.class);
        boolean validIdna2008 = IDNA2008Valid.containsAll(unicode) && Uts46.hasBidiOrContextError(unicode, extraErrors ) == 0;
        out.println(type
                + ";\t" 
    ...
    static class RandomString {
        ...
        static {
            ...
            String[] samples = NEW_FORMAT ? samplesNew : samplesOld;
