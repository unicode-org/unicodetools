...
public class TestAlternateIterator extends TestFmwk {
	...
	public void TestBasic() {
		final AlternateIterator foo = AlternateIterator.start().add("a", "b", "c").add("d", "e").build();
		int count = 0;
		for (final String items : foo) {
			logln(++count + "\t" + items);
		}
	}

	public void TestConfusables() {
		final String test = "mark-davis";
		final Confusables confusables = new Confusables(test).setNormalizationCheck(Normalizer.NFKC);
