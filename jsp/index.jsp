...
<ul>
	<li><b>Unicode Property Demo</b> window<ul>
		...
		<a target="properties" href="http://unicode.org/cldr/utility/properties.jsp#Age">Age</a>), you&#39;ll see 
		...
		<a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=%5B:Age=4.0.0.0:%5D">
...
<ol>
	...
	the following. Note that in that case, &quot;&amp;&quot; needs to be replaced by &quot;%26&quot;.<ul>
		<li>&nbsp;<a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[:whitespace:]"><code>http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[:whitespace:]</code></a></li>
	</ul>
	</li>
	...
	<i><b>Warning:</b> </i>the first three sets may be somewhat misleading: isLowercase means that 
		...
		<a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp&a=[[:isLowercase:]-[:^isCased:]]">
		<code>[[:isLowercase:]&amp;[:isCased:]]</code></a><ol>
	<li>The binary testing operations take no argument:<ul>
		<li><code><a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[:isLowercase:]">[:isLowercase:]</a></code></li>
		<li><a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[:isUppercase:]"><code>[:isUppercase:]</code></a></li>
		<li><a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[:isTitlecase:]"><code>[:isTitlecase:]</code></a></li>
		<li><a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[:isCaseFolded:]"><code>[:isCaseFolded:]</code></a></li>
		<li><code><a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[:isCased:]">[:isCased:]</a>.</code></li>
	</ul>
	</li>
	<li>The string functions are also provided, and require an argument. For example:<ul>
		<li><code><a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[:toLowercase=a:]">
		[:toLowercase=a:]</a> </code>- the set of all characters X such that toLowercase(X) = a</li>
		<li><code><a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[:toCaseFold=a:]">
		[:toCaseFold=a:]</a> </code></li>
		<li><code><a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[:toUppercase=A:]">
		[:toUppercase=A:]</a> </code></li>
		<li><code><a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[:toTitlecase=A:]">
	...
	<li>The binary testing operations have somewhat odd constructions:<ul>
		<li><a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[:^NFCquickcheck=N:]">
		[:^NFCquickcheck=N:]</a> (use for [:isNFC:], and so on).</li>
		<li><a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[:^NFKCquickcheck=N:]">
		[:^NFKCquickcheck=N:]</a></li>
		<li><a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[:^NFDquickcheck=N:]">
		[:^NFDquickcheck=N:]</a></li>
		<li><a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[:^NFKDquickcheck=N:]">
	...
	<li>The string functions are also provided, and require an argument. For example:<ul>
		...
		<li><a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[:toNFKD=A\u0300:]">[:toNFKD=A\u0300:]</a></li>
	</ul>
	...
	names) can also be determined. The available properties are listed below.<ol>
		...
		disallowed (on the registry side) by IDNA. An example is:<ul>
			<li><code><a target="character" href="http://unicode.org/cldr/utility/character.jsp?a=002C">U+002C</a></code> 
			( , ) COMMA</li>
		</ul>
		...
		<a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[[:idna=disallowed:]%26[:age=3.2:]]">[[:idna=disallowed:]&amp;[:age=3.2:]]</a></code>. 
		To also remove <i>private-use, unassigned, surrogates, </i>and<i> controls</i>, use <code>
		<a target="list" href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=[[:idna=disallowed:]%26[:age=3.2:]-[:c:]]">[[:idna=disallowed:]&amp;[:age=3.2:]-[:c:]]</a></code>.</li>
	</ol>
	</li>
</ol>
