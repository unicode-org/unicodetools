...
public class UnicodeRegex implements Cloneable, Freezable, StringTransform {
    ...
    private int processSet(String regex, int i, StringBuffer result, UnicodeSet temp, ParsePosition pos) {
        ...
        temp.clear();
        try {
          temp.applyPattern(regex.substring(i), 0);
          pos.setIndex(regex.length());
        } catch (IllegalArgumentException e) {
          // ugly, ugly hack
          // throw new IllegalArgumentException("Parse of \"" + pattern + "\" failed at " + i);
          String message = e.getMessage();
          int failPos = Integer.parseInt(message.substring(message.lastIndexOf(' ') + 1));
          pos.setIndex(i + failPos);    
        }
        //UnicodeSet x = temp.clear().applyPattern(regex, pos, null, 0);
        temp.complement().complement(); // hack to fix toPattern
        result.append(temp.toPattern(false));
